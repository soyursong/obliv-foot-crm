...
-- DRIFT 상태(AC-1 ground-truth):
--   MISSING : insurance_claims / claim_items / edi_submissions  (생성 대상)
--   EXISTS  : claim_diagnoses (別마이그 20260515000010 로 선적용 — CREATE IF NOT EXISTS 로 SKIP)
...
--   → 재실행/부분적용 환경 안전. claim_diagnoses 는 이미 존재하므로 정의 변경 없음(SKIP).
-- 게이트: data-architect CONSULT(PHI/금융 + RLS) GO + supervisor DDL-diff 통과 후에만 _pg --apply.
-- 롤백: A_insurance_claims.scoped_rollback.sql  (★원본 .down.sql 사용 금지 — claim_diagnoses DROP 위험)
...
-- ★ prod 에 이미 존재(20260515000010) → CREATE IF NOT EXISTS 로 SKIP. 정의/데이터 무변경.
