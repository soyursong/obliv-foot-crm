...
BEGIN
  FOR a IN
    SELECT * FROM public.payments_archive
     WHERE ticket_id = 'T-20260715-foot-CLOSING-SINGLEPAY-F4716-CHARTMATCH-RECUR'
  LOOP
    -- 원본 payments 행 복원(전 컬럼 스냅샷). 이미 존재하면 skip(재실행 안전).
    IF NOT EXISTS (SELECT 1 FROM public.payments WHERE id = a.original_id) THEN
      INSERT INTO public.payments SELECT * FROM jsonb_populate_record(NULL::public.payments, a.snapshot);
    END IF;

    -- 재앵커된 package_payments(MOVE memo 마커) 제거
    v_pkg := (a.snapshot->>'customer_id')::uuid;  -- 참고용
    DELETE FROM public.package_payments
     WHERE customer_id = (a.snapshot->>'customer_id')::uuid
  ...
  LOOP
    ...
    -- (정확 pre-state = Part1 apply 후 S1 완납. archive.snapshot 시점 paid_amount 는 packages 별도 원복 불요 —
