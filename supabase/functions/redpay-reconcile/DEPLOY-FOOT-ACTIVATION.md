...
> ⚠ **dead-band 경고:** `REDPAY_BUSINESS_NO` 정본 = **`457-23-00938`** (2026-07-23 RedPay flip 후). 구 **`511-60-00988`** 는 07-23부터 RedPay 발송제외된 **dead band** — 재배포/재설정 시 절대 재사용 금지(구값 set 시 실 API 피드 0건 → raw 적재 무음 실패).
...
- business_no `457-23-00938` = **공유 merchant**(롱레 8 TID + 풋 13 TID 동거, 계약 §519). business_no 단독 스코프 시 롱레 거래가 풋 탭에 섞임. (07-23 RedPay flip 전 구 band = `511-60-00988` = 현재 dead, 발송제외.)
...
- **clinic 행 존재:** `SELECT id,business_no FROM clinics WHERE business_no='457-23-00938';` → 1행이어야 EF clinic_id 조회 성공(index.ts:297-305, 부재 시 "clinic_id 조회 실패" throw). **부재 시 배포 보류 → planner FOLLOWUP.** (prod clinics.business_no = 457 정본; 구 `511-60-00988` 로 조회 시 0행 → 오탐 배포보류.)
