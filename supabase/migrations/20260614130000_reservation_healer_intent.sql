...
-- reservations 에 '힐러 의도' 영속 컬럼(healer_intent) 신설.
--   · 기존 healer_flag(1회성 — 체크인 시 Dashboard HL-blink 후 소모)와 분리.
--   · healer_intent 는 캘린더 직접예약·체크인 이후에도 유지되는 분류 SSOT.
--   · '힐러 N건(HL N)' 칩 / resvKind 분류는 (healer_intent OR healer_flag) 기준.
-- ADDITIVE: 신규 컬럼 추가 + 기존값 보존 backfill. 파괴적 변경 없음.

ALTER TABLE public.reservations
  ADD COLUMN IF NOT EXISTS healer_intent boolean DEFAULT false;

COMMENT ON COLUMN public.reservations.healer_intent IS
...
UPDATE public.reservations
  SET healer_intent = true
  WHERE healer_flag = true
    AND healer_intent IS DISTINCT FROM true;
