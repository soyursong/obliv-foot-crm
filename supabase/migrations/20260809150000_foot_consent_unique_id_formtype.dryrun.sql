...
--   그 후 post-probe 로 prod 제약이 여전히 4값(비영속)임을 fresh 쿼리로 실측(INV-3).
...
--   C. 기존 행 중 신규 5값 CHECK 위반 0 (ADDITIVE 무회귀 — 기존 4값은 모두 부분집합).
...
BEGIN
  ...
  -- (TEST C) 기존 행 중 신규 5값 CHECK 위반 확인(ADDITIVE 이므로 0 이어야 함)
  SELECT count(*) INTO v_violators
    FROM public.consent_forms
   WHERE form_type NOT IN ('refund','non_covered','treatment','privacy','unique_id');
...
-- (TEST B) 위 예외로 DROP+ADD 롤백됨. 아래 fresh 쿼리로 제약 정의에 'unique_id' 부재 실측(러너 post-probe).
-- SELECT pg_get_constraintdef(con.oid)
--   FROM pg_constraint con JOIN pg_class rel ON rel.oid=con.conrelid
--   JOIN pg_namespace nsp ON nsp.oid=rel.relnamespace
--   WHERE nsp.nspname='public' AND rel.relname='consent_forms' AND con.contype='c'
--     AND pg_get_constraintdef(con.oid) ILIKE '%form_type%';   -- expect: 4값(unique_id 부재)
--
