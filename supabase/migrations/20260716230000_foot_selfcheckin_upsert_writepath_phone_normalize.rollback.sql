-- ROLLBACK: T-20260715-foot-UPSERT-RPC-NORMALIZE-BEFORE-WRITE
-- ════════════════════════════════════════════════════════════════════════════
-- 본 migration(20260716230000_..._normalize.sql) 직전 상태 복원 = 20260714120000
--   (_selfcheckin_upsert_masked_pii_reject_guard.sql) 의 3함수 정의 verbatim.
--   · base(fn_selfcheckin_upsert_customer): INSERT phone RAW(p_phone) + dedup raw-digit(v_digits) 복원.
--   · v2/v3: INSERT phone NULLIF(p_phone,'') RAW 복원.
--   마스킹-reject 가드(_fn_is_masked_pii)·본문 나머지는 불변(가드 migration 이 SSOT). helper 는 건드리지 않음.
-- 멱등: CREATE OR REPLACE 재실행 수렴. self_checkin_create 는 본 티켓 범위 외 → 롤백 미포함.
