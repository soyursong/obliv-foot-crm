-- T-20260512-foot-CONTRACT-ALIGN §B — ROLLBACK
-- staff.role / user_profiles.role 이전 제약으로 복원
-- normalize_phone() 함수 제거
-- admin_register_user RPC 이전 버전 복원

BEGIN;

-- B-1. staff.role CHECK 롤백 (5종으로 복원)
ALTER TABLE public.staff DROP CONSTRAINT IF EXISTS staff_role_check;
ALTER TABLE public.staff
  ADD CONSTRAINT staff_role_check
  CHECK (role IN ('director','consultant','coordinator','therapist','technician'));

-- B-2. user_profiles.role CHECK 롤백 (director 제거)
ALTER TABLE public.user_profiles DROP CONSTRAINT IF EXISTS user_profiles_role_check;
ALTER TABLE public.user_profiles
  ADD CONSTRAINT user_profiles_role_check
  CHECK (role IN ('admin','manager','consultant','coordinator','therapist','technician','tm','staff'));

-- B-3. admin_register_user RPC 롤백 (director 제거)
