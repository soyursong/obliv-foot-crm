...
--   무영속 검증 = plpgsql DO 블록 안에서 forward(publication ADD ×9 + REPLICA IDENTITY FULL ×9 +
--     DEFAULT ×2) 실행 후 in-txn 관측 → RAISE EXCEPTION 으로 전체 롤백(DO 블록 트랜잭션 abort) → 영속 0.
...
--   INV-3(post-probe): 롤백 후 prod 에 9개 M-gap 이 여전히 pub 비-멤버 + FULL-대상 9개가 여전히
--     REPLICA IDENTITY 'd'(default)임을 fresh 쿼리로 실측(러너가 수행).
--   실 러너/PASS 판정: db-gate/ (supervisor DB-GATE) — GO-token lane. 본 파일은 재현 참조.
--
-- 검증 시나리오:
--   A. 적용 중(in-txn): 9개 M-gap 이 supabase_realtime 멤버(pub count=9 신규).
--   B. 적용 중(in-txn): FULL 대상 9개(payments..pending_payment + check_ins + reservations)
--      relreplident='f'.
...
DECLARE
  ...
  full_tables text[] := ARRAY[
    'payments','package_payments','closing_manual_payments','duty_roster','clinic_doctors',
    'redpay_raw_transactions','pending_payment','check_ins','reservations'
  ];
  v_pub_added  int;
  v_full_cnt   int;
BEGIN
  ...
  RAISE EXCEPTION 'DRYRUN_OK pub_added=% full_cnt=% (expected 9 / 9) — 무영속 롤백', v_pub_added, v_full_cnt;
END $$;
