-- ============================================================
-- ROLLBACK — T-20260608-foot-TICKET-DEDUCT-SLOT-DATA (reconcile-R5)
-- 20260706120000 역전: 두 통계함수를 reconcile 직전 현행 prod prosrc 로 복원
--   (AC1 iv-exclude 및 AC3 contract_date=kst_date 당일전환 필터 제거).
-- 시그니처 불변(by_category 3컬럼 / therapist_summary 10컬럼) → CREATE OR REPLACE, 42P13 불가.
-- DB: rxlomoozakkjesdqjtvd (obliv-foot-crm)
-- ============================================================

BEGIN;

-- ── by_category : reconcile 직전 prod prosrc (iv 필터 없음) ──
...
-- ── therapist_summary : reconcile 직전 prod prosrc (contract_date 필터 없음) ──
