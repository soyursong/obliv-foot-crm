/**
 * T-20260725-foot-MEDIREC-ROOMCAPTURE-FIELD-CONFIRM — 백필 DRY-RUN (READ-ONLY)
 *
 * ⚠ SELECT/시뮬레이션만. write/UPDATE 0. status_transitions.room_id 미변경(non-persistence).
 *   본 마이그(20260730120000_..._backfill.sql)의 freeze 로직을 inline SELECT 로 재현 →
 *   백필이 무엇을 채울지 사전 증거화 + archive-first(freeze-set JSON 덤프).
 *
 * 실행: node supabase/migrations/20260730120000_foot_status_transitions_room_id_backfill.dryrun.mjs
 * 산출:
 *   ① freeze-set — 백필이 채울 (st_id → assigned_room) : 건수 + to_status별 분포
 ...
 *   + archive-first: freeze-set 을 evidence/.../backfill_freeze_archive.json 으로 저장(롤백 근거).
 */
...
const FREEZE_SQL = `
  SELECT st.id AS st_id, st.to_status, st.transitioned_at, m.assigned_room
  ...
  WHERE st.clinic_id='${CLINIC}'
    ...
    AND st.transitioned_at >= now() - interval '${DAYS} days'
...
// non-persistence probe: room_id 상태 서명(룸수반 전이 대상 범위).
const PROBE_SQL = `
...
console.log('════════════════════════════════════════════════════════════════');
console.log(' DRY-RUN: status_transitions.room_id 60일 백필 (check_in_room_logs 소급)');
console.log(' clinic:', CLINIC, '· window:', DAYS, '일');
console.log('════════════════════════════════════════════════════════════════');
...
// to_status 별 분포
const dist = {};
for (const r of freeze) dist[r.to_status] = (dist[r.to_status] || 0) + 1;
console.log('\n──── ① freeze-set — 백필이 채울 (st_id → assigned_room) ────');
console.log(`  대상 룸수반 NULL 전이 전체 : ${total.total_room_null}건`);
console.log(`  fill(매칭 room log 존재)   : ${freeze.length}건`);
...
for (const r of freeze.slice(0, 10)) console.log(`    - ${r.st_id.slice(0, 8)} | ${r.to_status} | room=${r.assigned_room} | ${r.transitioned_at}`);

// archive-first: freeze-set 을 evidence JSON 으로 덤프(롤백 근거 = 정확 id-set + 값).
mkdirSync(dirname(ARCHIVE), { recursive: true });
writeFileSync(ARCHIVE, JSON.stringify({
  ticket: 'T-20260725-foot-MEDIREC-ROOMCAPTURE-FIELD-CONFIRM',
  clinic: CLINIC, window_days: DAYS,
...
console.log(consistent && noWrite
  ? ' 결과: 백필 준비 OK — freeze 정합 + 무영속 + archive-first 확인 ✅'
