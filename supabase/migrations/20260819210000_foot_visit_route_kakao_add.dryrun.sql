-- DRY-RUN (No-Persistence): T-20260819-foot-INFLOW-KAKAO-INBOUND-ADD
...
--   · 사후 무영속(post-probe)은 runner 의 별 트랜잭션(독립 API 콜)에서 CHECK 정의에 '카톡' 부재 재확인.
BEGIN;

-- customers.visit_route CHECK 재생성 (카톡 ADD)
...
-- reservations.visit_route CHECK 재생성 (카톡 ADD)
...
-- assertion: '카톡' ADD + 기존값('공홈'/'네이버') 존치 (customers/reservations 양측)
DO $chk$
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'DRYRUN-FAIL: customers_visit_route_check 카톡 ADD/기존값 존치 검증 실패';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    ...
    RAISE EXCEPTION 'DRYRUN-FAIL: reservations_visit_route_check 카톡 ADD/기존값 존치 검증 실패';
  END IF;
  -- 기존행 위반 0 검사(순수 ADDITIVE → superset 이므로 위반 불가, 방어적 확인)
  IF EXISTS (
    SELECT 1 FROM public.customers
     WHERE visit_route IS NOT NULL
       AND visit_route NOT IN ('TM','워크인','인바운드','지인소개','네이버','인콜','공홈','카톡')
  ) THEN
