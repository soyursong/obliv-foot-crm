-- T-20260720-foot-SUPABASE-TOKEN-DISCLOSURE-REQUEST  (dev-foot / WARN-2 ball)
...
-- ════════════════════════════════════════════════════════════════════
-- Step 9: 롤 프로비저닝 + GRANT (★gate_c 집행 — soyursong/CEO-admin 전용·본 파일 미실행)
--   dev-foot는 provisioner 아님(§5). 아래는 propagation runbook 템플릿(주석). 값은 gate_c/legal 확정 후.
-- --------------------------------------------------------------------
--   -- (a) 전용 제한 롤 (LOGIN·NOSUPERUSER·NOCREATEDB·NOCREATEROLE). pooler 직결 전용.
--   -- CREATE ROLE partner_eunsang LOGIN PASSWORD '<gate_c에서 발급·본 파일에 평문금지>'
--   --   NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION
--   --   CONNECTION LIMIT 4 VALID UNTIL '<만료일>';
--   -- (b) base public 직접 grant = 0 (하드가드). 스키마 노출도 뷰만.
--   -- REVOKE ALL ON ALL TABLES  IN SCHEMA public FROM partner_eunsang;
--   -- REVOKE ALL ON SCHEMA public FROM partner_eunsang;
--   -- (c) partner_ro 스키마 + 7뷰만 SELECT 노출.
--   -- GRANT USAGE ON SCHEMA partner_ro TO partner_eunsang;
--   -- GRANT SELECT ON partner_ro.v_customers_masked, partner_ro.v_reservations_masked,
--   --   partner_ro.v_check_ins_masked, partner_ro.v_status_transitions,
--   --   partner_ro.v_payments_masked, partner_ro.v_service_charges_masked,
--   --   partner_ro.v_package_sessions, partner_ro.v_redpay_masked TO partner_eunsang;
--   -- (d) 헬퍼 함수 EXECUTE (뷰 owner-rights로 이미 실행되나 명시).
--   -- GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA partner_ro TO partner_eunsang;
--   -- 하드가드 CONFIRM: service_role 금지·SUPABASE_ACCESS_TOKEN 금지·RLS 무확장·전체키 dump 금지.
--   -- WARN-3 assert: 뷰 컬럼목록 == 분류표 PASS집합 정확일치 / base grant=0(42501) / 마스킹 실렌더.
-- ════════════════════════════════════════════════════════════════════
