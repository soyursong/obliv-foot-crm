-- ══════════════════════════════════════════════════════════════════
-- ROLLBACK — T-20260727-foot-REDPAY-WHITELIST-EXPAND-0725GAP (0725 GAP superseded-remap 역전)
-- ══════════════════════════════════════════════════════════════════
-- 역전 계약(데이터손실 0):
...
--   ⚠ 신 538xxx raw 가 배포 후 실적재된 경우, rollback 하면 그 3행은 뷰에서 다시 탈락(gap 재발) — 정상(역전 의미).
...
SET tid = m.old_tid,
    ...
    source = 'redpay_foot_terminal_registry.md §2 (0725 GAP rollback — 구 479xxx 복원)',
    updated_at = now()
FROM remap m
