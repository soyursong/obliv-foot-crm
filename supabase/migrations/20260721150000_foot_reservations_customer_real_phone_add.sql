...
--
-- ★ 비파괴 불변식: 함수 signature(18-arg)/반환(UUID)/never-downgrade 가드/취소 fast-path/lifecycle
--   가드#5/timeline(rmh) upsert/brief_note 배선 全 무변경 = 선행 권위 body(20260713150000)의 strict copy
--   이며 customer_real_phone INSERT+ON CONFLICT 절만 ADD. 인자수 무변경 → DROP 불요(CREATE OR REPLACE ACL 보존).
...
--   국한됨을 확인. DA CONSULT RESOLVED(신규컬럼 승인 = §S2.4 게이트 충족). 대표게이트 면제(autonomy §3.1).
...
--     선행 권위 body(20260713150000)의 strict copy + customer_real_phone persist 절만 ADD.
-- ────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  ELSE
    ...
    ON CONFLICT (clinic_id, phone) DO UPDATE SET
      -- ★ never-downgrade 가드 (T-20260713; DA-20260713 §L12/§L40): 기존 non-empty no-touch, 빈값만 채움.
  ...
  -- ────────────────────────────────────────────────────────────────
  -- ★ 6. 예약메모 timeline 가드 INSERT (SoT 재타겟 = reservation_memo_history). (본 블록 불변.)
  -- ────────────────────────────────────────────────────────────────
...
COMMENT ON FUNCTION public.upsert_reservation_from_source(
  ...
  '도파민/외부 → 풋CRM reservations 표준 멱등 upsert (18-arg superset + lifecycle 가드#5 + 예약메모 timeline + 간략메모 brief_note + customers.name never-downgrade + 동행연락처 customer_real_phone 표시전용 배선).
   ...
   (COMPANION-PHONE-EXPOSE ⊃ NAME-NEVER-DOWNGRADE ⊃ BRIEF-NOTE ⊃ TM-EDIT-CANCEL ⊃ MEMO-PUSH-DROP[timeline] ⊃ COMPANION + 가드#5)';
