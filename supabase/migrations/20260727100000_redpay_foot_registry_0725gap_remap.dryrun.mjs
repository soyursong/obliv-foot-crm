/**
 * T-20260727-foot-REDPAY-WHITELIST-EXPAND-0725GAP — 0725 GAP superseded-remap DRY-RUN (무영속)
 *
 ...
 *   ④ forecast (READ-ONLY): 신 2 TID(538xxx)가 tid-membership 에 편입되면 7/25 gap 3행 소급 표면화 예측.
 *
 * ⚠ 순수 data-lane UPDATE(신규 DDL 0). 영속 write 0.
 * 실행: node supabase/migrations/20260727100000_redpay_foot_registry_0725gap_remap.dryrun.mjs
...
const UP = join(here, '20260727100000_redpay_foot_registry_0725gap_remap.sql');
...
const MERCH = ['1777289003', '1777289008'];
const NEW_TIDS = ['1047538235', '1047538245'];
const OLD_TIDS = ['1047479477', '1047479482'];

console.log('════ 0725GAP superseded-remap DRY-RUN (무영속) ════\n');
...
// ── ④ forecast (READ-ONLY): 신 TID 편입 후 7/25 gap 소급 표면화 예측 ──
const fc = await q(`
  WITH tid_after AS (
...
console.log('   · gap_rows_now/gap_amt = 현재 미표면화 gap (기대 3 / 31,000)');
console.log('   · visible_after_remap = remap 후 tid-membership 하 표면화 예측 (기대 3 = 완전 수렴)');
const converge = Number(fc[0].gap_rows_now) === 3 && Number(fc[0].visible_after_remap) === 3;
console.log(`   ⇒ AC-3 수렴 예측 ${converge ? '✅ (3→3)' : '⚠ 재확인 필요'}\n`);
