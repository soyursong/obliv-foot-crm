-- ROLLBACK: T-20260611-foot-SELFCHECKIN-CONSENT-ADDR-NOTSAVED (merge-path 보강)
-- fn_selfcheckin_rrn_match 를 20260609230000 버전(privacy_consent/sms_opt_in 이관 이전)으로 복원.
-- ※ 동의 컬럼(privacy_consent_at/sms_opt_in_at)은 다른 마이그가 소유하므로 DROP 하지 않음.
--
-- 적용 방법 (supervisor 실행):
--   supabase db push --file supabase/migrations/20260611140000_selfcheckin_rrn_match_consent_sync.rollback.sql
