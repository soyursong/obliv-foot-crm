/**
 * T-20260806-foot-REDPAY-WHITELIST-EXPAND-0806GAP — 288007 신규 admission DRY-RUN (무영속)
 *
 * Migration Dry-Run No-Persistence Protocol 준수 + G3/G4/G7 게이트:
 *   ① pre-probe / archive-first (READ-ONLY): 288007 registry 부재(신규 INSERT 검증) + tid 538244 전역 부재 +
 *      현 foot(active) 카운트 baseline(25/25/40) + superseded 컬럼 실재.
 *   ② trial-apply: up.sql 전문을 DO $dryrun$ … RAISE EXCEPTION sentinel 로 실행 → SQL 무오류 + INSERT rows-affected=1(G3) + 무영속.
 ...
 *   ③ post-probe (READ-ONLY): 288007 여전히 registry 부재 (영속 0 확증).
 *   ④ forecast (READ-ONLY, ★AC-4): raw @tid=1047538244 external_status=Y 현 적재 = 0(신규 미등록 → poller drop).
 *      ⚠ 본 건은 raw 미적재 → registry INSERT 만으로 뷰 소급 불가. admission 후 poller daily_full 재폴링 필요.
 *      forecast 는 (a) 현 raw 적재 0 확인 + (b) registry INSERT 후 26/26/41 카운트 예측.
 *
 * ⚠ 순수 data-lane INSERT(신규 DDL 0). 영속 write 0.
 * 실행: node supabase/migrations/20260806090000_redpay_foot_registry_0806gap_admission.dryrun.mjs
...
const UP = join(here, '20260806090000_redpay_foot_registry_0806gap_admission.sql');
...
console.log('════ 0806GAP 288007 신규 admission DRY-RUN (무영속) ════\n');
...
console.log('   기대: has_superseded_col=1, merchant_present=0(신규), new_tid_present=0(순수 신규), foot_active_rows=25');
if (Number(pre[0].merchant_present) !== 0) throw new Error('PRE_FAIL — 288007 이미 존재. 신규 admission 아님(remap 재검토).');
...
if (Number(post[0].merchant_present) !== 0) throw new Error('무영속 검증 실패 — 영속 흔적 탐지(288007 잔존).');
console.log('   무영속 ✅ PASS');

// ── ④ forecast (READ-ONLY, AC-4): raw 적재 현황 + INSERT 후 카운트 예측 ──
const fc = await q(`
  SELECT
...
console.log('\n④ forecast(READ-ONLY, AC-4):', JSON.stringify(fc[0]));
console.log('   · raw_ingested_now = 현 raw 적재(★기대 0 — 신규 미등록 merchant → poller drop → 미적재)');
console.log('   · visible_now_in_view = 현 뷰 표면화 (기대 0)');
console.log('   · foot_active_now = 현 registry(기대 25 → INSERT 후 26)');
if (Number(fc[0].raw_ingested_now) === 0) {
  console.log('   ⚠ AC-4 확정: raw 미적재 → registry INSERT 만으로 뷰 소급 불가. admission 후 poller daily_full(8/03~8/06) 재폴링 REQUIRED (티켓 "backfill 불요 예상" 반증).');
} else {
  console.log(`   raw 이미 ${fc[0].raw_ingested_now}건 적재 → registry INSERT 후 뷰 즉시 소급 가능(재폴링 불요).`);
}
console.log('\n════ DRY-RUN 종료 (영속 0) ════');
