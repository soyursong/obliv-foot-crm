...
-- ══════════════════════════════════════════════════════════════════
-- 신규 편입 1 merchant(1777288002, domain='foot') DELETE → VG1 baseline(27-set) 복원. 데이터손실 0(seed 데이터만 제거).
--   ⚠ 롤백 후 폴러 DEFAULT(288002 parity)·env(REDPAY_*_WHITELIST)도 되돌려야 소비처 정합.
--   ★AC-5 REVISED: seed+daily_full 재폴링(8/06~8/09) 後라면 redpay_raw_transactions 에 ₩260,000/3txn 이 적재돼 있음.
--     소비뷰는 registry active 서브쿼리 파생 → registry(288002) DELETE 시 뷰에서 자동 미표면화(뷰 3→0). raw 원장 행
--     자체는 잔존(비파괴)해도 무해. 원장 raw 물리 삭제는 본 롤백 scope 아님(필요 시 Data-Correction Backfill SOP per-row).
