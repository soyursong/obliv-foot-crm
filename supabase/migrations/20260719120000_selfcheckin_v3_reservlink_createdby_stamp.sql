-- T-20260716-foot-SELFCHECKIN-RPC-CREATEDBY-CANON — REOPENED FIX (ADDITIVE, INSERT-only created_by 스탬프)
-- ════════════════════════════════════════════════════════════════════════════
-- 근인(DIAG 실측, prod introspection 2026-07-19 READ-ONLY):
--   셀프체크인 new-visit 실호출 경로 = fn_selfcheckin_upsert_customer_resolve_v3
--     (foot-checkin 키오스크 SelfCheckIn.tsx L1748, T-20260628 ANON-KIOSK-CUTOVER).
--   prod v3 의 customers INSERT 에 created_by 스탬프 부재(NO ✗) → 스탬프 0건 landing.
--   지문 (a): 실호출이 스탬프 없는 함수(v3)를 탐. CANON 이 stamp 를 base(v1)+v2 에만 OOB 적용,
--             v3(=live path) 를 누락. 지문 (c): base/v2 stamp 는 git 미반영 OOB divergence
--             (introspection 함수 실재 ≠ 스탬프 실동작). 지문 (b)(INSERT후 UPDATE 덮음) = 반증
--             (v3/rl UPDATE SET 에 created_by 무존재 → 덮어쓰기 아님).
--   prod 실저장: customers 최근30일 total=450, created_by NOT NULL=0, ='self_checkin'=0.
--
-- 수정(2 landing INSERT 경로에 INSERT-only 스탬프, prod pg_get_functiondef verbatim 기반 = canon-forward):
--   1) fn_selfcheckin_upsert_customer_resolve_v3 — 'created' 분기 INSERT 에 created_by='self_checkin'.
--   2) self_checkin_with_reservation_link — genuine-new(match_count=0) INSERT 에 created_by='self_checkin'
--      (v3 resolveErr → customerId=null 폴백 시 발화하는 2차 landing 벡터 차단).
--   ★ UPDATE(linked/기존고객) 경로 created_by 무변경 — new-write-only(旣 NULL 덮어쓰기 금지 계약 준수).
--   ★ 스키마/컬럼/enum/시그니처/GRANT 무변경. customers.created_by 컬럼 旣존재. = ADDITIVE.
--
-- 게이트: read-only DIAG=착수게이트 불요. ADDITIVE 함수 DDL → supervisor DDL-diff + MIG-GATE
--   (부모 Q5 DA-cleared 계승, 신규 데이터정책 자문 불요). 롤백 = 스탬프-前 prod 정의 복원(.rollback.sql).
--   Phase2 백필(RRN-ENC-DESK-CONSENT-GATE 결정 B) freeze 전제 충족 = 본 fix 배포 후 신규 self_checkin 스탬프 실동작.
-- author: dev-foot / 2026-07-19 · ticket: T-20260716-foot-SELFCHECKIN-RPC-CREATEDBY-CANON
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  INSERT INTO customers(
    ...
    created_by  -- T-20260716-foot-SELFCHECKIN-RPC-CREATEDBY-CANON: self_checkin INSERT-only 스탬프(new-write-only)
  ) VALUES (
    ...
    'self_checkin'  -- T-20260716-foot-SELFCHECKIN-RPC-CREATEDBY-CANON: 셀프체크인 실호출 경로(ANON-KIOSK-CUTOVER 06-28) landing 스탬프
  )
