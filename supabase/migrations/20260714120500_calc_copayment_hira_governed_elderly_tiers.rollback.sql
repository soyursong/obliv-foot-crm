-- ROLLBACK — T-20260713-foot-HIRA-UNIT-VALUE-2026-UPDATE calc_copayment v1.3
-- → NULLFIX v1.2 복원 (default-deny + data_incomplete, 89.4 COALESCE fallback 有, 노인 단일정액).
-- ⚠ v1.1(phantom-covered 버그) 로는 원복하지 않음 — last-known-good = NULLFIX v1.2.
--   v1.3 이 subsume 한 NULLFIX 로직만 남기고 이슈1/이슈2 델타만 되돌림.
-- ⚠ 이 rollback 은 seed rollback(20260714110000 rollback, 89.4 복귀)과 세트로만 의미 있음.
...
COMMENT ON FUNCTION calc_copayment(UUID, UUID, UUID, DATE) IS
  '건보 본인부담 산출 v1.2 (rollback of v1.3) — 급여+hira_score NULL default-deny allowlist. (NULLFIX)';
