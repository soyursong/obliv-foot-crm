-- ROLLBACK of 20260725200000_foot_stats_conversion_exclude_onetime.sql
--   → LIVE(20260725190000_foot_stats_experience_include_pkg_trial) 의 exp_conv(전환 분자) 정의를 그대로 복원.
--   (exp_conv = check_in 자기 package_id 에 package_payments(payment) 존재 여부. experience_total/기타 지표 동일.)
--   ⚠ 복원 후 전환 분자는 다시 구 정의로 돌아감(prod delta=0이라 수치 무변동).
--
-- (아래는 복원 대상 원본 = T-20260725-foot-PKG-TRIAL-VISITTYPE-EXPERIENCE-FIX. 패키지 체험권 차감 내원을
