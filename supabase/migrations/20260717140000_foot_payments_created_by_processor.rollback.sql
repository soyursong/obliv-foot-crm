-- ROLLBACK: T-20260717-foot-SALESPATIENT-REFUND-PROCESSOR-COLUMN
-- 20260717140000_foot_payments_created_by_processor.sql 역연산.
--   · refund_single_payment RPC 를 직전 버전(20260525050000_refund_perm_expand, created_by 미포함)으로 복원.
--   · idx_payments_created_by 인덱스 DROP.
--   · payments.created_by 컬럼 DROP (신규 nullable 컬럼 → 데이터 유실 없음, PaymentDialog INSERT 는
--     created_by 미존재 시 무시되지 않으므로 FE 롤백과 함께여야 함 — 배포순서 주의).
-- ⚠ 주의: 이 컬럼을 제거하면 PaymentDialog/RPC 의 created_by INSERT 가 오류가 되므로
--         FE(SalesPatientTab/PaymentDialog) 롤백 배포와 반드시 함께 실행.

BEGIN;

-- 1. RPC 를 created_by 미포함 버전으로 복원 (20260525050000 기준)
...
BEGIN
  ...
  VALUES (
    p_clinic_id, v_original.check_in_id, v_original.customer_id, p_amount, p_method,
    'refund', 0, p_memo, p_payment_id, 'active'
  )
