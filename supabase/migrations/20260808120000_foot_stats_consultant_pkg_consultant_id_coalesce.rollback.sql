-- ============================================================
-- ROLLBACK: T-20260718-foot-PKG-CONSULTANT-ID-RPC-CUTOVER (Phase 2)
-- foot_stats_consultant 를 20260724130000_foot_stats_consultant_singlepay_customer_attr
--   본문(pkg_attr = heuristic-only, INNER JOIN)으로 복원. COALESCE(fact) 결합 취소.
-- 반환형 불변(7-col) → CREATE OR REPLACE(DROP 불요). 데이터/스키마 write 0.
-- ⚠ 권한 posture: admin-gate(20260727120000) authenticated 회수 유지(0724 template 의
--    `GRANT ... TO authenticated` 재도입 금지 — no-read-up 회귀 방지). 복원 대상 = 본문뿐.
...
-- 권한 posture 보존(admin-gate authenticated 회수 유지 — GRANT 재도입 금지).
