-- ROLLBACK: T-20260708-dopamine-FOOTRESV-NAILPROB-SUBFILTER-PUSH brief_note 배선 되돌리기.
-- 효과:
--   1) 18-arg upsert_reservation_from_source(p_brief_note 포함) 명시 DROP.
--   2) upsert_reservation_from_source 를 20260701020000(17-arg, timeline 재타겟 + 가드#5) body 로 복원
--      → brief_note INSERT/ON CONFLICT 배선 제거.
-- 주의: reservations.brief_note 컬럼은 본 migration 이 생성하지 않았으므로(20260624100000 旣존) DROP 하지 않는다.
--   본 rollback 적용 시 도파민 push 간략메모가 다시 풋 예약상세에 미표시(4번 회귀). 운영 unwind 필요시만.

BEGIN;

-- ── 18-arg signature 명시 DROP (오버로드 잔존 방지) ─────────────────────────────
...
-- ── RPC body 를 20260701020000(17-arg, brief_note 배선 이전) 으로 복원 ──────────
