-- ROLLBACK: T-20260819-foot-INFLOW-KAKAO-INBOUND-ADD
-- visit_route CHECK 제약을 직전 7값('TM','워크인','인바운드','지인소개','네이버','인콜','공홈')으로 복원 = '카톡' 제거.
--   ⚠ 롤백 전 '카톡' 값을 가진 행이 있으면 CHECK 재생성이 실패한다.
--      → 먼저 해당 행을 NULL 또는 기존 값으로 정리 후 롤백할 것(데이터 보존 판단은 운영).
--
-- 적용: supabase db push --file supabase/migrations/20260819210000_foot_visit_route_kakao_add.rollback.sql
