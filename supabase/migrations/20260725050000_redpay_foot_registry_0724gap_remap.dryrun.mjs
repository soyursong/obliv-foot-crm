/**
 * T-20260725-foot-REDPAY-WHITELIST-EXPAND-0724GAP — 0724 GAP superseded-remap DRY-RUN (무영속)
 *
 * Migration Dry-Run No-Persistence Protocol 준수:
 *   ① pre-probe (READ-ONLY): superseded_tids 컬럼 실재 + 4 merchant 현재 tid=479xxx / superseded=NULL.
 *   ② trial-apply: up.sql 전문을 BEGIN … ROLLBACK 로 실행 → SQL 무오류 검증 + 무영속.
 *   ③ post-probe (READ-ONLY): 4 merchant tid 여전히 479xxx (영속 0 확증).
 *   ④ forecast (READ-ONLY): 신 4 TID(538xxx)가 tid-membership 에 편입되면 7/24 gap 9행 소급 표면화 예측.
 *
 * ⚠ 순수 data-lane UPDATE(신규 DDL 0). 영속 write 0.
 * 실행: node supabase/migrations/20260725050000_redpay_foot_registry_0724gap_remap.dryrun.mjs
...
const UP = join(here, '20260725050000_redpay_foot_registry_0724gap_remap.sql');
...
const MERCH = ['1777288003', '1777288004', '1777288006', '1777289004'];
const NEW_TIDS = ['1047538236', '1047538231', '1047538241', '1047538237'];
const OLD_TIDS = ['1047479471', '1047479472', '1047479474', '1047479478'];

console.log('════ 0724GAP superseded-remap DRY-RUN (무영속) ════\n');
  ...
  SELECT
    ...
    (SELECT count(*) FROM redpay_terminal_registry
       WHERE domain='foot' AND merchant_id = ANY(ARRAY[${MERCH.map((m) => `'${m}'`).join(',')}])
         AND tid = ANY(ARRAY[${OLD_TIDS.map((t) => `'${t}'`).join(',')}]) AND active) AS four_at_old_tid,
    (SELECT count(*) FROM redpay_terminal_registry
...
console.log('   기대: has_superseded_col=1, four_at_old_tid=4, new_tids_present=0\n');
...
console.log('   4 merchant 현 상태:', JSON.stringify(preRows));

// ── ② trial-apply (BEGIN … ROLLBACK, 무영속) ──
const upBody = readFileSync(UP, 'utf8');
console.log('\n② trial-apply: up.sql 전문 BEGIN…ROLLBACK 실행(무오류 검증 + rows-affected)...');
// txn 내 상태 관측 후 강제 롤백(무영속). RAISE 로 unwind → 영속 0.
const trial = await q(`
DO $dryrun$
DECLARE
  v_affected int;
...
const clean = Number(post[0].still_old_tid) === 4 && Number(post[0].new_tid_persisted) === 0;
console.log(`   무영속 ${clean ? '✅ PASS' : '❌ FAIL — 영속 흔적!'} (still_old_tid=4, new_tid_persisted=0 기대)\n`);
if (!clean) throw new Error('무영속 검증 실패 — 영속 흔적 탐지');

// ── ④ forecast (READ-ONLY): 신 TID 편입 후 7/24 gap 소급 표면화 예측 ──
const fc = await q(`
  WITH tid_after AS (
  ...
  SELECT
    (SELECT count(*) FROM redpay_raw_transactions r
       WHERE (r.approved_at AT TIME ZONE 'Asia/Seoul')::date = DATE '2026-07-24'
         AND r.external_status='Y'
         AND (r.raw_payload->'data'->>'tid') = ANY(ARRAY[${NEW_TIDS.map((t) => `'${t}'`).join(',')}])) AS gap_rows_now,
    (SELECT COALESCE(sum((r.amount)::numeric),0) FROM redpay_raw_transactions r
       WHERE (r.approved_at AT TIME ZONE 'Asia/Seoul')::date = DATE '2026-07-24'
         AND r.external_status='Y'
         AND (r.raw_payload->'data'->>'tid') = ANY(ARRAY[${NEW_TIDS.map((t) => `'${t}'`).join(',')}])) AS gap_amt,
    (SELECT count(*) FROM redpay_raw_transactions r
       WHERE (r.approved_at AT TIME ZONE 'Asia/Seoul')::date = DATE '2026-07-24'
         AND r.external_status='Y'
         AND COALESCE(r.tid, r.raw_payload->'data'->>'tid') IN (SELECT tid FROM tid_after)
         AND (r.raw_payload->'data'->>'tid') = ANY(ARRAY[${NEW_TIDS.map((t) => `'${t}'`).join(',')}])) AS visible_after_remap`);
console.log('④ forecast(READ-ONLY, remap 후 예측):', JSON.stringify(fc[0]));
console.log('   · gap_rows_now/gap_amt = 현재 미표면화 gap (기대 9 / 4,750,000)');
console.log('   · visible_after_remap = remap 후 tid-membership 하 표면화 예측 (기대 9 = 완전 수렴)');
const converge = Number(fc[0].gap_rows_now) === 9 && Number(fc[0].visible_after_remap) === 9;
console.log(`   ⇒ AC-3 수렴 예측 ${converge ? '✅ (9→9)' : '⚠ 재확인 필요'}\n`);
