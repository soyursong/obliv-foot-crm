...
1. **처방 셀 — 귀가·미처방 '-'**(L1475~): 정적 데드텍스트 → **명시 '차트 열기' 버튼**(FileText 아이콘 + sky chip) `onOpenChart('full')`.
   QuickRxBar(인플레이스 처방 입력)는 이 진입점에서 미렌더 — 처방 수정은 차트 안에서만.
2. **임상경과 셀 — 귀가 빈값 '—'**(L1498~): readonly span(TOGGLE-READONLY S1) → **명시 '차트 열기' 버튼**(FileText + sky chip) `onOpenChart('full')`.
...
4. **확정 처방(RxConfirmedSummary)**: 귀가(blockedByGate)는 기존대로 '차트에서 수정'(onOpenChart) 진입 — 이미 일관(무변경, 이미 명시 버튼).

### ★ affordance 보강 (planner CLARIFY MSG-20260620-024423-fzjd)
reporter(문지은 대표원장)가 동일 thread 재확인에서 "데드 '-'/'—' invisible click 만으론 부족 — **눈에 보이는 '차트 열기' 버튼/링크**를 기대" 명시.
→ 1·2 진입점을 subtle 대시(hover underline)에서 **명시 라벨 '차트 열기' + FileText 아이콘 + sky 테두리 chip** 으로 가시화.
스펙·게이트는 기존 A안 그대로(인플레이스 mutate 잠금 rxMutationGuard fail-closed 무접촉). E2E S3(3건) 추가 — 귀가 행 명시 '차트 열기' 버튼 렌더 검증.
...
- E2E `tests/e2e/T-20260616-foot-DISCHARGED-DASH-RX-CHART-ACCESS.spec.ts` (S1 4건 + S2 5건 + **S3 3건 명시버튼 가시화**) +
  supersede 갱신한 `T-20260616-foot-DISCHARGED-DASH-RX-TOGGLE-READONLY.spec.ts` (12건) — **합산 24 PASS**.
