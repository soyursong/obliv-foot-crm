...
status: deployed
qa_result: pass
deploy_commit: 0893fb080e13
deployed_at: 2026-07-18T04:12:00+09:00
bundle_hash: n/a (db+ef only, no FE bundle — infra deploy)
...
mig_rollback: [supabase/migrations/20260618200000_staff_attendance_ssot.rollback.sql, supabase/migrations/20260618201000_attendance_sync_cron.rollback.sql]
mig_applied_prod: "2026-07-18 04:08 KST — 20260618200000 → 20260618201000 순서 apply, schema_migrations 원장 기록"
...
## 11. PROD 배포 완료 (dev-foot, 2026-07-18 04:12 KST) — DEPLOY-GO MSG-20260718-012818-3rbk

supervisor DDL-diff 5-check GO(Green/ADDITIVE, eb59fe60) 집행. **AC-1(테이블)+AC-3(cron+EF) live 확인**. evidence: `evidence/T-20260618-foot-STAFF-ATTENDANCE-SSOT-CRM_postdeploy.md`.

- **apply 순서 준수**: 20260618200000(테이블) 先 → 20260618201000(cron) 後 (`scripts/..._apply.mjs --apply`, applyMigration 단일경로 → schema_migrations 원장 기록).
- **EF**: `ef_deploy.sh attendance-sync --no-verify-jwt` + secrets 4종 주입.
- **⚠ verify_jwt 갭 발견·수정**: attendance-sync config 블록 부재 → gateway 기본 verify_jwt=true → cron worker(X-Internal-Cron header-only) 호출 EF 도달 전 401 실측(net._http_response 114269). `[functions.attendance-sync] verify_jwt=false` 추가+재배포로 해소(인증=EF 내부 X-Internal-Cron 단일관문, redpay-reconcile 동일).
- **수동 1틱(POST-DEPLOY #5)**: EF 직접호출 HTTP200 `inserted=170/unmatched=0/errors=0`; pg worker `trigger_attendance_sync()` → net 200(id 114271).
- **정합(AC-5, #6)**: staff_attendance 170행(전량 google_sheet), 오늘(07-18 KST) present=9, unmatched=0 → 시트↔DB 정합.
- **회귀금지(AC-4)**: 배정화면 '출근 N명'·자동배정 후보풀 = 여전히 시트 직접 read → 런타임 동작 변경 0.

### 잔여 — AC-2 배정화면 read-swap (HELD, planner FOLLOWUP)
시트 직접 read → staff_attendance read 전환 미착수. 근거: (1) 티켓 §10.4 "freshness 안정 확인 후 AC-2" — sync 방금 live(1틱), 다틱 안정성 관측 전. (2) 배정화면(자동배정 후보풀=회귀 핵심)의 런타임 변경 → DDL-diff 5-check 범위 밖 + db-only E2E 면제 무효화(E2E spec 신규 필요) → supervisor QA 별 게이트. → planner 시퀀싱 FOLLOWUP.
