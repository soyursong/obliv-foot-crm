...
- **AC-3a (MSG-20260607-210836-r0ww 흡수, commit 43f8a10)**: 위 게이트는 정상인데 현장 미동작 — 임상경과
  `<Textarea>`(data-testid=medical-chart-clinical)에 `autoComplete` 미지정 → 브라우저 네이티브 입력이력(과거
  입력 약이름 등) 자동완성 드롭다운이 `//` 키스트로크/방향키를 가로채 상용구 단축어 팝오버(PHRASE-SLASH)를 무력화.
  수정: 임상경과 textarea 한정 `autoComplete/autoCorrect/autoCapitalize off + spellCheck=false + PW매니저 ignore`.
  처방/약품 입력란(별 컴포넌트의 의도된 datalist)은 보존.
