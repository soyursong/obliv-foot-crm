...
## phase2 실렌더 재검증 (supervisor FIX-REQUEST MSG-20260615-194606-y4ed 대응)
qa_fail_reason=insufficient_verification 해소. **PROD(obliv-foot-crm.vercel.app) + 갤탭 뷰포트(1280×800, ko-KR/Asia/Seoul)** 실 브라우저 렌더로 시나리오0 직접 실행.
- 스크립트: `scripts/T-20260615-foot-CHART2-DWELLSWAP-phase2-render-verify.mjs` (SDK 로그인→세션 주입→/chart/:id→[체류시간] 탭 클릭→스크롤·캡처).
- **(A) 데이터 有** (장예지 `ac1b22a6…`, RPC 109행): 방문건별 슬롯 머문시간 표 정상 렌더(총 원내 체류·슬롯별 체류시간·시간순 동선). "조회 실패" 미발생, 무한로딩 없음.
  → `_handoff/qa_screenshots/DWELLSWAP_phase2_A_withdata.png`
- **(B) 데이터 無** (`30962e3e…`, 체크인 0건): "슬롯 체류시간 기록 없음" 빈상태 렌더. "조회 실패" 미발생, 무한로딩 없음.
  → `_handoff/qa_screenshots/DWELLSWAP_phase2_B_nodata.png`
- AC-6(누락 마이그 RC) 효과 확정: c6fed76(`fn_check_in_slot_dwell` prod 적용) 후 PROD에서 PGRST202/"조회 실패" 완전 소거.
- 결론: 양 경로 PASS → deploy-ready 재확정.
