...
//     - 진료 환자 목록 (DoctorPatientList) — 원장 진료 완료 고객의 처방 현황
//       (T-20260615-foot-RXLIST-RENAME-DOCFILTER: 진료완료 고객만 표시 행필터 유지.
//        T-20260615-foot-DOCDASH-RX-DISPLAY-REVAMP item7(대표원장 명시 결정): 라벨만 '처방 환자 목록'→'진료 환자 목록' 역전, 행필터 불변.)
    ...
    <div className="h-full overflow-auto p-2 md:p-3 space-y-4 w-full">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="mb-4 flex-wrap h-auto gap-1">
          ...
          {/* 진료 환자 목록 — 전체 공개. T-20260615-foot-DOCDASH-RX-DISPLAY-REVAMP item7(대표원장 U0ALGAAAJAV 명시 결정):
              라벨 '처방 환자 목록'→'진료 환자 목록' 역전(RXLIST-RENAME-DOCFILTER 4a1af35 리네임 역전, 텍스트만).
              value="patient_list"/data-testid="tab-patient-list" 절대 불변(E2E·탭 상태키 보존). RXLIST 행필터(원장 진료완료만)도 불변. */}
