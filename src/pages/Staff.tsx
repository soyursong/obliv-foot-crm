...
function StaffTab({ clinic }: { clinic: Clinic }) {
  ...
  //   ⚠ 변수명 actorRole — 아래 ROLE_ORDER.map((role)=>...) 의 카드 그룹 role 과 셰도잉 충돌 회피(actor≠카드역할).
  ...
  const assignableRoles = assignableStaffRolesFor(actorRole, ROLE_ORDER);
            ...
            <CardContent className="space-y-2">
              ...
              {grouped[role].map((s) => (
                ...
                >
                  ...
                  {/* T-20260630-foot-STAFFCRUD-CODY-PERM: 행 단위 관리 게이트. coordinator 는 원장(director) 행 미노출(RLS USING role<>director 미러). actorRole=로그인 역할(카드 그룹 role 셰도잉 주의). */}
                  {canManageStaffRow(actorRole, s.role) ? (
