/**
 * 급여(보험) 방문 진료기록 급여판정 유틸 — ★ 완료/수납 하드차단 해제됨(soft 리마인더 전용).
 *
 * T-20260612-foot-MEDLAW22-B-GATE (원안) → T-20260728-foot-INSUR-POPUP-REMOVE (하드차단 해제)
 *
 * 【현행 결정 — 문지은 대표원장 "B안" 직접 컨펌, 2026-07-28 (슬랙 스레드 본인 결정)】
 *  - 하드차단 해제: 급여 진료 + 진료기록(서명) 미작성 상태여도 [수납대기]→[완료] 전환을 막지
 *    않는다(완료 차단 팝업 미노출). 대표원장 본인 직접 결정 = foot §11 의료 sign-off 요건 충족.
 *  - 하드차단 호출부 3지점 제거: Dashboard 드래그 완료 / Dashboard 우클릭 완료 / PaymentDialog 수납 완료.
 *  - 급여 청구(EDI)·의료법상 진료기록 후속 작성 안내는 결제 미니창 inline ℹ️ soft 리마인더
 *    (PaymentMiniWindow, isCovered 판정 재사용)로만 존치 — 차단 아님.
 *  - 본 함수(evaluateMedicalRecordGate)는 존치: PaymentMiniWindow 가 isCovered(급여 여부)만 사용.
 *    blocked/reason 반환 필드는 하위호환 유지용(현재 이를 하드차단으로 소비하는 곳 없음).
 *
 * 【원안 이력 — 문지은 대표원장 2026-06-13, MSG-20260613-171308-kdvj (2026-07-28 해제됨)】
 *  - (구) 강도 = 하드차단(완전차단): 진료기록(+서명) 없으면 수납/완료 진행 불가. 사유 입력 우회 없음.
 *  - (구) 범위 = 급여(보험) 방문 한정:
