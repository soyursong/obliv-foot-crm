 ...
 *   **이 PC에 연결된 카드 단말기(프린터)로 재출력(reprint)** 하는 전문 조립·오케스트레이션.
 *
 ...
 *
 * ── ★ 라우팅: 이 PC에 연결된 로컬 단말(프린터) — terminal-agnostic ─────────────────
 ...
 *     (물리 시리얼 = 이 PC 로컬 단말 1대).
 *   ※ "단말기 번호"(1번 등) 선택/라우팅 개념 없음 — 벤더 XP 스펙 body 필드에 TID 부재
 *     (DEVICE_TYPE/CAT_PORT/CAT_BAUDRATE/P_ECSPOS_CMD1). 결제 단말기와 무관.
 *
