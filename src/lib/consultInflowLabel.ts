 ...
 *     · TM/인바운드/워크인 은 원문 그대로 = 무회귀. 빈값(라벨 부재)만 기존 기본값('워크인') 보존.
 *
 * ── ★ RED LINE (AC-fix4) ──
...
/** 유입경로 원본이 빈값(라벨 부재)일 때의 기본 라벨 — 기존 동작 보존(무회귀). */
export const INFLOW_DEFAULT_LABEL = '워크인';
...
export function consultInflowLabel(
  ...
  // 실제 유입경로 원문 노출(네이버·지인소개·공홈·TM·인바운드·워크인 …) / 빈값만 기존 기본값 보존.
  return raw || INFLOW_DEFAULT_LABEL;
}
