 ...
 *
 * ── T-20260817-foot-HOLIDAY-CHUSEOK-2026-WRONGYEAR-DATES (2026-08-17, census 부수발견 정정) ──
 *   추석 항목이 2025 추석 연도(2026-09-30~10-02)로 오복사되어 있던 것을 2026 관보 확정치로 정정.
 *   2026 추석 = 9/25(당일·금), 연휴 9/24(전날·목)~9/26(다음날·토) — 대체공휴일 없음(연휴 안에 일요일
 *   미포함, 9/26 토요일은 대체 비적용). 구값(9/30~10/2)은 2026 평일 → (a)실 추석 연휴 9/24~26에
 *   공휴일 가산 자동선택 누락(과소청구) + (b)무관한 평일 9/30~10/2 오적용(과다징수) 이중 오류.
 *   9/26(토)은 토요일 가산(dow===6 && hour>=9) 경로로도 커버되나 공휴일 종일(0~9시 포함) 정합 위해
 *   법정 목록에도 명시. body 자매 CHUSEOK 티켓과 값 동기화(cross-repo co-change, 동시 배포).
 *   출처 = 정부 공공 국가공휴일 목록(2026 관보 재대조, 추석 연휴 일요일 미포함→대체 없음 확인).
 *
 * ── AC-5 (2026-08-17, dev-body sweep fold-in, MSG-20260817-124300 / money-neutral) ──
 *   부처님오신날 본일(2026-05-24, 음력 4/8, 일요일) Set 추가. body 자매 Set 과 값 파리티(5/24·5/25 둘 다 present).
 *   2026 부처님오신날 = 5/24(일) → 일요일 겹침 → 대체공휴일 = 5/25(월). 대체일 5/25 는 census 에서 이미
 *   present(가산 정상), 본일 5/24 는 일요일이라 dow===0 종일 분기가 이미 가산 커버 → 청구 금액 변동 0(money-neutral).
 *   목적 = ①법정 목록 정확성(본일+대체일 둘 다 명시) ②body 자매 Set 과 값 동기화(supervisor co-deploy value-sync).
 *   ★foot KOREAN_HOLIDAYS_2026 은 순수 date Set(라벨 필드 없음) → '라벨 정정(석가탄신일→대체공휴일)'은 N/A
 *     (5/25 주석은 이미 '대체공휴일'로 정확). Set 값 추가(5/24)만 반영. 출처 = 2026 관보 재대조.
 */
export const KOREAN_HOLIDAYS_2026 = new Set<string>([
  ...
  '2026-05-05', // 어린이날
  '2026-05-24', // 부처님오신날 본일(음력 4/8, 일요일) — AC-5 추가: 일요일 dow===0 종일 분기로 이미 가산 커버(money-neutral), body 자매 Set 값 파리티
  '2026-05-25', // 부처님오신날 대체공휴일(5/24 일요일 대체·월) — AC-5: 본일 5/24 와 짝(둘 다 present), 순수 date Set 이라 라벨필드 N/A
  '2026-06-06', // 현충일 (토요일 — 대체공휴일 비대상, 6/8 추가 금지)
  '2026-08-15', // 광복절
  '2026-08-17', // 광복절 대체공휴일(8/15 토요일) — T-20260817 census 추가
  '2026-09-24', // 추석 전날(목) — T-20260817 CHUSEOK 정정: 구 2026-09-30(2025 추석) 오복사 제거
  '2026-09-25', // 추석(금) — 2026 관보 확정, 구 2026-10-01 대체
  '2026-09-26', // 추석 다음날(토) — 구 2026-10-02 대체 (9/24~26, 대체공휴일 없음: 연휴 내 일요일 미포함)
  '2026-10-03', // 개천절
