 ...
 *
 * 느슨 결합: customerId/stem 만 props 로 받는다(UI 비의존). 소비처는 저장된 차트 상세 뷰(list 모드 selectedChart).
 */
...
export function PenChartAttachPanel({
  ...
  return (
    <div className="mt-3 border-t pt-3" data-testid="penchart-attach-panel">
      ...
      {/* AC1: 업로드 영역 — 파일 선택 + 드래그&드롭 (foot 태블릿 '큰 버튼' 원칙) */}
      <div
      ...
      >
        ...
        <span className="text-[10px] text-neutral-400">이미지 파일 · 개당 최대 20MB</span>
        <input
          ref={fileInputRef}
          type="file"
          accept="image/*"
