        ...
        <DialogContent className="sm:max-w-md" data-testid="cband-pay-dialog">
          ...
          {ui === 'attention' && result && (
            <div className="space-y-3 rounded-lg border-2 border-amber-300 bg-amber-50 p-4" data-testid="cband-attention">
              ...
              {/* ★T-20260804-foot-CBAND-BLOCKED-SEND-PHANTOM-MSGTRACE-SUPPRESS (P0): 차단 경로(잠금으로 미송신,
                  precheck degrade-open 후 insert-first L2 백스톱 발화 포함)는 msgTrace='' → 팬텀 추적번호 억제.
                  실제 전송된 건(진짜 추적번호 존재)만 번호 노출(회귀0·AC-4). */}
              {result.msgTrace && (
