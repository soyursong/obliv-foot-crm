...
//   · 치료유형 축(내성 podologue / 각질 ribbon) + 무좀(athlete_foot_pc_nl, exam) → chart_treatment_requests 저장.
//   · 피검사(blood_test) / KOH(koh_fungal_test) → 既존 리스트업 엔티티 위임(request_*_for_customer RPC).
//     본 박스에서 체크 = 既존 토글과 동일 상태 공유(같은 플래그·같은 query key invalidate). 중복 저장소 없음(DA AC-4).
//
...
//   (blood_test_requested 폴백 패턴 동형). 배정·리스트업 동선 막지 않음.
...
interface Props {
  ...
  /**
   * 재진 자동 파생 코드(부모가 active 패키지 시술유형에서 산출).
   * 예: 패키지에 포돌로게 잔여 → ['podologue']. AC-3 스냅샷 소스.
   */
...
function useTreatmentRequests(checkInId: string | null | undefined) {
  return useQuery<CtrRow[]>({
    ...
    queryFn: async () => {
      ...
      if (error) {
        // 42P01(테이블 미존재) 등 마이그 전 → 빈배열 graceful
...
//   既존 토글(BloodTestRequestToggle/KohRequestToggle)과 동일 소스. anyOn 만 필요.
function useExamFlags(customerId: string | null | undefined) {
  return useQuery<{ blood: boolean; koh: boolean }>({
    ...
    queryFn: async () => {
      ...
      const SEL =
        'blood_test_requested, koh_requested, check_ins!inner(customer_id, status)';
      const { data, error } = await supabase
...
export default function TreatmentRequestBox({
  ...
  const sourceByCode = new Map(ctrRows.map((r) => [r.request_code, r.source]));

  // ── chart_treatment_requests upsert/delete (내성/각질/무좀) ────────────────
  ...
  useEffect(() => {
    ...
    if (ctrRows.length > 0 || derived.length === 0) return; // 이미 스냅샷됨 → 재삽입 금지(동결)
    snapshotDone.current = true;
    (async () => {
      const items = derived
        .map((code) => TREATMENT_REQUEST_ITEMS.find((i) => i.code === code))
        .filter((i): i is TreatmentRequestItem => !!i && i.axis === 'treatment');
  ...
  return (
    <div
      ...
      data-testid="pkg-tab-treatreq-section"
    >
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3" data-testid="treatreq-checkbox-grid">
        {TREATMENT_REQUEST_ITEMS.map((item) => {
          ...
          const derived = sourceByCode.get(item.code) === 'package_derived';
          return (
            <button
              key={item.code}
              type="button"
              ...
              data-testid={`treatreq-item-${item.code}`}
