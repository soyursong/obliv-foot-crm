...
// T-20260729-foot-DOCPRINT-BTN-ADMININFO-REPRINT-LINK (AC-1/AC-2): 발행 고정본 출력 공용 헬퍼(단일 소스) +
//   [행정정보 수정] 공용 편집기(치료테이블·2번차트 공유). 신규 출력/편집 스택 중복 금지.
...
import { Loader2, FileText, Users, CheckCircle2, Clock, Pencil, Printer } from 'lucide-react';
...
// ─── 서류 행정필드 편집기(재배선) 순수 로직 — SSOT = src/lib/docAdminEdit.ts ──────────
//   T-20260729-foot-DOCPRINT-BTN-ADMININFO-REPRINT-LINK (AC-3): 치료테이블·2번차트 두 surface 가 편집기를
//   공유하도록 순수 로직을 lib 로 승격. 여기선 backward-compat re-export(기존 E2E spec import 경로 유지).
export {
...
export type { DocAdminEditForm } from '@/lib/docAdminEdit';
...
export default function DiagDocSection({ date, nameInteraction }: Props) {
  ...
  // ── 행정정보 수정(공용 편집기, T-20260729-foot-DOCPRINT-BTN-ADMININFO-REPRINT-LINK AC-3) ──────────
  //   [행정정보 수정] → 서류 행정필드 전용 편집기(DocAdminEditDialog, 치료테이블·2번차트 공유). 편집 대상 =
  //   발행완료 요청행. 진료의 정정(Option A 컨펌 활성)·발급일·상병코드 편집(발행본 medical 본문 불변).
  ...
  // ── 출력(AC-1/AC-2): 발행 고정본 그대로 출력. 신규 출력 스택 금지 — printIssuedOpinionDoc 공용 헬퍼 재사용.
  //   행정정보 수정(adminOverrides)이 담당의·발급일을 정정했으면 출력물에 반영(단일 동선, AC-4).
  ...
  const handlePrintDoc = async (row: OpinionRequestRow | null) => {
    ...
    try {
      const ok = await printIssuedOpinionDoc({
        ...
        customerId: doc?.customerId ?? row.customerId ?? null,
        patientName: row.patientName ?? null,
        chartNo: doc?.chartNo ?? row.chartNo ?? null,
        body: (doc?.finalText || viewBody || '').trim(),
        docType: row.docType,
    ...
    <div className="flex flex-col gap-3" data-testid="diagdoc-section">
      ...
      <Dialog open={!!viewTarget} onOpenChange={(o) => { if (!o) setViewTarget(null); }}>
        ...
        >
          ...
          {/* 문서 뷰 하단 액션 — [행정정보 수정] · [출력] · [닫기].
              · [행정정보 수정] = 발행완료 요청행 행정필드 전용 편집기(발급요청일자/발급일/상병코드 + 진료의).
              · [출력] (T-20260729-foot-DOCPRINT-BTN-ADMININFO-REPRINT-LINK AC-2) = 발행 고정본 그대로 출력.
                행정정보 수정 옆에 인접 배치(현장 요청) — 출력 로직은 printIssuedOpinionDoc 공용 헬퍼 재사용.
          ...
          <DialogFooter className="shrink-0 flex-row justify-between gap-2 border-t pt-3 sm:justify-between">
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                data-testid="diagdoc-doc-view-print-btn"
              >
      ...
      {/* T-20260729-foot-DOCPRINT-BTN-ADMININFO-REPRINT-LINK: 서류 행정필드 전용 편집기(공용 컴포넌트, 치료테이블·2번차트 공유).
          편집 = 발행완료 요청행 field_data.admin_overrides(published 불오염, 의료법§22 스냅샷 불변).
          저장 후 all-time 발행완료 소스(별도 query key) 명시 invalidate(오버레이 즉시 반영, AC-4). */}
      <DocAdminEditDialog
        target={adminEditTarget}
        viewDoc={viewDoc}
        clinicId={clinicId}
        onClose={() => setAdminEditTarget(null)}
        onSaved={() => qc.invalidateQueries({ queryKey: ['opinion_request_published_all', clinicId] })}
        testIdPrefix="diagdoc-admin"
      />
    </div>
  );
}
