        ...
        return (
          <div key={status}>
            ...
            {isTreatment && showTreatmentSubmenu && hasTreatmentRooms && (
              <div className="border-t border-b border-amber-100 bg-amber-50/50 py-1 px-2">
                <div className="px-2 py-1 text-[10px] font-semibold text-amber-700 uppercase tracking-wide">
                  치료실 선택
                </div>
                {treatmentRooms.map((roomName) => (
                  <button
                    key={roomName}
                    className="flex w-full items-center gap-2 rounded-md px-3 py-2.5 text-sm text-amber-900 hover:bg-amber-100 active:bg-amber-200 transition font-medium"
                    onClick={() => {
