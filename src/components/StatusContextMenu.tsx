...
interface Props {
  ...
  /** 치료실 목록 (이름 배열) — T-20260511-foot-DASH-STAGE-ALL-SLOTS */
...
export function StatusContextMenu({
  ...
  // T-20260511-foot-DASH-STAGE-ALL-SLOTS: 방문유형 무관 전체 슬롯 표시 (동적 로드)
  const stages = NEW_PATIENT_STAGES;
    ...
    >
      ...
      {STATUS_FLAGS.map((flag) => {
        ...
        return (
          ...
          >
            <span className={cn('h-2 w-2 rounded-full shrink-0', STATUS_FLAG_DOT[flag])} />
      ...
      {/* ── 현 진행단계 변경 섹션 ───────────────────────────────────────────── */}
      <div className="px-2 py-0.5 text-[10px] font-semibold text-muted-foreground uppercase tracking-wide border-t border-b mt-0.5">
        현 진행단계
      </div>
          ...
          <div key={status}>
            ...
            >
              ...
              {showConsultArrow && (
                <ChevronRight
                  className={cn(
                    'ml-auto h-3 w-3 text-gray-400 transition-transform duration-150',
              ...
              {showTreatArrow && (
                <ChevronRight
                  className={cn(
                    'ml-auto h-3 w-3 text-gray-400 transition-transform duration-150',
                    showTreatmentSubmenu && 'rotate-90 text-amber-500',
                  )}
                />
              )}
              {showSubArrow && (
            ...
            {isConsult && showConsultSubmenu && hasConsultRooms && (
              <div className="border-t border-b border-indigo-100 bg-indigo-50/50 py-0.5 px-1">
                <div className="px-2 py-0.5 text-[10px] font-semibold text-indigo-700 uppercase tracking-wide">
                  상담실 선택
                </div>
                {consultationRooms.map((roomName) => (
                  <button
                    ...
                    className="flex w-full items-center gap-1 rounded-md px-2 py-1 text-[11px] min-h-[28px] text-indigo-900 hover:bg-indigo-100 active:bg-indigo-200 transition font-medium"
                    onClick={() => {
                ...
                <button
                  className="flex w-full items-center gap-1 rounded-md px-2 py-1 text-[10px] text-gray-500 hover:bg-gray-100 transition"
                  onClick={() => {
                    onStatusChange(checkIn, 'consultation');
                    onClose();
                  }}
            ...
            {isTreatment && showTreatmentSubmenu && hasTreatmentRooms && (
              <div className="border-t border-b border-amber-100 bg-amber-50/50 py-0.5 px-1">
                <div className="px-2 py-0.5 text-[10px] font-semibold text-amber-700 uppercase tracking-wide">
                  치료실 선택
                </div>
                {treatmentRooms.map((roomName) => (
                  <button
                    key={roomName}
                    className="flex w-full items-center gap-1 rounded-md px-2 py-1 text-[11px] min-h-[28px] text-amber-900 hover:bg-amber-100 active:bg-amber-200 transition font-medium"
                    onClick={() => {
                      if (onTreatmentStatusChange) {
                        onTreatmentStatusChange(checkIn, roomName);
                      } else {
                        onStatusChange(checkIn, 'preconditioning');
                      }
                      onClose();
                    }}
                  >
            ...
            {isLaser && showLaserSubmenu && hasLaserRooms && (
              <div className="border-t border-b border-emerald-100 bg-emerald-50/50 py-0.5 px-1">
                <div className="px-2 py-0.5 text-[10px] font-semibold text-emerald-700 uppercase tracking-wide">
                  레이저실 선택
                </div>
                {laserRooms.map((roomName) => (
                  <button
                    key={roomName}
                    className="flex w-full items-center gap-1 rounded-md px-2 py-1 text-[11px] min-h-[28px] text-emerald-900 hover:bg-emerald-100 active:bg-emerald-200 transition font-medium"
                    onClick={() => {
