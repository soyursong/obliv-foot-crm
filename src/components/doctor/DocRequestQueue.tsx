...
import {
  ...
  // T-20260729-foot-ALERTBOARD-DOBTXRX-COL-BLANK: 오늘시술·처방내역을 각 행의 방문(check_in_id) 스코프로 재결선
  //   (직전 today-글로벌+treatment_kind(NULL) 소스가 서류완료 과거일 행을 전면 공란화한 RC 교정).
...
export default function DocRequestQueue({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  //   AC-1 생년(만나이) = customers.birth_date live(스냅샷보다 우선, 8자리 정년도 파싱). AC-2 오늘시술 = 이 방문 package_sessions.session_type.
  //   AC-3 처방내역 = 이 방문 check_in_services 처방약. 대기+완료 두 테이블 동일 소스(방문 check_in_id 앵커).
  ...
  //   T-20260729-foot-ALERTBOARD-DOBTXRX-COL-BLANK AC-1: 표기(birthYearAgeDisplay)가 8자리 정년(ISO/스냅샷)도
  //   파싱하도록 교정됨 → live/스냅샷 어느 포맷이든 "YYYY (만 N세)" 결선(과거 6자리만 파싱하던 공란 RC 해소).
  ...
  // T-20260729-foot-ALERTBOARD-DOBTXRX-COL-BLANK (AC-2/3): 오늘시술·처방내역을 각 행의 방문(check_in_id) 스코프로 조회.
  //   AC-2 오늘시술 = 그 방문 package_sessions.session_type 간략형 / AC-3 처방내역 = 그 방문 check_in_services 처방약.
  const { data: visitProcRx = {} } = useQueueVisitProcedureRx(clinicId, checkInIds);
  // checkInId → 방문 시술/처방(없으면 빈 배열 → 셀 '—'). 내원 이력 없는 행(checkInId=null)도 graceful 공란.
...
function DocRequestRow({
  ...
  //   T-20260729-foot-ALERTBOARD-DOBTXRX-COL-BLANK: 소스를 이 행 방문(check_in_id) 스코프로 재결선.
  //   AC-3 처방내역 = 그 방문 check_in_services 처방약(', ' 나열). AC-2 오늘시술 = 그 방문 package_sessions.session_type
  //   간략형(차트2 티켓 차감/패키지 회차 차감). 차감/처방 없으면 '—'.
  ...
  return (
    ...
    >
      ...
      {/* T-20260729-foot-ALERTBOARD-DOBTXRX-COL-BLANK AC-2: 오늘시술 = 이 방문 package_sessions.session_type 간략형(티켓 차감) 전체 나열. */}
