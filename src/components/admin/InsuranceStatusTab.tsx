...
import { Loader2, Search, Pill, ShieldAlert, X, BadgeCheck, BadgeX, RefreshCw, Clock, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
export default function InsuranceStatusTab() {
  ...
  // HIRA 배치 동기화 현황(AC5) — 최근 실행 로그 read-only. RLS read=admin/manager.
  const { data: syncRuns, isLoading: syncLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await (supabase as any)
        ...
        .select('id,source_file,source_period,mode,status,total_rows,matched,updated,skipped_manual,unmatched,error_message,started_at,finished_at,run_by')
  ...
  return (
    <div className="space-y-4" data-testid="insurance-status-tab">
      ...
      {/* HIRA 배치 동기화 현황 (Phase2, AC5) — 심평원 약제급여목록 월간 xlsx 자동 갱신 이력 */}
