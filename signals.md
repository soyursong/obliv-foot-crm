...
| 2026-06-06 11:23 | dev-foot | deploy-ready | T-20260606-foot-DUTY-IMPORT-SHEET-FORMAT (P2): hold 해제+실측샘플 회수 후 구글시트 근무 캘린더 파서 재작성. ⚠️실측 시트가 "행=직원/열=날짜" flat 매트릭스가 아니라 주(week) 단위 캘린더 블록 확정 → extractCandidates 재작성(DutyRosterImportDialog.tsx). 셀 시맨틱: 셀에 이름 있으면 출근/비면 휴무(O/X 마킹 파싱 폐기—MARK_MAP/markToRosterType/parseSheetDate 제거, 매칭 직원 전부 regular). 로직: parseMonthHeader(연/월/팀 헤더 컨텍스트)→dayColumnsOf(day≥3 날짜행 분절)→resolveRowDates(칼럼→실날짜+월 롤오버: 일자 감소 시 다음달, 12월→연도+1)→날짜행 아래 칼럼별 비셀=출근자 matchStaff. 특수토큰: 휴진=skip/전직원=그날 활성staff 전체 확장/총괄=김주연(Q5)/이름 trim. 가드 불변(additive): AC-2 삽입 사람게이트·AC-4 (clinic,date,doctor) 중복차단·AC-6 admin/manager 권한. 실측 CSV(gid341864863) 단위검증: 8블록·2026-05-13~07-04·월롤오버(6-30→7-01)·전직원/총괄/휴진/trim 전부 정상. E2E 3 spec(주블록·월롤오버·특수토큰) + ALLSTAFF 회귀 모두 pass(4+4). build OK(3.52s). DB변경: 없음. commit 2001c73 pushed→main(Vercel auto). supervisor QA 요청 |
