...
| 2026-06-08 22:37 KST | dev-foot | deploy-ready | T-20260608-foot-RESV-AUTO-SMS-NOFIRE (FIX-REQUEST MSG-20260608-220603-p6sf, supervisor deploy_blocked_missing_credentials). **★배포 게이트 해소 — send-notification v6 ACTIVE★**. (1)**토큰 부재는 환경 문제 아님**: SUPABASE_ACCESS_TOKEN은 ~/.zshrc(line 5·20)에 존재 — supervisor 비대화형 셸이 source 안 했을 뿐. `source ~/.zshrc` 후 토큰 로드 확인. (2)**재배포 실행**: `supabase functions deploy send-notification --project-ref rxlomoozakkjesdqjtvd` → **v6(13:36 UTC=22:36 KST) ACTIVE**. 직전 세션 v5(2ff634a)에 이은 재배포로 현 HEAD 커밋(c1f15c9 toDomesticKR) 라이브 재확정. (3)**코드/DB 변경 0**: 배포만. EF 소스 last commit=c1f15c9(toDomesticKR), HEAD 포함 확인. (4)**FIX-REQUEST는 stale**: MSG 타임스탬프 22:06:03 < 직전 v5 배포 22:15:50 → 이미 해소된 게이트였음. 본 v6 재배포로 라이브 무결성 재보증. (5)**티켓 이미 field-soak**(§16, qa_result pass). 잔여=post-deploy 현장 confirm 1건(다음 +82 고객 실예약 auto-send → statusCode 4000 + 폰 실수신, responder→김주연 총괄). DB변경:없음. 코드변경:없음(배포만). |
