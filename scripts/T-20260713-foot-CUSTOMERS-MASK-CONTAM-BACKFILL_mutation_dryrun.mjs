 ...
 * post-probe: 6 phantom 잔존 + _backfill_* 3표 미생성 재확인 → 실영속 0 독립실측.
 *
...
const p1 = await q(`SELECT count(*) n FROM customers WHERE id IN (
  '0356b229-e8c7-4655-aa6e-651b15370c1f','512998d0-d51a-42c4-947e-b0cb2cc69da4',
...
console.log(`  phantom 6건 잔존(무영속 기대=6): ${n1}  ${n1==6?'✅':'❌ 영속 발생!'}`);
...
console.log('\nMUTATION_DRYRUN_RESULT:', JSON.stringify({
  sentinel_rollback: !!m,
  persistence: (n1==6 && n2==0) ? 'NONE' : 'LEAKED',
  phantom_survive: n1, backfill_tables: n2,
}));
if (!(n1==6 && n2==0)) { console.error('❌ 영속 감지 — supervisor 통지 필요'); process.exit(1); }
console.log('✅ 무영속 확증. mutation txn 리허설 PASS (실 apply=supervisor MIG-GATE).');
