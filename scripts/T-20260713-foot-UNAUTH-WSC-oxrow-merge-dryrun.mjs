...
for (const p of PAIRS) {
  ...
  console.log(`  ✓ ${p.label}: dup masked=✓ raw=✓ · phone-tail(${tail}) match=${tailMatch} · name-stem="${nameStem}" raw-name="${r.name}"`);
}
...
const nonZero = scan.filter((r) => r.n > 0);
log('전 FK 자식 스캔 (dup 참조 non-zero) — 전체 merge 대상', nonZero);
...
try {
  ...
  if (mm) {
    ...
    const ok = r.dup_children_after_reanchor_allFK === 0 && r.dup_deleted === 2;
    console.log(`\n판정: ${ok ? '✅ full merge 시뮬 정합 (전 FK 재앵커 후 dup 자식0·dup 삭제2)' : '❌ 시뮬 불일치'}`);
