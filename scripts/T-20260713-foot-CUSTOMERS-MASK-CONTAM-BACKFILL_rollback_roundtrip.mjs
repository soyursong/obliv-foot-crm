...
// 재스코프 5건(row1 0356b229 제외 — planner SPLIT MSG-20260714-020001-7xrp)
const PH=['512998d0-d51a-42c4-947e-b0cb2cc69da4','67ea1793-05e5-4d4a-b5c1-1ec73486e317','bd307dfe-79f0-4fea-86a6-0957cea492cd','44a6a076-ca66-458a-bdc5-e0a3a12c2e67','2dc21d1c-6e9f-4643-a733-dca92252d830'];
...
BEGIN
  -- 롤백 후 phantom 5건 복원됐는지
  SELECT count(*) INTO nph FROM customers WHERE id IN (${PH.map(p=>`'${p}'`).join(',')});
  -- 롤백 후 check_ins 가 다시 phantom 을 참조하는지(이동됐던 자식이 되돌아왔는지)
...
const pass = s.phantom_restored===5 && s.checkins_back_on_phantom>=5;
console.log(`  phantom 복원=${s.phantom_restored}/5  ·  check_ins phantom 복귀=${s.checkins_back_on_phantom}(≥5 기대)  ${pass?'✅':'❌'}`);
...
console.log(`  post-probe phantom 잔존(무영속 기대=5): ${nn}  ${nn==5?'✅':'❌'}`);
console.log('ROUNDTRIP_RESULT:',JSON.stringify({scope:'5-row',rollback_correct:pass,persistence:nn==5?'NONE':'LEAKED'}));
process.exit(pass&&nn==5?0:1);
