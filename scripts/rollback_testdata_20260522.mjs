/**
 * 롤백: [TEST6] 더미 데이터 삭제 — 5/22 현장 테스트 종료 후 실행
 * T-20260521-foot-DUMMY-TEST-DATA
 *
 * 삭제 순서 (FK 의존성):
 *   check_ins → reservations → customers
 *   (check_ins: 과거 체크인 포함 모두 삭제)
 */
...
async function main() {
  console.log('🗑️  [TEST6] 롤백 시작...');

  // Step 1: [TEST6] 고객 ID 목록 수집
  ...
  if (ids.length === 0) {
    console.log('  ℹ️  [TEST6] 데이터가 없습니다. 이미 삭제되었거나 삽입 전입니다.');
    return;
  }
