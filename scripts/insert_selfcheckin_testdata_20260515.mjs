/**
 * 풋센터 CRM 셀프접수 테스트 더미 예약 20건 — 5/16 현장 테스트용
 * T-20260515-foot-SELFCHECKIN-TESTDATA
 * 초진 10건 + 재진 10건 / [TEST2] prefix + is_simulation=true
...
// --- Main ---
async function main() {
  console.log('🚀 셀프접수 테스트 더미 예약 삽입 시작 (T-20260515-foot-SELFCHECKIN-TESTDATA)');
  ...
  // 중복 방지 체크 — [TEST2] 010-9902-0001 존재 여부 확인
  const { data: dupCheck } = await supabase
    .from('customers')
    .select('id')
    .eq('phone', '+821099020001')
    .eq('clinic_id', cid)
    .maybeSingle();
  if (dupCheck) {
    console.log('❌ +821099020001 이미 존재 — 이미 삽입된 것으로 보입니다. 종료.');
    console.log('💡 롤백 후 재실행: rollback_selfcheckin_testdata_20260515.sql');
    process.exit(0);
  }
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    const name = `[TEST2] 초진고객${num}`;
    const phone = `+82109902${String(i).padStart(4, '0')}`;  // +821099020001 ~ +821099020010
  ...
  for (let i = 1; i <= 10; i++) {
    const num = String(i).padStart(2, '0');
    const name = `[TEST2] 재진고객${num}`;
    const phone = `+82109902${String(10 + i).padStart(4, '0')}`;  // +821099020011 ~ +821099020020
  ...
  console.log('🗑  롤백: rollback_selfcheckin_testdata_20260515.sql 실행');

  // ==========================================================
  // AC-5 검증 — phone 기준 confirmed 예약 매칭 확인
  // ==========================================================
  console.log('\n--- AC-5 검증: phone → confirmed 예약 매칭 ---');
  const testPhones = ['+821099020001', '+821099020010', '+821099020011', '+821099020020'];
