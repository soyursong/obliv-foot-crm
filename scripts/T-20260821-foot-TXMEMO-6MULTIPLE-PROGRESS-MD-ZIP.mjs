/**
 * T-20260821-foot-TXMEMO-6MULTIPLE-PROGRESS-MD-ZIP — READ-ONLY 추출 (계보 재사용 · 코호트만 교체)
 *
 * P0 급건 (문지은 대표원장 직접 지시 "제일 0순위").
 * 계보: 5SEC-CLINITEXT(e93671c1)의 5섹션 추출 로직 **그대로 재사용**(재가공 금지). 코호트만 교체.
 *   부모(내원 3회+) → 본 티켓(오늘 기준 6배수 회차 도래자 = 경과분석 탭 모집단).
 * 동일 5섹션을 md 1파일에 구분선(---)으로 합산 생성:
 ...
 *   5. 임상 유의미 텍스트 (전체 상담메모·진료차트, 루틴상용구 제외 개별 특이정보만)
 *        ← customer_consult_memos(전량) + medical_charts(전 임상필드) − phrase_templates/super_phrases(루틴상용구)
 *
 * ── 코호트 (★교체된 부분 · 앱 SSOT src/lib/progressSixMultiple.ts + ProgressTargetsSection 그대로) ──
 *   대상 = "6배수 회차 도래자" (경과분석 탭 모집단, 오늘 2026-08-21 기준):
 *     · 활성 패키지(packages.status='active' AND total_sessions>0; 체험/Re:Born tier 0 배제) 보유,
 *     · used_sessions = package_sessions(status='used') per package,
 *     · anticipatedSession = used_sessions + 1 이 6의 배수(6·12·18·24…) → isSixMultipleTarget.
 *   is_test 제외(customers.is_test IS NOT TRUE). 헤더에 6배수 예정 회차(anticipated) + 다음 예약일 명시.
 *   ※ "예정 회차·예약일" = 도래하는(다음 내방) 6배수 회차 + 오늘 이후 미취소 최이른 예약(앱 정렬 로직 동일).
 *
 * canonical 소스 근거 (2026-08-21 DB 실측):
 *   - 코호트 = 위 6배수 도래(앱 경과분석 탭과 동일 모집단). 대상 4명·is_test 제외(이민태 1명 제외).
...
const OUT_ROOT = path.join(process.cwd(), '_artifacts', 'T-20260821-foot-TXMEMO-6MULTIPLE-PROGRESS-MD-ZIP');
const MD_DIR = path.join(OUT_ROOT, 'foot_경과분석_6배수회차도래');
const TODAY = '2026-08-21'; // 코호트 기준일(오늘). 다음 예약 = reservation_date >= TODAY 최이른.
...
(async () => {
  ...
  // ── 1. 대상 환자 (★코호트 교체 = 6배수 회차 도래자 · 앱 SSOT progressSixMultiple.isSixMultipleTarget 그대로) ──
  //   활성 패키지(status=active, total_sessions>0) + used_sessions(status='used') + (used+1)%6==0. is_test 제외.
  console.log(`\n=== 1. 대상 환자 (6배수 회차 도래 = 활성패키지 + (used+1)%6==0, 기준일 ${TODAY}, is_test 제외) ===`);
  const pkgTargets = await q(`
    WITH active_pkg AS (
      SELECT p.id, p.customer_id, p.total_sessions
      FROM packages p
      WHERE p.status = 'active' AND p.customer_id IS NOT NULL AND COALESCE(p.total_sessions, 0) > 0
    ),
    used AS (
      SELECT ps.package_id, count(*) AS n
      FROM package_sessions ps
      WHERE ps.status = 'used'
      GROUP BY ps.package_id
    )
    SELECT ap.id AS pkg_id, ap.customer_id, ap.total_sessions,
           COALESCE(u.n, 0) AS used_sessions, COALESCE(u.n, 0) + 1 AS anticipated,
           c.name, c.chart_number
    FROM active_pkg ap
    LEFT JOIN used u ON u.package_id = ap.id
    JOIN customers c ON c.id = ap.customer_id
    WHERE (COALESCE(u.n, 0) + 1) % 6 = 0
      AND c.is_test IS NOT TRUE
    ORDER BY c.name;
  `);
  const pkgRows = Array.isArray(pkgTargets) ? pkgTargets : [];
  console.log(`6배수 도래 대상 패키지 수: ${pkgRows.length}`);

  // 고객 단위 dedupe (고객이 복수 활성패키지 도래 시 anticipated 리스트로 합산 — 조용한 누락 금지)
  const byCust = new Map();
  for (const r of pkgRows) {
    if (!byCust.has(r.customer_id)) {
      byCust.set(r.customer_id, {
        id: r.customer_id, name: r.name, chart_number: r.chart_number,
        milestones: [],
      });
    }
    byCust.get(r.customer_id).milestones.push({
      pkg_id: r.pkg_id, anticipated: Number(r.anticipated),
      used: Number(r.used_sessions), total: Number(r.total_sessions),
    });
  }
  const patients = [...byCust.values()];
  console.log(`대상 환자 수(고객 dedupe): ${patients.length}`);
  const ids = patients.map(p => `'${p.id}'`);
  if (ids.length === 0) { console.error('대상 0명 — 중단'); process.exit(1); }
  const idIn = ids.join(',');

  // 내원 횟수(취소제외) — 부모 MD '내원 요약' 포맷 재사용용.
  const vcRows = await q(`
    SELECT customer_id, count(*) AS visit_count
    FROM check_ins
    WHERE status IS DISTINCT FROM 'cancelled' AND customer_id IN (${idIn})
    GROUP BY customer_id;`);
  const vcBy = new Map();
  for (const v of arr(vcRows)) vcBy.set(v.customer_id, Number(v.visit_count));
  for (const p of patients) p.visit_count = vcBy.get(p.id) ?? 0;

  // 다음 예약(오늘 이후 미취소 최이른) per customer — 헤더 '예정 회차·예약일'용 (앱 정렬 로직 동일, read-only).
  const resvRows = await q(`
    SELECT DISTINCT ON (customer_id) customer_id, reservation_date, reservation_time, registrar_name
    FROM reservations
    WHERE customer_id IN (${idIn})
      AND reservation_date >= '${TODAY}'
      AND status IS DISTINCT FROM 'cancelled'
    ORDER BY customer_id, reservation_date ASC, reservation_time ASC NULLS LAST;`);
  const resvBy = new Map();
  for (const rv of arr(resvRows)) resvBy.set(rv.customer_id, rv);
  ...
  for (const p of patients) {
    ...
    L.push('');
    // ── 6배수 예정 회차·예약일 (★신규 헤더 · 앱 경과분석 탭 anticipatedSession + 다음 예약) ──
    const nr = resvBy.get(p.id) ?? null;
    const msLabels = (p.milestones ?? [])
      .sort((a, b) => a.anticipated - b.anticipated)
      .map(m => `${m.anticipated}회 경과분석 (현재 ${m.used}/${m.total}회 진행 → 다음 내방 ${m.anticipated}회차)`);
    const resvLabel = nr
      ? `${nr.reservation_date}${nr.reservation_time ? ' ' + String(nr.reservation_time).slice(0, 5) : ''}${nr.registrar_name ? ' · 담당 ' + pad(nr.registrar_name) : ''}`
      : '(다음 예약 없음)';
    L.push('### 6배수 예정 회차 · 예약일');
    L.push('');
    L.push(`- 도래 회차(예정): ${msLabels.join(' / ')}`);
  ...
  // ── 요약 INDEX ──
  const idx = [];
  idx.push('# foot CRM 경과분석 자료 추출 요약 (6배수 회차 도래자)');
  idx.push('');
  idx.push(`- 계보: 5SEC-CLINITEXT(e93671c1) 5섹션 로직 그대로 재사용 · 코호트만 교체(내원3회+ → 6배수 회차 도래)`);
  idx.push(`- 코호트 기준(오늘 ${TODAY}): 활성 패키지(status=active, total_sessions>0) 보유 + (used_sessions+1) % 6 == 0. is_test 제외.`);
  idx.push(`  = 앱 경과분석 탭 모집단(src/lib/progressSixMultiple.isSixMultipleTarget). "예정 회차" = 다음 내방(6배수) 회차, "예약일" = 오늘 이후 미취소 최이른 예약.`);
  idx.push('');
  idx.push('## 섹션별 소스 (2026-08-21 DB 실측)');
  ...
  idx.push('');
  idx.push('## 커버리지');
  idx.push(`- 대상 환자: ${patients.length}명 (6배수 도래 활성 패키지 ${pkgRows.length}건 → 고객 dedupe)`);
  ...
  if (suspectTest.length) {
    idx.push('## ⚠ 테스트 의심 계정 (드롭 안 함 — 조용한 누락 금지)');
    idx.push('이름상 테스트로 보이나 is_test=false 라 canonical 필터(is_test IS NOT TRUE)에서 제외되지 않음. 경과분석 시 무시 가능.');
    for (const p of suspectTest) idx.push(`- ${p.chart_number ?? ''} / ${pad(p.name ?? '')} (내원 ${p.visit_count})`);
    idx.push('');
  }
  idx.push('| 차트번호 | 환자명 | 도래회차(예정) | 다음예약일 | 첫방문일 | 내원 | 치료메모 | 처방 | 과거력 | 첫날상담 | 상담메모(전량) | 유의미줄 |');
  idx.push('|---|---|---|---|---|---|---|---|---|---|---|---|');
  for (const p of patients) {
    const fvd = fvBy.get(p.id) ?? '';
    const nm = (memosBy.get(p.id) ?? []).length;
    const nrx = (rxBy.get(p.id) ?? []).length;
    ...
    const ms = (p.milestones ?? []).map(m => `${m.anticipated}회`).join(',');
    const rv = resvBy.get(p.id);
    const rvStr = rv ? `${rv.reservation_date}${rv.reservation_time ? ' ' + String(rv.reservation_time).slice(0, 5) : ''}` : '없음';
    idx.push(`| ${p.chart_number ?? ''} | ${pad(p.name ?? '')} | ${ms} | ${rvStr} | ${fvd} | ${p.visit_count} | ${nm} | ${nrx || '없음'} | ${hx} | ${cf} | ${cList.length} | ${kk} |`);
  }
  writeFileSync(path.join(MD_DIR, '_요약_INDEX.md'), idx.join('\n'), 'utf8');

  // ── zip ──
  const zipPath = path.join(OUT_ROOT, 'foot_경과분석_6배수회차도래.zip');
