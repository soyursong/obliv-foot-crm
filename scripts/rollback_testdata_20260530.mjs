/**
 * 풋센터 CRM 더미 데이터 정리 — 5/30 테스트 롤백 (V5)
 * T-20260530-foot-DUMMY-DATA-0530
 *
 * 삭제 대상:
 *   - is_simulation=true + phone IN (+821099060001 ~ +821099060128) 고객 및 연관 레코드
 *   - 전화번호 기준으로 정확히 삭제 (실환자 보호)
 */

import { createClient } from '@supabase/supabase-js';

// 삭제 대상 전화번호 (초진 64 + 재진 64 = 128명)
const TARGET_PHONES = [
  // 초진(동물) +821099060001 ~ +821099060064
  ...Array.from({ length: 64 }, (_, i) => `+8210990600${String(i + 1).padStart(2, '0')}`),
  // 재진(과일) +821099060065 ~ +821099060128
  ...Array.from({ length: 64 }, (_, i) => `+821099060${65 + i}`),
];
...
async function main() {
  console.log('🗑️  더미 데이터 정리 시작 (5/30 테스트 롤백 V5)');
