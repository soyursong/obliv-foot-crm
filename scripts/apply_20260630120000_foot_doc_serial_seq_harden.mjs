 ...
 *
 * ⚠ supervisor DDL-diff 게이트 통과 후에만 실행 (DA re-CONSULT GO s2cc + §3 의무5 spec 반영 확인).
 *   ADDITIVE: ADD COLUMN nullable / backfill 신규컬럼만 / partial unique / 신규 RPC. 기존행 mutation0.
 *
...
if (verifyOnly) {
  await runSql(
    `select
       ...
       (select bool_and(mx=cnt) from (select max(doc_serial_seq) mx, count(*) cnt from form_submissions group by clinic_id) t) as max_eq_count`,
    'VERIFY (col=1 rpc=1 valid_idx=1 max_eq_count=true 기대)');
  process.exit(0);
}
