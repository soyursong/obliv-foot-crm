/**
 * T-20260610-foot-DUMMY-RESV-REAPPLY — APPLY (prod INSERT 24 customers + 24 reservations)
 * 종로점(jongno-foot, clinic 74967aea) 2026-06-10 더미 예약 24건 재생성.
 * 12슬롯(12:00~17:30, 30분) × (초진1 new + 재진1 returning) = 24건.
 *
 * ⚠ CHARTVERIFY(09:06 cleanup됨)와의 유일·핵심 차이:
 *   - is_simulation=FALSE (true 절대 금지) — 10:00 배포 ADMIN-SIM-FILTER(9e065ae)가
 *     is_simulation=true 행을 admin+셀프접수 양쪽에서 숨김 → false라야 양쪽 노출되어 테스트 가능.
 *   - 식별 마커 memo='[TEST-DUMMY 20260610-REAPPLY]' (customers+reservations)
 *   - phone prefix +82108811 (09:06 cleanup된 +82108810과 구분)
 *
 * 차트 WSOD 재발 방지:
...
const MARKER = '[TEST-DUMMY 20260610-REAPPLY]';
...
// ── 0.5) 멱등 가드: REAPPLY 배치 잔존 시 abort (중복 INSERT 방지) ─────────
const { data: dup } = await sb.from('reservations').select('id').eq('clinic_id', CLINIC_ID).eq('reservation_date', DATE).eq('memo', MARKER);
if (dup?.length) { console.error(`ABORT: REAPPLY 배치 이미 ${dup.length}건 존재. cleanup 후 재실행 필요.`); process.exit(1); }
...
const phone = (seq) => `+8210${'8811'}${String(seq).padStart(4,'0')}`; // +82108811000X

// 초진(new) 12 + 재진(returning) 12 — 자연 한국 성명 24개 고유
const NEW12 = ['김태윤','이하린','박준호','최서진','정예원','강민호','조서아','윤지호','임채원','한도경','오나윤','서지후'];
const RET12 = ['신유나','권도현','황서연','안준우','류하은','배지안','남시우','문가은','양준서','백예진','고민재','차하준'];
...
// 6/9 JONGNO 30명 + 6/10 CHARTVERIFY 24명 이름과 충돌 검사 (clinic 내 동명이인 → homonym 분기 예방)
...
const CHARTVERIFY0610 = ['김도현','이준영','박서윤','최지우','정민서','강태현','조유빈','윤하준','임수빈','한예준','오지율','서동현','신아람','권민수','황지헌','안유리','류재민','배소율','남기훈','문채영','양승우','백하늘','고지원','차도윤'];
const collide = allNames.filter(n => JONGNO0609.includes(n) || CHARTVERIFY0610.includes(n));
if (collide.length) { console.error('기존 배치 이름과 충돌:', collide); process.exit(1); }

const custRows = [];
let seq = 0;
NEW12.forEach((name) => { seq++; custRows.push({ clinic_id: CLINIC_ID, name, phone: phone(seq), visit_type: 'new', is_simulation: false, memo: MARKER }); });
RET12.forEach((name) => { seq++; custRows.push({ clinic_id: CLINIC_ID, name, phone: phone(seq), visit_type: 'returning', is_simulation: false, memo: MARKER }); });
