/**
 * T-20260630-foot-TESTDATA-PURGE — 삭제 후 종결 검증 (READ-ONLY, throttle-safe)
 * AC4: 보존 30명 자식이력 무손상  AC5: orphan 0  AC6: 잔존 = 30
...
const ac6=a.total===30&&a.preserve_remaining===30&&a.leftover===0;
...
`))[0];
L(`\n[AC4] 보존 30명 자식이력 카운트(무손상 — 0 이상 정상 존재):`);
for(const [k,v] of Object.entries(ac4)) L(`  ${k}: ${v}`);

L(`\n=== 종결 판정 ===`);
L(`  AC6 잔존=30/보존=30: ${ac6?'PASS':'FAIL'}`);
