 ...
 *   질문2(김OO 3건, +8210XXXXXXXX) = 유지(별개방문, 시각역전 근거) → drop 안 함.
 *
 ...
 *
 * ★ 구조적 가드: 김OO 3건(customer 83ab4fe1)은 전부 동일 KST-day(2026-06-02) 활성이다.
...
// ── CEO 질문1 confirm: QA 가명 76 drop-target (김OO 3건 제외) ──────────────
const CANCEL_IDS = [
...
// ★ 절대 취소 금지 — CEO 질문2 "유지" 결정 (김OO 3건).
const NEVER_CANCEL_IDS = [
  "207bf234-8851-4a38-8c56-c0191bea96b8", // 김OO done
  "6425a5c8-8fb7-46d6-a762-93d9922eeb48", // 김OO done
  "d404c423-d638-4652-bf8c-daff068a361f", // 김OO payment_waiting
];
...
(async () => {
  ...
  out(`   CANCEL 후보(질문1): ${CANCEL_IDS.length}건 / NEVER_CANCEL(질문2 김OO): ${NEVER_CANCEL_IDS.length}건`);
  ...
  if (dup_groups !== 0) {
    ...
    out(`\n   ⚠️ 원인: 김OO 3건(동일 KST-day 2026-06-02) CEO "유지" + 미확인 테스트 그룹.`);
    out(`      partial index 는 day 단위라 김OO 3건 keep 시 동일 키 3중복 → 23505.`);
