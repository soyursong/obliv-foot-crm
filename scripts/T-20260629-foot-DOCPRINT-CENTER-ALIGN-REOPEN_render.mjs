// T-20260629-foot-DOCPRINT-CENTER-ALIGN (REOPEN / AC-5): 인쇄 출력물 "상단 하향" 실측 하니스(엔진-충실).
//
// 배경: 1차(DOCOUTPUT) 모델은 상·하 12mm 대칭(중앙배치)였으나 현장 2차 — "출력물이 아직 위로 쏠림,
//   엔터 4~5줄(약 60~80px) 더 아래로 내려달라". → 상단 @page margin 12mm→30mm 비대칭 하향.
// 본 하니스: 인쇄창이 소유하는 @page margin(상30/하12/좌우10mm)을 "시트(전체 A4) 안쪽 padding"으로 물리
//   재현하고, 콘텐츠박스(.page, A4-여백)를 그 안에 깔아 양식 wrap 위치를 시트 기준으로 측정한다.
// AC-5 판정: (1) 상단여백 ≈ 30mm 로 1차(12mm) 대비 ≥14mm(≈53px↑) 더 아래, (2) 하단여백 확보(클립 없음),
//   (3) 좌우 대칭 유지, (4) 콘텐츠박스 내(축소 없음), (5) 단일 페이지(넘침 없음) — 전 양식 일관.
// 실행: node scripts/T-20260629-foot-DOCPRINT-CENTER-ALIGN-REOPEN_render.mjs
...
// 인쇄창 @page margin (DocumentPrintPanel/PaymentMiniWindow/printOpinionDoc 와 동일) — 비대칭 상향
const PAGE_MARGIN_TOP = 30; // 상단(1차 12mm → 30mm, 약 +68px 하향)
const PAGE_MARGIN_BOT = 12; // 하단
const PAGE_MARGIN_LR  = 10; // 좌·우
const BASELINE_TOP    = 12; // 1차(DOCOUTPUT) 상단여백 — 비교 기준

fs.mkdirSync('evidence/docprint-reopen', { recursive: true });
...
async function measure(formKey, orient) {
  ...
  const boxWmm = sheetWmm - 2 * PAGE_MARGIN_LR;          // 콘텐츠박스 폭(= A4 - @page 좌우여백)
  const boxHmm = sheetHmm - PAGE_MARGIN_TOP - PAGE_MARGIN_BOT; // 콘텐츠박스 높이(= A4 - 상 - 하)
  ...
  await page.setContent(`<!DOCTYPE html><html lang="ko"><head><meta charset="utf-8"><style>
    ...
    .page-landscape { width: 100%; min-height: ${sheetHmm - PAGE_MARGIN_TOP - PAGE_MARGIN_BOT}mm; }
  ...
  const downDeltaMm = topMm - BASELINE_TOP;             // 1차(12mm) 대비 더 내려간 양
  const downDeltaPx = downDeltaMm * 96 / 25.4;

  // 판정 — 시트(전체 A4) 기준 물리 여백 (AC-5)
  const symLR    = Math.abs(leftMm - rightMm) < 3;      // 좌우 대칭(중앙정렬)
  const movedDown = downDeltaMm >= 14;                  // 1차 대비 ≥14mm(≈53px) 더 아래 — 현장 60~80px↑ 충족
  const noShift  = topMm > 20;                          // 상단 쏠림 없음(충분히 하향)
  const bottomOK = bottomMm > 5;                        // 하단 여백 확보(클립 없음)
  const fitsBox  = wrapWmm <= boxWmm + 0.5;             // 콘텐츠박스 내(축소 유발 없음)
  const onePage  = sheetHpx <= sheetHmm + 2;            // 단일 페이지(넘침 없음)
  const ok = symLR && movedDown && noShift && bottomOK && fitsBox && onePage;
  if (!ok) fail++;

  console.log(`  ${ok ? '✅' : '❌'} ${formKey} [${orient}]  ` +
    `여백 좌${leftMm.toFixed(1)}/우${rightMm.toFixed(1)}/상${topMm.toFixed(1)}/하${bottomMm.toFixed(1)}mm  ` +
    `하향Δ +${downDeltaMm.toFixed(1)}mm(${downDeltaPx.toFixed(0)}px)  ` +
    `wrap폭 ${wrapWmm.toFixed(1)}mm(박스 ${boxWmm}mm)  시트 ${sheetHpx.toFixed(0)}mm`);
  if (!ok) {
    console.log(`      [실패] 좌우대칭=${symLR} 하향=${movedDown} 쏠림없음=${noShift} 하단여백=${bottomOK} 박스적합=${fitsBox} 단일페이지=${onePage}`);
  }
  await page.screenshot({ path: `evidence/docprint-reopen/${formKey}.png`, fullPage: true });
  await page.close();
}

console.log('=== T-20260629 DOCPRINT-CENTER-ALIGN(REOPEN/AC-5) 인쇄 상단 하향 실측(@page 상30/하12 물리 재현) ===');
for (const k of PORTRAIT) await measure(k, 'portrait');
for (const k of LANDSCAPE) await measure(k, 'landscape');
await browser.close();
console.log(`\n결과: ${fail === 0 ? '전 양식 PASS ✅ (상단 ~30mm 하향·단일페이지·클립없음)' : `${fail}건 FAIL ❌`}`);
