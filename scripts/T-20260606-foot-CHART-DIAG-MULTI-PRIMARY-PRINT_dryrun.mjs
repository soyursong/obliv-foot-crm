...
const introspect = `
  select json_build_object(
    ...
    'indexes', (select json_agg(indexname order by indexname) from pg_indexes where schemaname='public' and tablename='chart_diagnoses'),
    'policies', (select json_agg(cmd order by cmd) from pg_policies where schemaname='public' and tablename='chart_diagnoses'),
...
chk('인덱스 chart/service 2종', (info?.indexes ?? []).filter((i) => /chart_diagnoses/.test(i)).length >= 3);
chk('RLS 활성 + 정책 4종(SELECT/INSERT/UPDATE/DELETE)',
  info?.rls === true && (info?.policies ?? []).length === 4);
