 ...
 *   AC-1 service_code KCD-8 형식 검증 ^[A-Z][0-9]{2}(\.[0-9]{1,4})?$
...
test('시나리오1 정상: 유효 KCD-8 코드는 통과(에러 null)', () => {
  for (const ok of ['M79', 'M79.3', 'S93.401', 'A00', 'Z99.9']) {
...
test('결선: DiagnosisNamesTab 가 diagnosisCode 검증을 저장/입력 경로에 결선', () => {
  const src = read(TAB);
  expect(src).toContain("from '@/lib/diagnosisCode'");
  expect(src).toContain('validateServiceCode');
  expect(src).toContain('isDuplicateDiagnosisName');
  // 저장 payload 는 정규화된 코드(대문자) — trim() 단독이 아닌 normalizeServiceCode
  expect(src).toContain('normalizeServiceCode(form.service_code) || null');
  // 인라인 에러 노출 + 저장 차단 결선
  ...
  expect(src).toContain('disabled={upsert.isPending || !!codeError}');
});
