...
test.describe('7ADJ ③ 엑셀 격자(시간 행 × 초진/재진 열) + 칸클릭 신규예약', () => {
  test('일간 뷰가 grid-cols-[64px_1fr_1fr] 엑셀 격자로 렌더 + 초진/재진 헤더', async ({ page }) => {
    ...
    // 시간 행(들)이 동일 grid 트랙으로 렌더. 구 수평 컬럼 토큰(w-[90px]) 잔존 금지(substrate 교체).
