...
test.describe('7ADJ ⑦ 고객박스 클릭 선택 → Ctrl+C 클립보드 힌트', () => {
  test('일간 격자 카드 클릭 → Ctrl+C → 클립보드 힌트 바 노출', async ({ page }) => {
    ...
    // 클립보드 복사/이동(Ctrl+C/X)은 설계상 confirmed('예약') 예약만 대상 (핸들러 가드 `r.status === 'confirmed'`, Reservations.tsx L1266/L1274).
    //   checked_in('체크인')·cancelled('취소') 카드는 이동 대상이 아니므로 Ctrl+C가 no-op → 힌트 미노출.
    //   FIX(T-20260630 phase2 재QA): 첫 카드가 체크인/취소일 수 있어 `.first()`로 무작정 집으면 no-op이 되던 flaky 원인 제거.
    //   → 상태 배지 '예약'(STATUS_LABEL.confirmed, exact) 을 가진 confirmed 카드를 명시 선택.
    const confirmedCards = page
      .locator('[data-testid^="resv-card-"]')
      .filter({ has: page.getByText('예약', { exact: true }) });
    test.skip((await confirmedCards.count()) === 0, 'confirmed(예약) 카드 없음 — skip');
    const card = confirmedCards.first();
    await card.click();
    await page.waitForTimeout(350); // 선택 상태 반영(일간 카드 onClick=setSelectedResvId 즉시)
    await page.keyboard.press('Control+c');
    const hint = page.getByTestId('clipboard-hint');
    await expect(hint).toBeVisible({ timeout: 3000 });
    await expect(hint).toContainText('복사 대기'); // copy 모드 힌트 문구 확인
  });
});
