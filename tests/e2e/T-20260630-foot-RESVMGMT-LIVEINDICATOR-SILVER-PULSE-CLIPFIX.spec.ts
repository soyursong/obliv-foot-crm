 ...
 *  S2 실버·펄스: 현재시각 컬럼이 존재하면 border-[#BBBBBB] + animate-live-border-pulse 보유 + amber 미보유,
...
// 현재 시각 컬럼(isNow) = border-[#BBBBBB] 보유 day 컬럼. 영업외/타일자면 0개일 수 있음.
async function findLiveColumn(page: Page) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (cls.includes('border-[#BBBBBB]')) return { el: cols.nth(i), cls };
  }
  return null;
}
...
test.describe('T-20260630-foot-RESVMGMT-LIVEINDICATOR-SILVER-PULSE-CLIPFIX — 실시간 반영 실버·펄스 + 클립 해소', () => {
  ...
  test('S2: 현재시각 컬럼은 실버(#BBBBBB) border + animate-live-border-pulse, amber 잔재 0, 헤더 bg-slate-100', async ({ page }) => {
    const grid = page.getByTestId('resv-day-horizontal');
    await expect(grid).toBeVisible({ timeout: 8000 });
    ...
    expect(live.cls, '실버 #BBBBBB border 보유').toContain('border-[#BBBBBB]');
