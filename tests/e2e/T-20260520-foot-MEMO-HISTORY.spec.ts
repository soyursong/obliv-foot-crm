...
test('AC-1: saveNewTreatmentMemo — INSERT + prepend to list (덮어쓰기 없음)', () => {
  ...
  // UPDATE 단건 (전체 교체 아님).
  // T-20260716 INPUT-LAG-DATALOSS-RCA: 입력 상태 격리로 saveTreatmentMemoEdit(text) 파라미터화 →
  //   editingMemoText.trim() 인라인 → const content = text.trim() 로 변경(동일 단건 UPDATE, payload 불변).
  expect(src).toContain(".update({ content, updated_at: now })");
...
test('AC-2: 작성자 이름 + 작성일시 포맷 표시', () => {
  ...
  // 작성일시 포맷 — 이후 티켓에서 date-fns 직접호출 → 공용 formatDateTimeDots(점 구분) 헬퍼로 전환됨.
  expect(src).toContain("formatDateTimeDots(memo.created_at)");
  // 알 수 없음 fallback
