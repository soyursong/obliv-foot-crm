...
test.describe('DOCREQ-DIAGCERT-CONTRA-MUTEX — 진단서↔금기증 배타 + 단일선택 가드', () => {
  ...
  test('SRC2 실장 요청박스 — 선택 UI 배타 disable + 제출 시점 배타 정규화 경유', () => {
    const src = read(REQUEST_BOX);
    // 선택 UI 배타 disable.
    expect(src).toMatch(/const disabled = hasDiagnosis/);
    expect(src).toMatch(/<button[\s\S]*?disabled=\{disabled\}/);
    // 제출 시점 가드 — applyPrefillExclusivity 정규화 후 persist(깨진 조합 저장 차단).
    expect(src).toMatch(/applyPrefillExclusivity\(\[\.\.\.selected\]/);
