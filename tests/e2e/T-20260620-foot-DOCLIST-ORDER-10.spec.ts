...
// T-20260630-foot-DOCCONFIRM-FORMPANEL-SPLIT supersede: 진료확인서 슬롯 단일 'treat_confirm' →
//   2 발급폼(code/nocode) 분리. 목록 10종 → 11항목(진료확인서만 1→2, 동일 서류종류 표시변이).
const EXPECTED_ORDER = [
  'bill_receipt',
  'bill_receipt_new', // T-20260714-foot-DOCFEE 완결: 진료비 계산서·영수증 신양식(원양식 바로 뒤)
  'bill_detail',
  'koh_result',
  'diag_opinion',
  'diagnosis',
...
test('시나리오1: SSOT DOCLIST_ORDER_10 — 확정 순서(신양식 additive 후 12항목)', () => {
  expect(DOCLIST_ORDER_10).toEqual(EXPECTED_ORDER);
  expect(DOCLIST_ORDER_10.length).toBe(12);
  // 중복 없음
  expect(new Set(DOCLIST_ORDER_10).size).toBe(12);
});

test('시나리오1: 결제미니창 — orderDocList 결과가 확정 순서/항목수(12)', () => {
...
// T-20260714-foot-DOCFEE-BODYCENTER-REDESIGN divergence 재발 가드:
//   DB row·fallback·compose handler 가 있어도 이 렌더 화이트리스트(DOCLIST_ORDER_10)에 미등록이면
//   orderDocList 필터에서 탈락 → 현장 목록 미노출. QA-PASS 였으나 이 불변식이 없어 divergence 가 통과했다.
//   두 화면(결제미니창·1/2번 차트)이 공유하는 orderDocList 결과에 신양식이 실제 포함됨을 고정한다.
test('DOCFEE 재발가드: bill_receipt_new 가 orderDocList 목록에 실제 노출(원양식 직후)', () => {
  const result = orderDocList(VISIBLE_DB_TEMPLATES);
  const keys = result.map((t) => t.form_key);
  expect(keys, '신양식이 렌더 목록에 없음(divergence 재발)').toContain('bill_receipt_new');
  // 원양식(bill_receipt) 바로 뒤 진열
  expect(keys.indexOf('bill_receipt_new')).toBe(keys.indexOf('bill_receipt') + 1);
  // 제증명 그룹에도 귀속(그룹 렌더에서 탈락하지 않음)
...
test('시나리오2: 차트 — 결제미니창과 동일 함수(SSOT) → 순서·항목 수 동일', () => {
  ...
  expect(chartResult).toHaveLength(12);
});
