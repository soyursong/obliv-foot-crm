...
// 기관명 슬롯({{clinic_name}}←clinics.name=사업자상호 옵션B)을 가진 전 출력서류 form_key.
// ⚠ T-20260714-foot-OBLIVORIGIN-INSTNAME-REPPRINT(요양기관명 축 델타): bill_detail·bill_receipt 의
//   '요양기관 명칭' 셀은 요양기관명 전용 축({{hira_institution_name}})으로 재배선됨 → 이 두 form 은
//   더 이상 {{clinic_name}} 슬롯을 갖지 않음(축 분리). 두 form 의 요양기관명 렌더 가드는
//   T-20260714-foot-OBLIVORIGIN-INSTNAME-REPPRINT.spec.ts 가 담당한다. ins_claim_form(보험청구서
//   의료기관명)은 rebind 대상 아님 → clinic_name 유지.
const FORM_KEYS_WITH_CLINIC_NAME = [
  ...
  'medical_record_request', 'diag_opinion_v2', 'rx_standard', 'ins_claim_form',
];
...
test.describe('기관명 옵션B 렌더 회귀 가드 (T-20260714 OBLIVORIGIN)', () => {
  ...
  test('데이터 구동 증명: 하드코딩 지점명 없음 — 입력값 그대로 렌더', () => {
    ...
    // (bill_receipt 는 요양기관명 축 분리로 clinic_name 슬롯 제거 → clinic_name 유지 form 인 diagnosis 로 검증)
