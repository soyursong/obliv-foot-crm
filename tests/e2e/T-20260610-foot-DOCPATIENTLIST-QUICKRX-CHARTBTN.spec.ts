...
test.describe('S2 AC2 — LOGIC-LOCK L-004 단일 차트 게이트웨이', () => {
  test('DoctorPatientList 가 진료차트 단일 진입(openTreatmentChart) 경유로 차트 진입(신규 경로 0)', () => {
    const src = SRC('components/doctor/DoctorPatientList.tsx');
    // T-20260621-foot-DOCDASH-PASTDATE-CHARTROUTE BUG-2 (문지은 대표원장 confirm):
    //   진료대시보드 이름 클릭 = '무조건 진료차트'. 기존 useChart().openChart(2번차트 서랍/고객차트) 진입을
    //   로컬 상태 MedicalChartPanel 직접오픈(openTreatmentChart)으로 정정 — 형제 DoctorCallDashboard와 동일 패턴.
    //   고객관리 등 다른 진입점의 useChart().openChart(2번차트 서랍, CHART-LOCK) 동선은 무접촉(AC-3).
    //   → 본 목록의 단일 차트 진입 게이트웨이 = openTreatmentChart (L-004 의도 보존, 라우팅 타깃만 진료차트로 정정).
    expect(src).toMatch(/const openTreatmentChart = \(customerId: string/);
    // PatientRow 에 onOpenChart = customer_id 있을 때만 진료차트 직접오픈(없으면 undefined → 버튼 비노출).
    expect(src).toMatch(/onOpenChart=\{row\.customer_id\s*\?\s*\(\)\s*=>\s*openTreatmentChart\(row\.customer_id[\s\S]*?:\s*undefined\}/);
...
test.describe('S4 회귀가드 — DATEMODE-HISTORY / SORT-LAYOUT / 확정패널 차트열기', () => {
  ...
  test('R2(SORT-LAYOUT): 기본 행 grid 고정 열 + 원내 우선 그룹 정렬 보존', () => {
    const src = SRC('components/doctor/DoctorPatientList.tsx');
    // 고정 grid-template(7트랙). T-20260617-foot-DOCDASH-DOCLIST-5FIX B2-3/B2-4 (문지은 대표원장 confirm):
    //   진료알림판 진료완료 섹션 미러 → 방·상태 칼럼 제거 + '당일시술' 칼럼 추가.
    //   컬럼셋: 방문유형(3rem)·이름(5rem)·차트번호(4.5rem)·당일시술(7rem)·처방(5.5rem)·예약메모(1fr)·액션(auto).
    expect(src).toMatch(/grid-cols-\[3rem_5rem_4\.5rem_7rem_5\.5rem_minmax\(0,1fr\)_auto\]/);
