...
test.describe('T-20260601 DOCTOR-CALL-POPUP-RELOC — 하단고정 → 슬롯 빈공간 팝업', () => {
  ...
  //   T-20260601-foot-DASH-HSCROLL-CHART-LOC #1 (REOPEN 정정):
  //   72314ef의 position:fixed(뷰포트 고정) 폐기 → POPUP-RELOC 원안(absolute, 슬롯 칸 종속)으로 복귀.
  //   단, 현장 요청대로 좌하단 → 우측 하단으로 위치 변경. 가로스크롤 시 콘텐츠와 함께 이동.
  test('AC-1·AC-2: 위젯이 하단 고정 바가 아닌 칸반 스크롤 컨테이너 내부 absolute(슬롯 종속) 팝업으로 렌더', async ({ page }) => {
    ...
    // AC-1) 하단 고정 바(static flex 자식)가 아니라 absolute(슬롯 칸 종속 팝업)로 렌더된다.
    const position = await list.evaluate((el) => getComputedStyle(el).position);
    expect(position).toBe('absolute');

    // 회귀 가드: 정정 마커 — 슬롯 종속(scroll-bound). fixed(뷰포트 고정)로 회귀 금지.
    await expect(list).toHaveAttribute('data-position-mode', 'scroll-bound');

    // AC-2) 칸반 슬롯 빈공간에 떠야 하므로, positioning 기준(가장 가까운 positioned 조상)이
    //       overflow:auto/scroll 컨테이너(=칸반 스크롤 영역) 안에 위치해야 한다(빈공간 종속).
    const insideScrollContainer = await list.evaluate((el) => {
      let cur = el.parentElement;
      while (cur && !cur.matches('[data-testid="dashboard-root"]')) {
        const cs = getComputedStyle(cur);
        if (cs.overflowX === 'auto' || cs.overflowX === 'scroll' ||
            cs.overflowY === 'auto' || cs.overflowY === 'scroll') {
          return true; // 칸반 스크롤 컨테이너 내부 = 빈공간 종속(팝업)
        }
        cur = cur.parentElement;
      }
      return false;
    });
    expect(insideScrollContainer).toBe(true);
