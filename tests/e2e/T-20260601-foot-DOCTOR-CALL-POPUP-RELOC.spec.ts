...
test.describe('T-20260601 DOCTOR-CALL-POPUP-RELOC — 하단고정 → 슬롯 빈공간 팝업', () => {
  // ── 시나리오1 / AC-1·AC-2: 하단 고정 해제 + 플로팅 팝업 ───────────────────────────────
  //   T-20260601-foot-DASH-HSCROLL-CHART-LOC #1로 supersede:
  //   기존(POPUP-RELOC): 칸반 스크롤 컨테이너 내부 absolute(빈공간 종속, 가로 스크롤 시 사라짐).
  //   변경(HSCROLL #1): position:fixed (뷰포트 좌하단 고정) — 가로 스크롤해도 화면에서 안 사라짐.
  test('AC-1·AC-2(supersede): 위젯이 하단 고정 바가 아닌 position:fixed 플로팅 팝업으로 렌더', async ({ page }) => {
    ...
    // HSCROLL #1) 하단 고정 바(static flex 자식)가 아니라 position:fixed(뷰포트 고정 팝업)로 렌더된다.
    const position = await list.evaluate((el) => getComputedStyle(el).position);
    expect(position).toBe('fixed');

    // HSCROLL #1) data-position-mode 마커 확인 (회귀 가드)
