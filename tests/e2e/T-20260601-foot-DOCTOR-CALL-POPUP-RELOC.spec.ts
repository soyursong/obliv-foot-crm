...
test.describe('T-20260601 DOCTOR-CALL-POPUP-RELOC — 하단고정 → 슬롯 빈공간 팝업', () => {
  // ── 시나리오1 / AC-1·AC-2: 하단 고정 해제 + 칸반 빈공간 absolute 플로팅 팝업 ──────────
  test('AC-1·AC-2: 위젯이 하단 고정 바가 아닌, 칸반 스크롤 컨테이너 내부 absolute 플로팅 팝업으로 렌더', async ({ page }) => {
    ...
    // AC-1) 더 이상 하단 고정 바(static flex 자식)가 아니라 absolute(플로팅 팝업)로 렌더된다.
    ...
    //       overflow:auto/scroll 컨테이너(=칸반 스크롤 영역) 안에 위치해야 한다(빈공간 종속, OPEN-Q A).
