...
test.describe('B. 처방내역 테이블 (현장 시나리오 2: 처방내역 표현)', () => {
  test('AC-4 처방내역 행(tr)에 가로 구분선(border-b) 추가, 마지막행 제거', () => {
    const src = PANEL();
    // gray-100→gray-200: 후속 EDITSTATE-RX-POLISH AC-4(reporter '잘 보이게' 재요청)로 구분선 가시성 강화(동일 구분선, 색만 진하게).
    expect(src).toMatch(/<tr\s+key=\{idx\}\s+className="border-b border-gray-200 last:border-b-0"/);
  ...
  // AC-12 (문지은 대표원장, 동일 스레드): 처방제품 셀 클릭/포커스 시 테두리(focus ring/outline) 안 뜸.
  // 분석: DIAG-RX AC-4 [&_input]:border-0 은 정적 테두리만 제거 → shadcn Input의 focus-visible:ring-2 잔존
  //       → 클릭 시 링(테두리) 노출. 포커스 상태 ring/offset/outline/shadow 까지 0 처리(case b).
  test('AC-12 처방내역 셀 클릭/포커스 시 테두리(focus ring/outline/shadow) 전제거', () => {
    const src = PANEL();
    // 처방내역 테이블 래퍼 div 에 포커스 상태 테두리 제거 arbitrary variant 적용.
    expect(src).toContain('[&_input]:focus-visible:ring-0');
    expect(src).toContain('[&_input]:focus-visible:ring-offset-0');
    expect(src).toContain('[&_input]:focus-visible:outline-none');
    expect(src).toContain('[&_input:focus]:border-0');
    expect(src).toContain('[&_input:focus]:shadow-none');
    expect(src).toContain('[&_button]:focus-visible:ring-0');
    // 포커스 제거 클래스가 prescription-items-table 래퍼에 함께 존재(같은 className 문자열).
    expect(src).toMatch(/\[&_input\]:focus-visible:ring-0[\s\S]{0,400}data-testid="prescription-items-table"/);
    // 회귀 가드: 정적 테두리 제거(DIAG-RX AC-4) + 행/세로 구분선(AC-4/5) 유지.
    expect(src).toContain('[&_input]:border-0 [&_input]:shadow-none [&_input]:bg-transparent [&_button]:border-0');
    // 입력·편집 접근성 유지 — 인라인 편집 onChange 핸들러(updateRxItem) 무변경.
    // (canon-align 2026-07-03: dosage 인라인 표시는 후속 RXTABLE-PRESCRIPTION-ALIGN AC4로 제거됨.
    //  updateRxItem 편집 경로는 '일수(days)' 셀에서 그대로 보존 — AC-12 편집 접근성 무회귀 가드.)
    expect(src).toContain("updateRxItem(idx, 'days', e.target.value)");
  });
});
...
test.describe('C. 진료일/진료의 (현장 시나리오 3: 한 줄·정렬)', () => {
  // canon-align 2026-07-03: 진료일/담당의 행은 후속 MEDREC-LAYOUT-4REFINE·DATE-DIAG-UI-REFINE로
  // 'flex flex-col sm:flex-row' → 'flex flex-wrap items-center'(라벨 stacking 제거)로 수렴.
  // AC-7 의도(진료일·담당의 한 줄 수평 배치)는 flex-wrap+items-center로 그대로 유지됨.
  test('AC-7 진료일·담당의 한 줄 수평 배치(flex flex-wrap items-center) — 유지', () => {
    const src = PANEL();
    expect(src).toMatch(/data-testid="chart-date-doctor-row"/);
    expect(src).toMatch(/flex flex-wrap items-center[\s\S]{0,60}data-testid="chart-date-doctor-row"/);
  });

  // canon-align 2026-07-03: 담당의 블록 우측끝 정렬(AC-8 sm:ml-auto)은 유지. 감싼 클래스는
  // 'sm:flex sm:flex-col sm:items-end' → 'flex items-center gap-2'로 수렴(select 내부에서 컬럼 stack).
  test('AC-8 담당의 블록이 행 가장 끝(우측, sm:ml-auto)', () => {
    const src = PANEL();
    expect(src).toContain('min-w-0 sm:ml-auto');
