...
test.describe('T-20260516-foot-CHART-UNIFORM-LOCK — 고객별 차트 동작 균일화', () => {
  test.beforeAll(async () => {
    ...
    const { data: custA } = await service
      .from('customers')
      .insert({ clinic_id: clinicId, name: `LOCK테스트A_${sfx.slice(-4)}`, phone: `DUMMY-${sfx}` })
      .select('id')
      .single();
    ...
    const { data: custB } = await service
      .from('customers')
      .insert({ clinic_id: clinicId, name: `LOCK테스트B_${sfx2.slice(-4)}`, phone: `DUMMY-${sfx2}` })
      .select('id')
      .single();
    ...
    const { data: ciB } = await service
      ...
      .insert({
        ...
        reservation_id: null,
        customer_name: `LOCK테스트B_${sfx2.slice(-4)}`,
        customer_phone: `DUMMY-${sfx2}`,    // phone으로만 연결
        visit_type: 'new',
        status: 'waiting',
      })
      .select('id')
  ...
  test('AC-3: customer_id null 케이스에서 phone 해석으로 고객 식별 가능', async () => {
    ...
    const phone = `DUMMY-${Math.floor(Math.random() * 100_000_000)}`;
