...
// 인쇄창 @page margin (DocumentPrintPanel.openBatchPrintWindow / printOpinionDoc 와 동일해야 함)
...
async function measureForm(page: import('@playwright/test').Page, formKey: string, orient: 'portrait' | 'landscape') {
  ...
  // 인쇄창 @page margin 을 시트(전체 A4) 안쪽 padding 으로 물리 재현 → 실제 프린터가 여백 존중 배치한 좌표와 동치.
...
test.describe('T-20260629-foot-DOCOUTPUT-PRINT-CENTER-LAYOUT — 출력물 중앙·여백 배치', () => {
  ...
  test('AC-4 — 공통 @page 물리 여백 메커니즘 소스 가드(인라인 땜질/전폭 회귀 차단)', () => {
    ...
    // 인쇄창(배치)·raw(소견서/진단서) 모두 HTML 양식 @page 에 물리 여백(12mm 10mm)을 선언해야 한다.
    expect(panel, 'openBatchPrintWindow HTML 모드 @page 물리 여백').toContain('@page { size: A4 portrait; margin: 12mm 10mm; }');
    expect(panel, 'openBatchPrintWindow HTML 모드 가로 @page 물리 여백').toContain('@page { size: A4 landscape; margin: 12mm 10mm; }');
    expect(opinion, 'printOpinionDoc raw @page 물리 여백').toContain('@page { size: A4 portrait; margin: 12mm 10mm; }');
    ...
    // 양식 wrap 은 자체 page 여백을 갖지 않는다(margin:0 auto). 전폭 page 안 margin:12mm auto 잔재(쏠림 재발) 0건.
    expect(tpl, 'form-wrap @media print 자체 page 여백 제거').toContain('.form-wrap { width: 190mm; min-height: 273mm; padding: 6mm 8mm; margin: 0 auto; }');
