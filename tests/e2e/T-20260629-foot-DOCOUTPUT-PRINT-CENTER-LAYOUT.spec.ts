...
// T-20260629-foot-DOCPRINT-CENTER-ALIGN(REOPEN/AC-6): 현장 3차 — 2차(30) 살짝 과함 → 상단 30mm→23mm(약 -7mm ≈ 2줄↑).
// T-20260706-foot-DOCCONFIRM-LAYOUT-VSPACE-ADJUST: 현장 추가 요청(총괄) — 상단 23mm→16mm(추가 -7mm ≈ 2줄↑).
const PAGE_MARGIN_TOP = 16; // 상단(1차 12 → 2차 30 → 3차 23 → 4차 16mm, 3차 대비 ~2줄↑)
...
async function measureForm(page: import('@playwright/test').Page, formKey: string, orient: 'portrait' | 'landscape') {
  ...
  // VSPACE-ADJUST(4차): 상단 ~16mm — AC-6(23mm) 대비 추가 2줄↑. 1차(12mm)보다는 아래, 3차(23mm)보다는 위.
  //   현장 총괄 추가 피드백("금일 위로 두줄 더") 충족. 상하 '대칭'은 의도적으로 비대칭화(상>하) 유지.
  expect(topMm, `${tag} 상 여백 하향 유지(>13mm)`).toBeGreaterThan(13);
  expect(topMm, `${tag} AC-6 대비 상향 재조정(<19mm, ~16mm)`).toBeLessThan(19);
  expect(topMm - BASELINE_TOP, `${tag} 1차 대비 ≥3mm 하향(VSPACE-ADJUST 유지)`).toBeGreaterThanOrEqual(3);
...
test.describe('T-20260629-foot-DOCOUTPUT-PRINT-CENTER-LAYOUT — 출력물 중앙·여백 배치', () => {
  ...
  // 시나리오 3: AC-4 — 공통 print 레이아웃에서 일괄 처리(중앙배치 물리 여백 메커니즘) 소스 가드.
  //   T-20260702-foot-DOCPRINT-BROWSERHEADER-REMOVE 로 모델 진화: 중앙배치 물리여백을 @page margin(비-0)에서
  //   @page margin:0 + 콘텐츠 padding(.page / body) 으로 이관(브라우저 자동 헤더 제거). 물리 위치(상30·좌우10·하12mm)
  //   불변 → 중앙배치 회귀 없음. 아래 소스 가드도 신규 모델(@page:0 + padding)로 갱신하되 "경로 divergence" 는 유지.
  test('AC-4/AC-5/AC-6 — 공통 중앙배치 물리 여백(상23/하12) 소스 가드(@page:0+padding 신규 모델 · 헤더 재노출/경로 divergence 차단)', () => {
    ...
    // AC-5/AC-6(갱신): 인쇄창(배치)·raw(소견서/진단서)·결제미니창(1순위) 모두 @page margin:0(헤더 제거) + 콘텐츠 padding(상23/좌우10/하12).
    //   AC-6(현장 3차): 상단여백 30mm→23mm(약 -7mm ≈ 2줄↑) 미세 재조정, 전 경로 일관.
    expect(panel, 'openBatchPrintWindow @page 세로 margin:0(헤더 제거)').toContain('@page { size: A4 portrait; margin: 0; }');
    expect(panel, 'openBatchPrintWindow @page 가로 margin:0(헤더 제거)').toContain('@page { size: A4 landscape; margin: 0; }');
    expect(panel, 'openBatchPrintWindow 물리여백 padding 이관(상16/좌우10/하12, VSPACE-ADJUST)').toContain('padding: 16mm 10mm 12mm;');
    expect(opinion, 'printOpinionDoc @page margin:0(헤더 제거)').toContain('@page { size: A4 portrait; margin: 0; }');
    expect(opinion, 'printOpinionDoc body padding 16mm 10mm 12mm').toContain('padding: 16mm 10mm 12mm;');
    // [경로 divergence 가드] 1순위 메인 출력(PaymentMiniWindow.buildPrintHtml)이 경로1과 동일 @page:0+padding 모델을 쓰는지.
    expect(mini, 'buildPrintHtml @page 세로 margin:0(경로1과 통일)').toContain('@page { size: A4 portrait; margin: 0; }');
    expect(mini, 'buildPrintHtml @page 가로 margin:0(경로1과 통일)').toContain('@page { size: A4 landscape; margin: 0; }');
    expect(mini, 'buildPrintHtml 물리여백 padding 이관(경로1과 통일)').toContain('padding: 16mm 10mm 12mm;');
    expect(mini, 'buildPrintHtml IMG-오버레이 격리 분기 유지').toContain('isLegacyImg');

    // 회귀 가드: 헤더 유발 비-0 @page 여백(상30/하12)으로 되돌아가면 실패(브라우저 헤더 재노출 차단).
    expect(panel.match(/@page \{ size: A4 (portrait|landscape); margin: 30mm 10mm 12mm; \}/g), 'openBatchPrintWindow 비-0 @page 여백 잔재 0건').toBeNull();
    expect(opinion.match(/@page \{ size: A4 portrait; margin: 30mm 10mm 12mm; \}/g), 'printOpinionDoc 비-0 @page 여백 잔재 0건').toBeNull();
    expect(mini.match(/@page \{ size: A4 (portrait|landscape); margin: 30mm 10mm 12mm; \}/g), 'buildPrintHtml 비-0 @page 여백 잔재 0건').toBeNull();
    ...
    // 양식 wrap 은 자체 page 여백을 갖지 않는다(margin:0 auto) + 콘텐츠박스 269mm(=297-16-12, VSPACE-ADJUST).
    expect(tpl, 'form-wrap @media print 콘텐츠박스 269mm + margin:0 auto').toContain('.form-wrap { width: 190mm; min-height: 269mm; padding: 6mm 8mm; margin: 0 auto; }');
