...
const PAGE_MARGIN_TOP = 23; // 상단(1차 12mm → 2차 30mm → 3차 23mm, 2차 대비 ~2줄↑)
...
async function measureForm(page: import('@playwright/test').Page, formKey: string, orient: 'portrait' | 'landscape') {
  ...
  // AC-6(REOPEN 3차): 상단 하향 — 1차(12mm) 대비 ≥9mm 더 아래(2차 30 대비 ~2줄↑=23mm) + 하단 여백 확보 + 단일 페이지(클립/넘침 없음).
  //   현장 3차 피드백("2차가 살짝 과함, 2줄만 위로") 충족. 상하 '대칭'은 의도적으로 비대칭화(상>하).
  expect(topMm, `${tag} 상 여백 하향 유지(>20mm)`).toBeGreaterThan(20);
  expect(topMm, `${tag} 2차 대비 상향 재조정(<26mm, ~23mm)`).toBeLessThan(26);
  expect(topMm - BASELINE_TOP, `${tag} 1차 대비 ≥9mm 하향(AC-6 2줄↑ 재조정)`).toBeGreaterThanOrEqual(9);
...
test.describe('T-20260629-foot-DOCOUTPUT-PRINT-CENTER-LAYOUT — 출력물 중앙·여백 배치', () => {
  ...
  test('AC-4/AC-5/AC-6 — 공통 중앙배치 물리 여백(상23/하12) 소스 가드(@page:0+padding 신규 모델 · 헤더 재노출/경로 divergence 차단)', () => {
    ...
    expect(panel, 'openBatchPrintWindow 물리여백 padding 이관(상23/좌우10/하12)').toContain('padding: 23mm 10mm 12mm;');
    expect(opinion, 'printOpinionDoc @page margin:0(헤더 제거)').toContain('@page { size: A4 portrait; margin: 0; }');
    expect(opinion, 'printOpinionDoc body padding 23mm 10mm 12mm').toContain('padding: 23mm 10mm 12mm;');
    ...
    expect(mini, 'buildPrintHtml 물리여백 padding 이관(경로1과 통일)').toContain('padding: 23mm 10mm 12mm;');
    ...
    // 양식 wrap 은 자체 page 여백을 갖지 않는다(margin:0 auto) + 콘텐츠박스 262mm(=297-23-12, AC-6).
    expect(tpl, 'form-wrap @media print 콘텐츠박스 262mm + margin:0 auto').toContain('.form-wrap { width: 190mm; min-height: 262mm; padding: 6mm 8mm; margin: 0 auto; }');
