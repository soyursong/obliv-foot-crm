...
test.describe('T-20260629-foot-DOCOUTPUT-PRINT-CENTER-LAYOUT — 출력물 중앙·여백 배치', () => {
  ...
  test('AC-4/AC-5 — 공통 중앙배치 물리 여백(상30/하12) 소스 가드(@page:0+padding 신규 모델 · 헤더 재노출/경로 divergence 차단)', () => {
    ...
    // AC-5(갱신): 인쇄창(배치)·raw(소견서/진단서)·결제미니창(1순위) 모두 @page margin:0(헤더 제거) + 콘텐츠 padding(상30/좌우10/하12).
    ...
    expect(panel, 'openBatchPrintWindow 물리여백 padding 이관(상30/좌우10/하12)').toContain('padding: 30mm 10mm 12mm;');
    expect(opinion, 'printOpinionDoc @page margin:0(헤더 제거)').toContain('@page { size: A4 portrait; margin: 0; }');
    expect(opinion, 'printOpinionDoc body padding 30mm 10mm 12mm').toContain('padding: 30mm 10mm 12mm;');
    ...
    expect(mini, 'buildPrintHtml 물리여백 padding 이관(경로1과 통일)').toContain('padding: 30mm 10mm 12mm;');
