 ...
 * AC-3/AC-4: 인접 요소(차트코드 열·서류발행·세금/합계) reflow·정렬·금액 계산 회귀 0.
 *
 ...
 *
 * ── T-20260720-foot-PAYMINI-CHARTCODE-SPLIT 반영 ──
 *   중앙 컬럼이 3분할 → 4분할로 재편: ① [data-testid="pmw-code-grid"]
 *   ② [data-testid="pmw-chartcode-col"](헤더 "차트 코드") ③ [data-testid="pmw-settle-lane"]
 *   (헤더 "진료비 산정") ④ [data-testid="pmw-zone3"]. 이 4-column split 이 기존 compact-toggle
 *   레이아웃을 대체함 — 접기/펼치기 토글 [data-testid="pmw-feeitem-toggle"] 및 그 행
 *   [data-testid="pmw-feeitem-row"] 는 제거되고, 차트코드 내용은 이제 독립 열
 *   [data-testid="pmw-chartcode-col"] 로 항상 노출된다(품목 리스트 [data-testid="pricing-list"] 는
 *   해당 열 내부에 잔존). 라벨 "수가 항목" → "치료내용".
 */
...
test.describe('T-20260713-foot-PAYMINI-VERTICAL-STACK-REVERT — 팔레트 가로 다열 그리드 복원', () => {
  ...
  test('AC-4: 인접 zone 유지 + 팔레트 클릭 시 합계 계산 회귀 없음', async ({ page }) => {
    ...
    // 인접 zone 유지(무접촉): 차트코드 독립 열 + 서류발행 패널
    // T-20260720-foot-PAYMINI-CHARTCODE-SPLIT: 4-column split 이 compact-toggle 을 대체.
    //   접기/펼치기 토글 pmw-feeitem-toggle 제거 → 차트코드는 이제 항상 노출되는 독립 열
    //   pmw-chartcode-col(헤더 "차트 코드"). 토글 클릭 불필요(내용 상시 표시).
    await expect(dialog.locator('[data-testid="pmw-chartcode-col"]')).toBeVisible();
    await expect(dialog.getByText('서류발행')).toBeVisible();

    // 팔레트 항목 클릭 → 수가/총액 계산 정상(세금 구분·settle-lane 볼드 총액 노출)
    // T-20260716-foot-SETTLE-LANE-TOTAL-LABEL-SPEC-STALE: COPAY-BALANCE-SPLIT(deployed) 이후
    //   settle-lane 볼드 총액 라벨 '합계'→'수납잔액'. 잔여 '합계'(진료비산정/세트/분할)는 금액 인라인
    //   동거라 exact:true 미매칭 → pmw-settle-lane 스코프 + '수납잔액' exact 로 교정(sweep 발견 3건째).
    const items = dialog.locator('[data-testid="pmw-palette-item"]');
    expect(await items.count()).toBeGreaterThanOrEqual(1);
