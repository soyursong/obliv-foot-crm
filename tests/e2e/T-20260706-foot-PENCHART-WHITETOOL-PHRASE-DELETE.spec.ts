...
// ── AC-2: 저장 시 상용구 rasterize '후' 화이트 재적용 = 화이트아웃 ──
//   ※SUPERSEDED — T-20260708-foot-PENCHART-REGRESSION-3FIX 이슈3(AC-3a/3c): 재적용 합성을
//     destination-out(투명화=구멍/양식 비침) → source-atop(불투명 흰색 덮기)로 전환.
//     현장 "화이트=지우개 동일 / 덮이지 않고 양식이 보임" 해소. bg(양식) 불변 불변식은 유지.
test.describe('WHITETOOL-PHRASE-DELETE AC-2: 저장본 화이트아웃(상용구 위 흰 덮기, source-atop)', () => {

  test('AC-2: 상용구 fillText 후 화이트 source-atop 재적용 → 획 통과 픽셀은 흰색 덮임, 미통과 픽셀은 상용구 보존', async ({ page }) => {
    ...
    const result = await page.evaluate(() => {
      ...
      // 2) 그 '후' 누적 화이트 획 source-atop 재적용(실코드 신규 블록)
      ...
      const underWhite = Array.from(ctx.getImageData(80, 100, 1, 1).data);   // 획 통과 → 상용구 위 흰 덮기
      const outsideWhite = Array.from(ctx.getImageData(150, 100, 1, 1).data); // 획 미통과 → 상용구 보존
      return { underWhite, outsideAlpha: outsideWhite[3], outsideR: outsideWhite[0] };
    });

    // 획이 지나간 상용구 픽셀 = source-atop 불투명 흰색으로 덮임(투명화 아님)
    expect(result.underWhite).toEqual([255, 255, 255, 255]);
    // 획이 지나가지 않은 상용구 픽셀 = 불투명 유지(상용구 그대로 남음)
    expect(result.outsideAlpha).toBe(255);
    expect(result.outsideR).toBe(0); // 검정 상용구 픽셀 보존
  });

  test('AC-2: 화이트는 별도 bg(양식) 캔버스를 건드리지 않는다(source-atop=draw 레이어 내용 위에만)', async ({ page }) => {
    await page.goto('about:blank');
    const result = await page.evaluate(() => {
      ...
      // draw 캔버스에서만 화이트 source-atop (양식 영역엔 draw 내용 없음 → no-op)
    ...
    // bg 양식은 완전 보존(파란 배경 그대로) — 화이트는 별도 draw 캔버스에만 작동(AC-FORM-SAFE)
