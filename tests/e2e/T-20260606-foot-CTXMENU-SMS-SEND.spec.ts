...
// ── 시나리오 2: 권한 게이트 (consultant 등 미노출) ─────────────────────────
test('S2: 권한 게이트 — 비-admin/manager 계정은 [문자] 항목 미노출', async ({ page }) => {
  const email = process.env.TEST_CONSULTANT_EMAIL;
  const password = process.env.TEST_CONSULTANT_PASSWORD;
  if (!email || !password) {
    test.skip(true, 'consultant 테스트 계정(TEST_CONSULTANT_EMAIL) 미설정 — 권한 게이트 스킵');
    return;
  }
  ...
  await menu.waitFor({ timeout: 5000 });
  // consultant 는 [문자] 미노출
  await expect(menu.getByTestId('quick-menu-sms-btn')).toHaveCount(0);
});
