...
test.describe('T-20260609-foot-PAY-DOCPRINT-FEE-MISSING — 수납 전 서류 진료비 누락', () => {
  ...
  test('AC-1/2: 진료비계산서(영수증)에 수납 전 라이브 진료비가 렌더된다', async ({ page }) => {
    ...
    // T-20260714-foot-DOCPRINT-GONGDAN-HIDE-COPAY-ONLY(B안) 이후 bill_receipt 소계·총 진료비 합계 셀은
    //   {{receipt_total}}(본인부담+비급여, 공단 제외) placeholder 로 개명됨(htmlFormTemplates L1724~1731).
    //   applyBillingFallback 은 total_amount(공단 포함 grandTotal)만 채우므로 렌더 경로(PMW/DocumentPrintPanel)와
    //   동일하게 receipt_total 을 별도 세팅해야 총 진료비 합계가 렌더된다. 이 픽스처는 공단 split 없는 순수 total 케이스라 동일값.
    values.receipt_total = values.total_amount;
