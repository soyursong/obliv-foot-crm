...
// ── S1: 행 선택 → 내보내기 → 파일 다운로드 ───────────────────────────────────
// T-20260613-foot-CUSTLIST-MULTISELECT-EXPORT supersede: xlsx → CSV(무의존), '내려받기' → '내보내기'.
test('S1: 행 체크박스 선택 후 [내보내기] → CSV 파일 다운로드', async ({ page }) => {
...
// ── S3: 미선택 상태에서도 내보내기 활성(필터된 전체) ──────────────────────────
// T-20260613-foot-CUSTLIST-MULTISELECT-EXPORT supersede: 선택0건 → 필터된 전체 CSV.
test('S3: 미선택 상태 [내보내기] → 필터된 전체 대상으로 다운로드', async ({ page }) => {
  await loginIfNeeded(page);
  const hasRows = await gotoCustomers(page);
  if (!hasRows) {
    test.skip(true, '고객 행 없음 — 스킵');
