 ...
 *   표기.
 *
 * ⚠ 형식 재조정(T-20260718-foot-RXPRINT-FORMAT-ADJUST 항목2, deployed 후속):
 *   구 '[코드] 약품명'(대괄호) → 신 '코드 | 약품명'(파이프). 본 spec 의 형식 assertion 은
 *   신 파이프 서식으로 갱신됨. prefix 위치(약품명 앞)·fallback(코드 없으면 약품명만) 성질은 불변.
 *
 ...
 *  - AC2 형식 = '코드 | 약품명' (파이프 + 공백 구분자)
 *  - AC3 코드 NULL/미매핑/공백 → 코드 없이 약품명만 (파이프 덩그러니·'null' 노출 금지 = graceful fallback)
...
// ── AC3: 코드 미등록/미매핑 → 코드 없이 약품명만 (파이프 덩그러니 금지) ──────────
test('AC3 fallback: service_code = null → 코드 없이 약품명만, 파이프·"null" 노출 없음', () => {
  const html = buildRxItemsHtml([{ name: '무코드약', code: null }]);
  const names = nameCells(html);
  expect(names).toContain('무코드약');
  // graceful: 파이프 덩그러니·null 문자열 절대 없음
  expect(html).not.toContain('| 무코드약');
...
test('AC3 fallback: 공백/whitespace code → prefix 미표기(파이프 덩그러니 방지)', () => {
  const html = buildRxItemsHtml([
    { name: '공백코드약', code: '   ' },
    { name: '빈문자약', code: '' },
  ]);
