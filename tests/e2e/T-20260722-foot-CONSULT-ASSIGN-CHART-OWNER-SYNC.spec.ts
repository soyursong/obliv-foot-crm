 ...
 *   item2  2번차트 Zone1 담당자(영구) 변경 → assigned_staff_id 수정 + 당일 열린 check_in.consultant_id 하향전파(scenario1, AC-1/AC-3).
...
test.describe('[정적] OWNER-SYNC 결정 로직', () => {
  ...
  test('헬퍼 계약: 당일(KST)·미취소만 대상 + rows-affected 검증', () => {
    const idx = chart.indexOf('const updateTodayOpenCheckInConsultant');
    expect(idx).toBeGreaterThan(0);
    const block = chart.slice(idx, idx + 1400);
...
test.describe('[DB] 하향전파 타깃팅 계약', () => {
  ...
  test('당일 열린 내원만 consultant_id 갱신 / 어제·취소 내원 불변 / 영구값 독립', async () => {
    const ts = Date.now();
    ...
    // 헬퍼 계약 재현: 당일(KST) + 미취소인 latestCheckIn(=A) 1건만 consultant_id 하향전파.
    //   (컴포넌트는 latestCheckIn 단건을 id로 갱신 — 여기선 그 대상 선별 결과가 A임을 확인 후 동일 UPDATE)
    ...
    // 검증: A만 newStaff / B(어제)·C(취소)는 oldStaff 불변
    ...
    expect(byName['A']).toBe(newStaff);   // 당일 열린 → 전파
