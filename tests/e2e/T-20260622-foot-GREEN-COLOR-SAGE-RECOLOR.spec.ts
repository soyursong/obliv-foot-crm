 ...
 *
 * ⚠ PALETTE SUPERSEDED → T-20260623-foot-GREEN-PASTEL-RETUNE (총괄 pick ③ 따뜻한 파스텔 그린).
 *   본 spec 은 sage 토큰이 적용된 "표면(surface) 집합"의 living regression 으로 유지하되,
 *   토큰 *값* 단언은 현행 파스텔 값(sage-100=#DCEDC8 등)으로 갱신됨. carve-out·구조 단언은 불변.
 *
 * 풋센터 녹색(emerald/green) 의미색 → 세이지(현 파스텔 그린) 톤 교체.
 *  - 현행 앵커 #DCEDC8(RGB 220,237,200) = sage-100(배지/칩 bg). WCAG AA 가능 스케일.
 *    text=sage-700(#556E32) on white 5.73:1 / on sage-100(#DCEDC8) 4.64:1 (AA 본문 ≥4.5 ✓).
 ...
 *   본 spec 은 그린이 'sage' 토큰(앵커 #DCEDC8)·dot 'bg-sage-500'·예약카드 'sage-400' 이던 시절 기준.
 *   이후 CHART2-MONOTONE redesign 으로 sage→그레이 재배치 + 그린→'firstvisit' 토큰 분리되어 현 코드와
 *   불일치(다중 redesign 으로 이미 red). 현 색 체계 권위 spec = T-20260625-...-WARMPASTEL-DESATURATE.
...
test.describe.skip('GREEN-COLOR-SAGE-RECOLOR — tailwind 토큰 (단일 SSOT)', () => {
  test('sage 스케일이 단일 토큰으로 정의되고 앵커 #DCEDC8=sage-100 이다(현행 파스텔)', () => {
    expect(tw).toMatch(/\bsage:\s*\{/);
    expect(tw).toMatch(/100:\s*"#DCEDC8"/i);   // 앵커(총괄 pick ③) = 배지/칩 bg
...
test.describe.skip('GREEN-COLOR-SAGE-RECOLOR — 대상 A: 신분증 확인 완료 배지', () => {
  test('확인 완료 배지가 파스텔 그린(#DCEDC8/#556E32/#C8DDA9/bg-sage-500)으로 교체됐다', () => {
...
test.describe.skip('GREEN-COLOR-SAGE-RECOLOR — 대상 B/C: 예약관리 녹색', () => {
...
test.describe.skip('GREEN-COLOR-SAGE-RECOLOR — 제외 영역 비침범', () => {
...
test.describe.skip('GREEN-COLOR-SAGE-RECOLOR — 컴파일 CSS 가드 (빌드 산출물)', () => {
  test('빌드 CSS 에 sage hex(앵커·bg·text)가 반영된다(JIT 생성 확인)', () => {
    ...
    //   (현행 파스텔) sage-100=#DCEDC8→220 237 200 / sage-500=#83A451→131 164 81 / sage-700=#556E32→85 110 50.
