...
test.describe('T-20260623-foot-DOCOUTPUT-DUP-ITEM-REMOVE — 잔존 "진료비 영수증 재발급" 중복 카드 제거 + 기능 통합', () => {
  ...
  // AC-3: '진료비 계산서·영수증' 행 → "영수증 관리" 펼침 토글로 연결
  //   T-20260719-foot-DOCLIST-RECEIPT-CONSOLIDATE-REORDER: 구 bill_receipt 목록 제거에 따라 "영수증 관리"
  //   펼침 패널을 정본 행(신양식 bill_receipt_new)으로 이관(재발급/등록/삭제 핸들러 그대로 — 기능손실 0).
  test('AC-3: 진료비 계산서·영수증(bill_receipt_new) 행 "영수증 관리" 펼침 토글로 패널 연결', () => {
    const src = dpp();
    // 부모 → TemplateSection 으로 정본 영수증 행에만 패널 주입
    expect(src).toContain("renderRowExtra={(formKey) => (formKey === 'bill_receipt_new' ? receiptManagePanel : null)}");
