...
test.describe('T5: DB — 패키지 미보유 초진 고객 버튼 렌더 조건', () => {
  test('DB-level: 패키지 없는 초진 check_in → btn 렌더, 패키지 있는 경우 → disabled', async () => {
    ...
    const { data: custA } = await sb
      .from('customers')
      .insert({ clinic_id: CLINIC_ID, name: `qa-T5-nopkg-${ts}`, phone: `+8210${String(ts).slice(-8)}`, visit_type: 'new' })
      .select()
      .single();
    ...
    const { data: custB } = await sb
      .from('customers')
      .insert({ clinic_id: CLINIC_ID, name: `qa-T5-haspkg-${ts}`, phone: `+8210${String(ts + 1).slice(-8)}`, visit_type: 'returning' })
      .select()
      .single();
