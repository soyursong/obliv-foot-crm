...
test.describe('AC1/AC3 STATS — 통계 집계 마이그 정합 (reconcile-R5)', () => {
  // reconcile-R5: 구 20260608160000(7컬럼, 42P13 stale)은 .SUPERSEDED 격리.
  // 정본=prod 실재 base 위에 body-only 필터 재이식한 단일 reconciling 마이그를 검증한다.
  const migFile = MIG('20260706120000_foot_stats_reconcile_iv_exclude_sameday_conv.sql');

  test('AC1: by_category 회차 집계(pkg_used)에서 session_type=iv 제외', () => {
    const m = read(migFile);
    expect(m).toContain("ps.session_type <> 'iv'");
    // by_category 함수가 교체 대상에 포함 + prod base = pkg_used (pkg_created 아님)
    expect(m).toContain('CREATE OR REPLACE FUNCTION foot_stats_by_category');
    expect(m).toContain('pkg_used AS (');
    // 시그니처 보존 (3컬럼) — 42P13 회피
    expect(m).toMatch(/RETURNS TABLE\s*\(\s*category\s+TEXT,\s*sessions\s+BIGINT,\s*amount\s+BIGINT/i);
  });

  test('AC3: therapist_summary 전환은 contract_date = 체험 내원일(kst_date) 인 건만 인정', () => {
    const m = read(migFile);
    expect(m).toContain('CREATE OR REPLACE FUNCTION foot_stats_therapist_summary');
    // prod 10컬럼 base 의 필드명 kst_date 로 당일전환 필터 재이식
    expect(m).toContain('pk.contract_date = b.kst_date');
    // 분모(exp_total)·차단 없음: experience 필터는 유지, 차감 insert 변경 문구 없음
    expect(m).toContain("WHERE b.visit_type = 'experience'");
    // 시그니처 보존 (10컬럼 v2) — designated_rate 까지 유지 = 후속계보 무regress
    expect(m).toContain('designated_rate');
  });
  ...
  test('REGRESSION 42P13: 구 stale 마이그(7컬럼) + pkg_created(G1 gated)는 .SUPERSEDED 로 격리', () => {
    // 활성 .sql 로 부활하면 db push 시 prod 10컬럼과 42P13 재발 → 강제 차단
    const staleActive = MIG('20260608160000_foot_stats_iv_exclude_trial_conversion.sql');
    expect(fs.existsSync(staleActive)).toBe(false);
    expect(fs.existsSync(staleActive + '.SUPERSEDED')).toBe(true);
    // pkg_created(G1 미충족 gated) 마이그도 별도 적용 차단(.SUPERSEDED)
    const pkgCreatedActive = MIG('20260619010000_foot_stats_by_category_pkg_created.sql');
    expect(fs.existsSync(pkgCreatedActive)).toBe(false);
    expect(fs.existsSync(pkgCreatedActive + '.SUPERSEDED')).toBe(true);
  });
});
