...
test.describe('AC-1 — 테이블뷰(열=이름|방|처방|상태) + 방이름 회귀 없음', () => {
  ...
  test('회귀가드: 이름→진료차트 / 진료차트 / 진료완료 / 의사ack 액션 보존', () => {
    ...
    // T-20260613-foot-DOCDASH-MONOTONE-RELAYOUT AC-4 supersede: 진료완료 버튼 + 의사ack 버튼/배지 3종
    //   → 상태셀 HandToggle(✋ 단색 3-상태) 단일화. ack/완료 SSOT(recordAck/applyStatusFlagTransition)는 토글 내부 보존.
...
test.describe('AC-2 — 임상경과 한 줄 인풋(singleLine), tall 아코디언 제거', () => {
  ...
  // T-20260616-foot-DOCDASH-ELAPSED-CLINICAL-3FIX AC-3 supersede: onSaved 에 미리보기 refetch(onClinicalSaved?.()) 추가.
  //   토글 접힘(setShowClinical(false)) 보존 + 저장 즉시 미리보기 반영. 양 섹션 동일(2회).
  test('저장 성공 시 onSaved → 토글 접힘(setShowClinical false) + 미리보기 refetch 보존(양 섹션)', () => {
    const src = DASH();
    const onSaved = src.match(/onSaved=\{\(\) => \{ setShowClinical\(false\); onClinicalSaved\?\.\(\); \}\}/g) ?? [];
