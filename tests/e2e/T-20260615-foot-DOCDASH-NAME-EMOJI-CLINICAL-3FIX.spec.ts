 ...
 * + item7(DOCDASH-RX-DISPLAY-REVAMP, 대표원장 명시 결정) — DoctorTools 서브탭 라벨 '처방 환자 목록'→'진료 환자 목록' 역전.
 *
 * 정적 소스 검증 스타일 — 인접 DOCDASH spec 컨벤션 동일(DASH()/TOOLS() 소스 grep).
 *
 * ⚠ GUARD (AC5 회귀 0):
...
// ─────────────────────────────────────────────────────────────────────────────
// item7 — DoctorTools 서브탭 라벨 '처방 환자 목록' → '진료 환자 목록' (REVAMP item7, conflict 해제)
// ─────────────────────────────────────────────────────────────────────────────
test.describe('item7 — 서브탭 라벨 진료 환자 목록(value/testid 불변)', () => {
  test('라벨 텍스트 = 진료 환자 목록 (처방 환자 목록 역전)', () => {
    const t = TOOLS();
    // 렌더되는 JSX 텍스트 노드(주석 제외)로 검증 — patient_list TabsTrigger 블록.
    const start = t.indexOf('data-testid="tab-patient-list"');
    const block = t.slice(start, t.indexOf('</TabsTrigger>', start));
    expect(block).toContain('진료 환자 목록');
    expect(block).not.toContain('처방 환자 목록');
  });

  test('value="patient_list" / data-testid="tab-patient-list" 절대 불변(E2E·상태키 보존)', () => {
    const t = TOOLS();
    expect(t).toContain('value="patient_list"');
    expect(t).toContain('data-testid="tab-patient-list"');
  });

  test('헤딩 설명문구 일관(진료 알림판 · 진료 환자 목록 · 균검사지)', () => {
    const t = TOOLS();
    expect(t).toContain('진료 알림판 · 진료 환자 목록 · 균검사지(KOH)를 확인합니다.');
  });
});
