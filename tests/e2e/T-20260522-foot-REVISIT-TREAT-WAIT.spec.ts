...
// ── AC-2: 대시보드 칸반 '치료대기' 칸 — 소스 정적 검증 ──────────────────────────
// 2026-05-31 재작성: qa_runner 3회 연속 미인증 false-negative(browser QA login 리다이렉트)를
// 해소하기 위해 브라우저 렌더 체크 → 소스 코드 정적 검증으로 대체.
// 실제 렌더링은 auth storageState 필요 → playwright desktop-chrome 프로젝트로 실행 시 AC-1b/AC-3으로 커버됨.
// 본 AC-2 소스 체크는 auth 없이 실행 가능 (파일시스템 읽기만).
//
// 검증 항목:
//   ① treatment_waiting_col: '치료대기' — 칸반 칼럼 헤딩 매핑 존재
//   ② COLUMN_ORDER 배열에 'treatment_waiting_col' 포함 — 칸반에 렌더됨
//   ③ nextStatus = returning ? 'treatment_waiting' : 'consult_waiting' — 재진→치료대기 분기
//   ④ status: nextStatus (INSERT 직접 세팅) — 2단계 패턴 폐기 확인 (ebe1dd7 fix 생존)

test('AC-2: 대시보드 칸반 치료대기 칸 — Dashboard.tsx 소스 정적 검증 (auth 불필요)', () => {
  ...
  // ① 칸반 칼럼 헤딩 매핑
  expect(dashSrc, "treatment_waiting_col: '치료대기' 매핑 없음").toContain(
    "treatment_waiting_col: '치료대기'"
  );

  // ② COLUMN_ORDER/DEFAULT_COL_ORDER 배열에 포함
  expect(dashSrc, "COLUMN_ORDER에 'treatment_waiting_col' 없음").toContain(
    "'treatment_waiting_col'"
  );

  // ③ 재진→치료대기 nextStatus 분기 (ebe1dd7 핵심 fix)
  expect(
    dashSrc,
    "재진→treatment_waiting nextStatus 분기 없음 — ebe1dd7 fix 회귀 의심"
  ).toMatch(/visit_type\s*===\s*['"]returning['"]\s*\?\s*['"]treatment_waiting['"]/);

  // ④ INSERT payload에 status: nextStatus 직접 세팅 (2단계 UPDATE 패턴 폐기 확인)
  expect(dashSrc, "status: nextStatus INSERT 없음 — 2단계 패턴 회귀 의심").toContain(
    'status: nextStatus'
  );
});
