 ...
 *
 * 수정 이력:
 *   2026-05-31 — FIX-REQUEST(supervisor MSG-20260531-021419) 반영
 *     - 보고된 AC-1c/AC-3 '셀프 접수' 미노출은 QA가 잘못된 base url
 *       (https://happy-flow-queue.pages.dev/jongno-foot = 롱레 도메인)로 실행한 환경 이슈.
 *       풋 셀프접수는 localhost:8089 또는 obliv-foot-crm 배포의 /checkin/jongno-foot 경로.
 *       정식 webServer(localhost:8089) 기준 AC-1c/AC-3 PASS 확인.
 *     - 실제 결함: AC-1b/AC-3 NewCheckInDialog의 name:'재진'/'초진' 이 예약 슬롯
 *       버튼('빨강 10:30 재진' 등 4개)과 strict mode 충돌 → dialog 스코프 + exact:true로 특정.
 *   ★ canonical BASE_URL = http://localhost:8089 (playwright webServer) — 롱레 도메인 사용 금지
...
test('AC-1c: SelfCheckIn — 재진 경로 진입 가능한 초기 UI 확인', async ({ page }) => {
  // Fix: SelfCheckIn 초기 화면은 이름+연락처 입력 → "예약하고 왔어요/예약 없이 방문했어요" 레이아웃.
  // "재진"/"초진" 버튼은 고객 DB 매칭 이후 단계에서만 나타남 → 초기 진입 UI 검증으로 대체.
  // 코드 경로: SelfCheckIn.tsx line 866
  //   `status: visitType === 'returning' ? 'treatment_waiting' : 'consult_waiting'`
  // e2e_spec_exempt_reason: selfcheckin_btn_phase2_only (재진/초진 버튼은 DB 매칭 후 단계)
  await page.goto(`${BASE_URL}/checkin/jongno-foot`);
  await expect(page.getByText('셀프 접수').or(page.getByText('Self Check-In')).first()).toBeVisible({ timeout: 15000 });

  // 예약 여부 버튼 — 초기 화면에서 접근 가능한 UI 검증
...
test('AC-3: SelfCheckIn — 초진 경로 진입 가능한 초기 UI 확인 (회귀 방지)', async ({ page }) => {
  // Fix: SelfCheckIn 초기 화면은 이름+연락처 입력 → "예약하고 왔어요/예약 없이 방문했어요" 레이아웃.
  // "초진"/"재진" 버튼은 고객 DB 매칭 이후 단계에서만 나타남 → 초기 진입 UI 검증으로 대체.
  // 코드 경로: SelfCheckIn.tsx line 866
  //   `status: visitType === 'returning' ? 'treatment_waiting' : 'consult_waiting'`
  // e2e_spec_exempt_reason: selfcheckin_btn_phase2_only (초진/재진 버튼은 DB 매칭 후 단계)
  await page.goto(`${BASE_URL}/checkin/jongno-foot`);
  await expect(page.getByText('셀프 접수').or(page.getByText('Self Check-In')).first()).toBeVisible({ timeout: 15000 });

  // 초기 화면에서 접근 가능한 UI 검증 — 두 진입 경로 모두 노출 확인
  await expect(page.getByRole('button', { name: '예약하고 왔어요' })).toBeVisible();
  await expect(page.getByRole('button', { name: '예약 없이 방문했어요' })).toBeVisible();
});
