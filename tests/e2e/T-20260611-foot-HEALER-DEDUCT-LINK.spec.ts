...
test.describe('시나리오1: 다음 예약 힐러 표시', () => {
  ...
  test('AC1-4: 카드 자신이 힐러면 중복 표시 안 함 + 미래(카드보다 늦음)일 때만 노출', () => {
    ...
    // T-20260614-foot-HEALER-RESV-CLASSIFY-DEF supersede: 가드를 healer_flag 단독 → resvKind(영속 is_healer_intent||healer_flag)로 격상.
    //   is_healer_intent(영속)로 힐러인 카드도 '다음 힐러' 배지 중복 회피 대상에 포함(분류 SSOT 일치).
