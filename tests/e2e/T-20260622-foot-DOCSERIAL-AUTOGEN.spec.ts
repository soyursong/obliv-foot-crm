...
test('회귀: 미등록 form_key → 안전 fallback(null = 발번 보류, 임의 prefix 금지)', () => {
...
// T-20260630-foot-DOCCONFIRM-FORMPANEL-SPLIT: treat_confirm_code/nocode 는 동일 서류종류(진료확인서)
//   표시변이 → prefix 'VC' 공유 등록(11번째 서류종류 신설 아님).
test('SPLIT: treat_confirm_code/nocode 는 VC 공유(서류종류 1개 유지)', () => {
...
// ── AC-2: 서류종류코드 단일 config ──────────────────────────────────────
// T-20260630-foot-DOCCONFIRM-FORMPANEL-SPLIT supersede: 진료확인서 표시변이 2종(code/nocode) 추가 등록 →
//   prefix 키 10 → 12 (VC 공유, 서류종류는 여전히 10종). VC/AV 분리는 유지.

test('AC-2: prefix 단일 config — 확정 매핑(VC=진료확인서, AV=통원확인서 분리)', () => {
  expect(DOC_SERIAL_PREFIX).toEqual({
    ...
    diagnosis: 'DIAG',
    treat_confirm: 'VC',
    treat_confirm_code: 'VC',
    treat_confirm_nocode: 'VC',
    referral_letter: 'REF',
    visit_confirm: 'AV',
    medical_record_request: 'MR',
    rx_standard: 'RX',
  });
  expect(Object.keys(DOC_SERIAL_PREFIX)).toHaveLength(12);
