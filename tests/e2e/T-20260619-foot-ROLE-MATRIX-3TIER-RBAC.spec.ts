...
test.describe('T-20260619-foot-ROLE-MATRIX-3TIER-RBAC — 진료관리 EDIT 게이트(canEditClinicMgmt)', () => {
  ...
  test('봉직의(director, flag無) → 진료관리 수정 X (진료만)', () => {
    expect(canEditClinicMgmt(P.doctor_assoc)).toBe(false);
  });
  ...
  test('하위호환: role 문자열 인자도 동작', () => {
    ...
    expect(canEditClinicMgmt('director')).toBe(false); // flag 없는 문자열은 봉직의로 간주(보수적)
