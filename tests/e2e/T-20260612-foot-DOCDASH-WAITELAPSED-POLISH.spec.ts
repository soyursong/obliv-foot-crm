...
test.describe('시나리오1 — AC-2 헤더 경과시간 + AC-3 +N분', () => {
  // T-20260613-foot-DOCDASH-MONOTONE-RELAYOUT AC-2 supersede: 헤더 '경과시간' → '시간'(시계 이모지 제거).
  //   대기 섹션 '시간' 칼럼 유지(헤더 1), 완료 섹션은 시간 칼럼 제거(UX7). 구 '경과시간'/'콜경과시간' 잔존 0.
  test("헤더 '경과시간' → '시간'(대기 섹션, 구 라벨 잔존 0)", () => {
    const s = DASH();
    expect(s).toContain('>시간</th>');
    expect(s).not.toContain('>경과시간</th>');
...
test.describe('시나리오2 — AC-0 회귀 rebase(부모 동작 보존)', () => {
  ...
  // MONOTONE-RELAYOUT AC-4 supersede: HandRaiseFlow → HandToggle(✋ 단색 3-상태). 진료완료 전이(pink) 1곳 보존(신설 write 0).
  test('MONOTONE-RELAYOUT: HandToggle + applyStatusFlagTransition(pink) 전이 1곳(신설 write 0)', () => {
    const s = DASH();
  ...
  // MONOTONE-RELAYOUT AC-3 supersede: 차트 칼럼 신설 → 호출 9 / 완료 8(완료 시간 칼럼 제거 유지). flat 테이블 구조·순서 보존.
  test('RESTRUCTURE: flat 테이블 보존 — 호출 9칼럼 / 완료 8칼럼(MONOTONE-RELAYOUT)', () => {
    const s = DASH();
    const colgroups = s.match(/<colgroup>[\s\S]*?<\/colgroup>/g) ?? [];
    const theads = s.match(/<thead>[\s\S]*?<\/thead>/g) ?? [];
    expect(colgroups.length).toBe(2);
    expect((colgroups[0].match(/<col /g) ?? []).length).toBe(9);
    expect((colgroups[1].match(/<col /g) ?? []).length).toBe(8);
    expect(theads.length).toBe(2);
    expect((theads[0].match(/<th /g) ?? []).length).toBe(9);
    expect((theads[1].match(/<th /g) ?? []).length).toBe(8);
    expect(s).toContain('const DOCDASH_COLSPAN = 9');
    expect(s).toContain('const DOCDASH_COMPLETED_COLSPAN = 8');
    // MONOTONE-RELAYOUT: 테이블당 인라인 펼침(차트) 행 1개 — 임상경과/처방 펼침은 셀 내부 토글(별도 full-width 행 아님).
