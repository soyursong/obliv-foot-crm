...
test.describe('T-20260617-foot-CHECKIN-CHART-LINK-3KEY — 복합키 오배정 차단', () => {
  ...
  //
  //   [SKIP 사유 — cross-repo 경로 결함, FIX-REQUEST MSG-20260721-103357-qqoq]
  //   SelfCheckIn.tsx 는 obliv-foot-crm 트리에 존재하지 않는다. 키오스크 셀프접수 화면은
  //   별도 레포 `foot-checkin/src/pages/SelfCheckIn.tsx`(line 1732~)에 실재하며 AC-1 ① 복합키
  //   fix 도 그곳에 적용돼 있다(이후 T-20260716 에서 self_checkin_resolve_v3 RPC 서버측 복합키
  //   match-or-insert 로 재구현 → phoneCanonDigits/ambiguousLink 문자열 마커 자체도 stale).
  //   따라서 이 레포의 Playwright CI 는 해당 파일을 readFileSync 할 수 없어(ENOENT) S5 가 항상
  //   결정적 FAIL → 회귀신호 오염. 코드 결함이 아니라 spec 의 repo 가정 오류이므로 skip 한다.
  //   회귀 커버리지는 foot-checkin 레포 자체 spec 으로 이관 필요(별도 티켓/레포 스코프).
  //
  //   [커버리지 이관 완료 — T-20260721-foot-SELFCHECKIN-E2E-COVERAGE-RESTORE]
  //   S5 의 검증 의도(SelfCheckIn 셀프접수 고객해소 = 성함 AND 연락처 복합키, phone 단독
  //   잔존 금지)는 원 레포 soyursong/foot-checkin 에서 등가 복원됨:
  //     · tests/e2e/T-20260721-foot-SELFCHECKIN-COMPKEY-COVERAGE-RESTORE.spec.ts
  //     · scripts/checkin-compkey-guard.mjs (규칙 SSOT, `npm run test:checkin-guard`)
  //     · .github/workflows/checkin-compkey-guard.yml (회귀 시 red, secret-free)
  //   현행 실경로 = 서버 SECDEF RPC fn_selfcheckin_upsert_customer_resolve_v3(성함+연락처
  //   복합키 match-or-insert, link_status linked/created/ambiguous) + kiosk SelfCheckIn.tsx.
  //   obliv-foot-crm 내 S5 는 대상 파일 부재로 skip 유지가 정답(이관 후에도 계속 skip).
  test.skip('S5: SelfCheckIn 체크인 생성 — 복합키(성함 AND 연락처) 해소(정적) [SKIP: SelfCheckIn 은 foot-checkin 레포 소속 → 커버리지 이관 T-20260721-...-COVERAGE-RESTORE]', async () => {
    const fs = await import('fs');
    const path = await import('path');
    const { fileURLToPath } = await import('url');
    const dir = path.dirname(fileURLToPath(import.meta.url));
  ...
  // ── S7 (AC-7 차트번호 1급 권위 키): 교차검증이 연결 차트의 UNIQUE 차트번호를 조회·표기 ──
  //   총괄 reframe(MSG-utzu): 고객박스=고객차트, 차트번호(F-XXXX)는 예약 등록 시 자동 발번된
  //   UNIQUE·NOT NULL 키 = 세 키 중 최강 disambiguator. 성함/연락처 상충 시 확인 프롬프트에
  //   차트번호를 1급 권위 키로 명시해 staff 가 어느 차트인지 정확히 식별하게 한다.
  test('S7: verifyChartLinkOrConfirm 이 chart_number 를 1급 권위 키로 조회·표기(정적)', async () => {
    const fs = await import('fs');
    const path = await import('path');
    const { fileURLToPath } = await import('url');
    const dir = path.dirname(fileURLToPath(import.meta.url));
    const src = fs.readFileSync(path.resolve(dir, '../../src/pages/Dashboard.tsx'), 'utf8');
    // 교차검증이 chart_number 를 SELECT 하는지 (1급 권위 키 조회)
    expect(/\.select\('name, phone, chart_number'\)/.test(src), 'verifyChartLinkOrConfirm 이 chart_number 를 조회하지 않음(AC-7)').toBe(true);
    // 차트번호를 확인 프롬프트/경고에 표기(chartLabel/[F-..]) — 1급 권위 키 명시
    expect(src.includes('chartLabel'), '차트번호 표기 라벨(chartLabel) 없음 — AC-7 1급 권위 키 미명시').toBe(true);
    // AC-7 참조 주석 존재(매칭 우선순위 명문화)
    expect(src.includes('AC-7'), 'AC-7(차트번호 1급 권위 키) 참조 주석 없음').toBe(true);
  });
});
