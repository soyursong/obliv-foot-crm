 ...
 * AC-3: 카테고리 [서류] → [담당자] 라벨 변경 (T-20260706: '원장님' → '담당자', key='document' 불변)
 * AC-4: 펜차트 [불러오기] 패널도 사이드 메뉴 카테고리 + 담당자 라벨
...
test('AC-3: 상용구 사이드 메뉴에 [담당자] 라벨이 표시되고 [원장님]/[서류]가 없다', async ({ page }) => {
  await loginAsAdmin(page);
  const found = await openPhrasesTab(page);
  if (!found) {
    test.skip(true, '어드민 탭 미노출 — 스킵');
  ...
  await expect(sideMenu).toContainText('담당자');

  // [원장님]/[서류] 텍스트가 카테고리 버튼 레이블에 없음
...
// ── AC-3: 상용구 추가 다이얼로그 카테고리 select에도 담당자 존재 ───────────
test('AC-3: 상용구 추가 다이얼로그 카테고리에 [담당자] 옵션 존재', async ({ page }) => {
  await loginAsAdmin(page);
  const found = await openPhrasesTab(page);
  if (!found) {
    test.skip(true, '어드민 탭 미노출 — 스킵');
...
test('AC-4: 펜차트 draw 모드 상용구 불러오기 패널이 사이드 메뉴 구조를 사용한다', async ({ page }) => {
  ...
  // AC-3: 담당자 라벨 존재, 원장님/서류 없음 (T-20260706)
  await expect(catTabs.locator('[data-testid="phrase-cat-document"]')).toContainText('담당자');
  await expect(catTabs.locator('[data-testid="phrase-cat-document"]')).not.toContainText('원장님');
