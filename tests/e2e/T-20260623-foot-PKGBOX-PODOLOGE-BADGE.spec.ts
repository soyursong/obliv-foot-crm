 ...
 * 활성 패키지 중 podologe_sessions>0 보유 고객을 [PD] 배지(orange)로 표시.
 *
 ...
 * AC-3: PD 배지가 orange 계열 스타일 — 기존 패키지(violet)/초진 딱지와 시각 구분, 가로 오버플로우 없음
 *
 * 현장 클릭 시나리오:
 *   S-0: 포돌로게 보유 카드에 PD 배지가 실제 렌더된다 (AC-1)
 *   S-1: PD 배지가 orange 계열 스타일이다 (AC-3 시각 구분)
...
test.describe('T-20260623-foot-PKGBOX-PODOLOGE-BADGE — 대시보드 포돌로게(PD) 배지', () => {
  ...
  test('S-1: AC-3 — PD 배지가 orange 계열 스타일', async ({ page }) => {
    const pdCard = await gotoDashboardWithSeededCards(page);
