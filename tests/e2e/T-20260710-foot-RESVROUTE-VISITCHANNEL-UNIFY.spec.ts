...
test.describe('T-20260710 RESVROUTE-VISITCHANNEL-UNIFY — DB 계약(비파괴)', () => {
  test('AC1/AC6-a: customers.visit_route CHECK 가 통일 6값 전부 허용', async () => {
    ...
    for (const v of UNIFIED_MUST_HAVE) {
      const { data, error } = await sb!
        .from('customers')
        .insert({ clinic_id: clinicId, name: TEST_TAG, phone: `DUMMY-${Date.now()}`, visit_type: 'new', visit_route: v })
        .select('id')
        .single();
  ...
  test('AC1/AC6-b: legacy 인콜 값도 여전히 허용(기존행 read-time 보존)', async () => {
    ...
    const { data, error } = await sb!
      .from('customers')
      .insert({ clinic_id: clinicId, name: TEST_TAG, phone: `DUMMY-${Date.now()}`, visit_type: 'new', visit_route: '인콜' })
      .select('id')
      .single();
  ...
  test('AC3/AC4: 지인소개 + referral_name 저장·재조회 유지', async () => {
    ...
    const { data, error } = await sb!
      .from('customers')
      .insert({ clinic_id: clinicId, name: TEST_TAG, phone: `DUMMY-${Date.now()}`, visit_type: 'new', visit_route: '지인소개', referral_name: '홍길동' })
