 ...
 *
 * 항목1 (교부번호): '제 20260718000025 호' → '20260718 제 000025 호'
...
test('AC1: 저장 issue_no(14자리) → 표시 date8/순번 분리 (issue_date=20260718, issue_no=000025)', () => {
  const stored = buildIssueNo('20260718', 25, 6)!; // '20260718000025' (부모 저장값)
  const out = splitIssueNoForDisplay({ issue_no: stored });
  // 양식 '{{issue_date}} 제 {{issue_no}} 호' 슬롯 조립 결과 = '20260718 제 000025 호'
  expect(out.issue_date).toBe('20260718');
  expect(out.issue_no).toBe('000025');
  expect(`${out.issue_date} 제 ${out.issue_no} 호`).toBe('20260718 제 000025 호');
});
...
test('AC1 파라미터화: N=5(심평원 13자리) 저장값도 앞8/나머지로 정확 분리', () => {
  ...
  expect(`${out.issue_date} 제 ${out.issue_no} 호`).toBe('20260718 제 00025 호');
});

test('AC1 발번값 무변경: split 은 앞8+나머지 재결합 시 원본 issue_no 와 동일(로직 무변경)', () => {
  const stored = buildIssueNo('20260718', 7, 6)!;
  const out = splitIssueNoForDisplay({ issue_no: stored });
  expect(`${out.issue_date}${out.issue_no}`).toBe(stored); // 재결합 = 저장 발번값 그대로
});
...
test('AC4 멱등: 이중 적용 안전 — 멱등은 issue_no 포맷으로 판정(issue_date 가드 아님)', () => {
  ...
  expect(twice.issue_date).toBe('20260718');
...
test('AC1 운영경로: issue_date=today(dashed) 선바인딩 + 14자리 issue_no → compact 8자리로 교정되어 split', () => {
  ...
  // 가드 제거로 split 이 실제 발동 → 앞날짜가 dashed today 가 아닌 compact 8자리로 덮어써짐
  expect(rendered.issue_date).toBe('20260718'); // '2026-07-18' 이 아님 (compact 교정 확인)
  expect(rendered.issue_no).toBe('000025');
  // 슬롯 조립 결과 = 요구 형식
  expect(`${rendered.issue_date} 제 ${rendered.issue_no} 호`).toBe('20260718 제 000025 호');
  // 타 필드 무오염
