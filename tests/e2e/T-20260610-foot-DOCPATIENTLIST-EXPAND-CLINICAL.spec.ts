...
function clinicalGate(args: {
  ...
  // T-20260615-foot-DOCPATIENTLIST-DONE-CLINICAL-READONLY: 진료완료(completed_at||pink) 여부.
  //   기본 false → 기존 호출 모델(완료조건 미가산) 무회귀.
  isVisitDone?: boolean;
}): ClinicalGate {
  ...
  // T-20260615-foot-DOCPATIENTLIST-DONE-CLINICAL-READONLY: 편집 활성 = '당일' AND '진료 미완료'.
  //   AC-3(당일 외 = 읽기전용)는 그대로 — 완료조건만 OR 가산(회귀 0).
  const readOnly = !args.isToday || !!args.isVisitDone;
  // 당일 & 미완료 = readOnly false → 편집(신규 작성 즉시 editMode) 가능. 당일 외 또는 진료완료 = readOnly true → 편집 차단.
...
test.describe('S1 — 당일 접수환자: 임상경과 보기+수정', () => {
  ...
  test('DoctorPatientList: 펼침 임상경과에 readOnly={!isToday || isVisitDone} 전달 — 당일 & 진료 미완료만 편집', () => {
    ...
    // T-20260615-foot-DOCPATIENTLIST-DONE-CLINICAL-READONLY: AC-3(당일외=읽기전용) 회귀 0 유지 + 완료조건 OR 가산.
...
test.describe('S2 — 당일 외 환자: 읽기전용/미표시 (오기입 방지)', () => {
  ...
  test('당일 외/진료완료 라벨에 "읽기전용" 명시 — 현장 인지', () => {
    const src = SRC('components/doctor/DoctorPatientList.tsx');
    // T-20260615-foot-DOCPATIENTLIST-DONE-CLINICAL-READONLY: 라벨 게이트도 완료조건 가산(!isToday || isVisitDone).
