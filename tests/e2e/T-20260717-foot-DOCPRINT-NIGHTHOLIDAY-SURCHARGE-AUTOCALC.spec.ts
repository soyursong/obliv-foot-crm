...
test.describe('AC-1/AC-3 — 출력시점 야간·공휴일 자동 판정 (겹침=공휴일 우선 단일)', () => {
  ...
  // ── addendum 2026-07-19 신규 요건 1: 토요일 야간 회귀 재검증.
  //    일요일(dow===0) 수정이 다른 요일 야간 로직을 깨지 않았는지 가드.
  //    토요일(dow===6)은 공휴일 아님 → 18시 이후는 'night'(holiday 아님)여야 함.
  test('★토요일 야간 회귀(dow===6): 18시 이후 night, 주간 null, 공휴일 아님', () => {
    const satNight = at(2026, 7, 18, 19); // 2026-07-18(토) 19:00
    const satNoon = at(2026, 7, 18, 14); // 2026-07-18(토) 14:00
    expect(satNight.getDay()).toBe(6); // dow===6 = 토요일
    // 토요일 야간 → night (holiday 로 오판되지 않음 = 일요일 수정 회귀 없음)
    expect(detectSurchargeKind(satNight, false)).toBe('night');
    expect(surchargeMark(detectSurchargeKind(satNight, false), 'night')).toBe('■');
    expect(surchargeMark(detectSurchargeKind(satNight, false), 'holiday')).toBe(' ');
    // 토요일 주간 → 가산 없음(회귀0)
    expect(detectSurchargeKind(satNoon, false)).toBeNull();
    // 18시 경계 토요일도 평일과 동일하게 동작
    expect(detectSurchargeKind(at(2026, 7, 18, 18, 0), false)).toBe('night');
    expect(detectSurchargeKind(at(2026, 7, 18, 17, 59), false)).toBeNull();
  });
