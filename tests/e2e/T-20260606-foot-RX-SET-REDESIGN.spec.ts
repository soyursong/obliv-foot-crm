...
test.describe('T-20260606 RX-SET-REDESIGN — 약품 폴더 트리 + 묶음처방 분할', () => {
  ...
  // ── 시나리오 2: 약품 여러 건 추가 (AC-R5) ────────────────────────────────────────
  //   T-20260609-foot-RXSET-ITEM-ARROW-INSERT: 체크박스 다중선택+'선택 추가' bulk UI 제거 →
  //   각 약품 좌측 `<` 즉시삽입 버튼을 순차 클릭해 여러 건 적재(1클릭=1건).
  test('S2: 약품 좌측 `<` 버튼 순차 클릭 → 여러 건 추가', async ({ page }) => {
    await openMedicalChartRx(page, seed!.customerId);

    // 약1(루트) + 약2(하위) — 좌측 `<` 즉시삽입 버튼 순차 클릭
    await expect(drugItem(page, DRUG_1).first()).toBeVisible({ timeout: 10_000 });
    await drugItem(page, DRUG_1).first().getByTestId('drug-folder-item-arrow').click();
    await expect(drugItem(page, DRUG_2).first()).toBeVisible();
    await drugItem(page, DRUG_2).first().getByTestId('drug-folder-item-arrow').click();
    ...
    // GUARD: 제거된 bulk UI(체크박스/'선택 추가')는 더 이상 존재하지 않음
