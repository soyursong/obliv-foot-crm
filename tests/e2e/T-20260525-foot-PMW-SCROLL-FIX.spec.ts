...
test('AC-1: 세트코드 드롭다운 리스트에 max-h-48 overflow-y-auto 클래스 포함', async ({ page }) => {
  ...
  // T-20260713-foot-PAYMINI-ZONE2-CHARTFEE-LEFTSPLIT: b276877b 이후 세트코드 드롭다운은
  //   컴팩트 [차트 코드 · 진료비 산정] 접힘 토글 '내부'에 위치 → 펼침 선행 필요(shipped 동작 정합).
...
test('AC-4/5: PaymentMiniWindow 기본 렌더 + 세트코드 드롭다운 토글', async ({ page }) => {
  ...
  // 컴팩트 한 줄 헤더 (b276877b 이후 '·', 항상 표시)
  await expect(page.locator('text=차트 코드 · 진료비 산정')).toBeVisible({ timeout: 8000 });

  // T-20260713-foot-PAYMINI-ZONE2-CHARTFEE-LEFTSPLIT: 세트코드 드롭다운은 접힘 토글 내부 → 펼침 선행.
  await page.locator('[data-testid="pmw-feeitem-toggle"]').first().click();
