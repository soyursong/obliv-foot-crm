...
test.beforeAll(async () => {
  ...
  const { data: cust, error: custErr } = await supabase
    ...
    .insert({
      ...
      // T-20260719-foot-PAYMINI-SUGA-SCROLL-BLOCK: is_simulation:true 제거.
      //   simulationFilter(src/lib/simulationFilter.ts) 복원 후, is_simulation=true 고객의 check_in 은
      //   대시보드에서 숨겨져 btn-pay 미노출 → openMiniWindow 타임아웃(단독 실행 시 항상 실패, 배치에선
      //   타 비-시뮬 seed 에 편승해 우연 통과하던 pre-existing harness 결함). 가시 payment_waiting 행이
      //   필요하므로 실고객(is_simulation 미지정=false)으로 seed. afterAll cleanupSeed(phone 기준)로 정리.
...
test('AC-1: 세트코드 드롭다운 리스트에 max-h-48 overflow-y-auto 클래스 포함', async ({ page }) => {
  ...
  // T-20260720-foot-PAYMINI-CHARTCODE-SPLIT: 3열→4열 분리로 접이식 토글 제거 → 세트코드 드롭다운은
  //   ② 차트 코드 칸 하단 footer에 상시 노출(펼침 선행 불필요).
...
// ─────────────────────────────────────────────────────────────────────────────
// AC-3: 수납 스크롤 소유자 = pmw-settle-lane 통합 단일 창
//   ★ T-20260719-foot-PAYMINI-SUGA-SCROLL-BLOCK 로 아키텍처 갱신:
//     구: 하단 action-buttons div 자체가 overflow-y-auto/shrink/min-h-0 로 스크롤 소유 →
//         4ZONE reflow 시 창이 ~131px 로 압축돼 수가항목 추가 후 btn-settle 도달 불가(수납 차단).
//     신: 스크롤 소유권을 settle-lane 통합 창(sm:overflow-y-auto + sm:min-h-[200px])으로 이관,
//         내부 action-buttons div 자체 스크롤 제거. 협소창(131px) 회귀 차단.
//   → 본 AC-3 는 옛 내부창 CSS 단언을 폐기하고, 새 스크롤 소유자(settle-lane)를 단언한다.
// ─────────────────────────────────────────────────────────────────────────────
test('AC-3: 수납 스크롤 소유자 = pmw-settle-lane 통합 창 (구 내부 협소창 폐기)', async ({ page }) => {
  expect(seedOk, '시드 실패 — 수납대기 환자 준비 불가').toBeTruthy();
  await openMiniWindow(page);
  ...
  // btn-settle 조상 체인에서 첫 스크롤 컨테이너(overflowY auto/scroll & scrollHeight>clientHeight)를 찾는다.
  // 신 아키텍처: 그 소유자는 반드시 pmw-settle-lane(통합 창)이어야 한다(옛 내부 버튼 div 아님).
  const probe = await settleBtn.first().evaluate((el) => {
    ...
    while (node) {
      ...
      if ((cs.overflowY === 'auto' || cs.overflowY === 'scroll') && node.scrollHeight > node.clientHeight + 1) {
        return { owner: node.getAttribute('data-testid'), ch: node.clientHeight };
      }
      node = node.parentElement;
    }
    return { owner: null, ch: 0 };
  });
  expect(probe.owner, '스크롤 소유자 = settle-lane 통합 창').toBe('pmw-settle-lane');
  expect(probe.ch, '스크롤 창 ≥180px(131px 협소창 회귀 가드)').toBeGreaterThanOrEqual(180);
});
...
test('AC-4/5: PaymentMiniWindow 기본 렌더 + 세트코드 드롭다운 토글', async ({ page }) => {
  ...
  // T-20260720-foot-PAYMINI-CHARTCODE-SPLIT: 3열→4열 분리 — ② '차트 코드' / ③ '진료비 산정' 독립 칸.
  await expect(page.locator('[data-testid="pmw-chartcode-col"]')).toBeVisible({ timeout: 8000 });
  await expect(page.locator('[data-testid="pmw-settle-lane"]')).toBeVisible();

  // 세트코드 드롭다운은 ② 차트 코드 칸 하단 footer에 상시 노출 → 펼침 선행 불필요.
