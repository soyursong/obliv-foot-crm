...
test.describe('T-20260716 GONGHOM-ADD — DB 계약(순수 ADDITIVE)', () => {
  test('AC-A2-a: customers.visit_route CHECK 가 공홈 허용(신규값)', async () => {
    ...
    const { data, error } = await sb!
      .from('customers')
      .insert({ clinic_id: clinicId, name: TEST_TAG, phone: `+8210${String(Date.now()).slice(-8)}`, visit_type: 'new', visit_route: '공홈' })
  ...
  test('AC-B(DB): 네이버 존치(여전히 CHECK 통과)', async () => {
    ...
    const { data, error } = await sb!
      .from('customers')
      .insert({ clinic_id: clinicId, name: TEST_TAG, phone: `+8210${String(Date.now()).slice(-8)}`, visit_type: 'new', visit_route: '네이버' })
      .select('id, visit_route')
      .single();
