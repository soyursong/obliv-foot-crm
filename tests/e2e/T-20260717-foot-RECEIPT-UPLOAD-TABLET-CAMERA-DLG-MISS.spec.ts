...
test.describe('AC2 — 다이얼로그 오픈 견고화(리마운트/컨텍스트 교란 내성)', () => {
  ...
  test('업로드 성공 직후 sessionStorage에 오픈 의도(pending 스탬프)를 기록', () => {
    expect(SEC).toMatch(/sessionStorage\.setItem\(PENDING_DLG_KEY, String\(Date\.now\(\)\)\)/);
  ...
  test('마운트 시 5분 내 pending 스탬프가 있으면 다이얼로그를 복원(리마운트 대비)', () => {
    ...
    expect(SEC).toMatch(/sessionStorage\.getItem\(PENDING_DLG_KEY\)/);
...
test.describe('AC1/AC4 — PC 회귀 없음 + 매출연동 로직 불변', () => {
  ...
  test('다이얼로그 종료(등록/건너뛰기)는 closeAmountDlg 경유로 pending 스탬프 정리', () => {
    ...
    expect(SEC).toMatch(/sessionStorage\.removeItem\(PENDING_DLG_KEY\)/);
