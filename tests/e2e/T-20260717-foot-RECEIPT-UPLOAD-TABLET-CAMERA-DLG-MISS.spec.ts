...
test.describe('AC2 — 다이얼로그 오픈 견고화(리마운트/컨텍스트 교란 내성)', () => {
  ...
  test('persist 저장소에 오픈 의도(pending 스탬프)를 기록 + 빈 업로드엔 미기록', () => {
    // T-20260720: sessionStorage → pendingDlgStore(localStorage 우선) 전환. persist 규칙 불변.
    // T-20260720-CONSULT-RECEIPT-UPLOAD-TABLET-POPUP: 무거운 외부앱('템플릿으로 업로드') 경로 커버 위해
    //   stamp 기록 시점을 '업로드 착수'로 앞당김. 빈 FileList(취소)는 stamp 이전 early return으로 미기록,
    //   업로드 전량 실패 시 !uploadedOk 분기에서 removeItem 정리(스탬프 잔류 금지) → 규칙 유지.
    expect(SEC).toMatch(/pendingDlgStore\.setItem\(PENDING_DLG_KEY, String\(Date\.now\(\)\)\)/);
    // 빈 FileList는 stamp 이전에 early return.
    expect(SEC).toMatch(/if \(!files \|\| files\.length === 0\)[^\n]*return;/);
    // 전량 실패 분기에서 stamp 정리(removeItem) 존재.
    expect(SEC).toMatch(/if \(!uploadedOk\)/);
