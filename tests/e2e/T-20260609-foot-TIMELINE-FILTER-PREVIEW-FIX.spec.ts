 ...
 *   AC-8 (TICKET-UPDATE r6mr) 필터 칩 레이블 단축('치료/진료/처방') + 유형별 색상(닷과 통일)
 *   AC-9 (TICKET-UPDATE r6mr) 기록 항목 유형 = 원형 컬러 닷 + 고정 컬럼(치료=blue/진료=emerald/처방=amber)
 *   AC-10 (TICKET-UPDATE r6mr, e7cc=B) 특이 필터 칩 제거 + 특이사항 상단 고정 카드 일원화
 *
 * 보존: PROGRESS-TIMELINE-AUTHOR 작성자명(timeline-recorder / timeline-expanded-recorder),
 *        FILTER 3종 칩(치료/진료/처방), 처방 li(timeline-rx-item), 특이사항 상단 섹션(special-note-section).
 *
...
test.describe('T-20260609-TIMELINE-FILTER-PREVIEW-FIX — 필터·미리보기 동작 정정', () => {
  ...
  // ── AC-8: 필터 칩 레이블 단축 + 유형별 색상 ───────────────────────────────
  test('AC-8: 칩 레이블이 "치료/진료/처방" 단축형이고 유형별 색상이 적용된다', async ({ page }) => {
    if (!(await openMedicalChart(page))) {
      test.skip(true, '진료차트 Drawer 미열림 — 스킵');
      return;
    }
    ...
    // 단축 레이블 (긴 '치료메모/진료메모' 아님)
    await expect(treatChip).toHaveText('치료');
    await expect(page.locator('[data-testid="memo-filter-doc"]')).toHaveText('진료');
    await expect(page.locator('[data-testid="memo-filter-rx"]')).toHaveText('처방');
    // 유형별 색상: 활성화 시 치료=blue / 진료=emerald / 처방=amber (닷과 통일)
    await treatChip.click();
    await expect(treatChip).toHaveClass(/bg-blue-600/);
    await treatChip.click(); // OFF
    const docChip = page.locator('[data-testid="memo-filter-doc"]');
    await docChip.click();
    await expect(docChip).toHaveClass(/bg-emerald-600/);
    await docChip.click();
    const rxChip = page.locator('[data-testid="memo-filter-rx"]');
    await rxChip.click();
    await expect(rxChip).toHaveClass(/bg-amber-600/);
    await rxChip.click();
  });

  // ── AC-9: 유형 컬러 닷 + 고정 컬럼 정렬 ───────────────────────────────────
  test('AC-9: 각 항목에 3슬롯 유형 닷이 있고 유형별 색상이 고정 컬럼을 점유한다', async ({ page }) => {
    if (!(await openMedicalChart(page))) {
      test.skip(true, '진료차트 Drawer 미열림 — 스킵');
      return;
    }
    ...
    const dots = entry.locator('[data-testid="timeline-type-dots"]');
    await expect(dots).toHaveCount(1);
    // 3개 슬롯(치료/진료/처방) — 부재 유형도 transparent 닷으로 컬럼 유지
    await expect(dots.locator('[data-type="treat"]')).toHaveCount(1);
    await expect(dots.locator('[data-type="doc"]')).toHaveCount(1);
    await expect(dots.locator('[data-type="rx"]')).toHaveCount(1);
  });

  // ── AC-10: 특이사항 필터 칩 제거 + 상단 고정 카드 상시 노출 ──────────────────
  test('AC-10: 특이 필터 칩이 제거되고 특이사항 섹션이 상단에 상시 존재한다', async ({ page }) => {
    if (!(await openMedicalChart(page))) {
      test.skip(true, '진료차트 Drawer 미열림 — 스킵');
      return;
    }
    // 특이 칩 제거 (필터 행은 치료/진료/처방 3종만)
    await expect(page.locator('[data-testid="memo-filter-notable"]')).toHaveCount(0);
    // 특이사항 상단 고정 섹션은 필터와 무관하게 상시 존재
    const special = page.locator('[data-testid="special-note-section"]');
    if ((await special.count()) === 0) {
      test.skip(true, '특이사항 섹션 미렌더(데이터/뷰) — 스킵');
      return;
    }
    await expect(special).toBeVisible();
    // 필터를 토글해도 특이사항 섹션은 그대로 보존
    const treatChip = page.locator('[data-testid="memo-filter-treat"]');
    if ((await treatChip.count()) > 0) {
      await treatChip.click();
      await page.waitForTimeout(150);
  ...
  // ── 보존: FILTER 3종 칩(치료/진료/처방) 존재 ──────────────────────────────
  test('보존: 글로벌 메모 필터 칩 3종(치료/진료/처방)이 존재한다', async ({ page }) => {
    if (!(await openMedicalChart(page))) {
      test.skip(true, '진료차트 Drawer 미열림 — 스킵');
      return;
    }
