  ...
  test('AC-1/AC-3: getAssignedSlotName — 치료대기는 treatment_room 잔존값이 있어도 null(뱃지 미노출), 입실 단계는 방 유지', async ({ page }) => {
    ...
    const result = await page.evaluate(() => {
      ...
      const getAssignedSlotName = (ci: Record<string, string | null>) => {
        switch (ci.status) {
          ...
          case 'treatment_waiting':
            // 대기 단계 = 방 미입실 → 잔존값 무시
            return null;
          // T-20260611-foot-CALLLIST-ROOM-LABEL AC-3: preconditioning(치료실 입실)은
          // treatment_room을 읽는다(치료실 방번호 write 위치). laser 그룹과 분리.
      ...
      return {
        ...
        // 회귀: 실제 입실 단계는 방 이름 그대로 표출 (preconditioning은 treatment_room — ROOM-LABEL AC-3)
        preconditioningRoom: getAssignedSlotName({ status: 'preconditioning', treatment_room: '치료실A' }),
