...
test('AC-1: 2패널 grid(좌 폴더관리 / 우 상병항목) 렌더', () => {
  ...
  expect(src).toContain('md:grid-cols-[240px_minmax(0,1fr)]'); // 좌 고정폭 + 우 가변
...
// ── AC-4: 미분류 버킷 ──
test('AC-4: "미분류" 버킷 — 항상 표시 + drop 가능(폴더 배정 해제)', () => {
  const src = read(TAB);
  expect(src).toContain('UnassignedBucket');
  expect(src).toContain("UNASSIGNED = '__unassigned__'");
  expect(src).toContain("NO_FOLDER = '미분류'");
  // 미분류 = folder_id NULL 항목 집계
...
// ── 시나리오 3: 엣지 — 미분류 + 권한 ──
test('시나리오3: 미분류 버킷 노출 + 비관리 role read-only', () => {
  const src = read(TAB);
  expect(src).toContain('UnassignedBucket');
  // 읽기 전용 안내
