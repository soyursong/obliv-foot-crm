...
test.describe('시나리오2 — UX 표시(AC-2~8)', () => {
  ...
  test('AC-2 헤더 "시간"(경과시간/콜경과시간 잔존 0)', () => {
    const s = DASH();
  ...
  // MONOTONE-RELAYOUT AC-3 supersede: 완료 시간 칼럼 제거(UX7 유지) + 차트 칼럼 신설 → 호출 9 / 완료 8.
  test('AC-7 진료 완료 섹션 시간 칼럼 제거 + 차트 신설(호출 9 / 완료 8)', () => {
    const s = DASH();
    // 완료 셀 testid 소멸(시간)
    expect(s).not.toContain('data-testid="doctor-completed-elapsed-cell"');
    // 완료 thead 8칼럼(시간 제거 + 차트 신설), 임상경과=처방 왼쪽.
    ...
    expect(compOrder).toEqual(['이름', '차트번호', '상태', '방', '오늘시술', '임상경과', '처방', '차트']);
    // 호출 thead 는 시간 유지(9칼럼)
    const callThead = s.slice(s.indexOf('doctor-call-feed-table'), s.indexOf('doctor-call-feed-rows'));
    expect((callThead.match(/<th /g) ?? []).length).toBe(9);
    expect(callThead).toContain('>시간</th>');
    // colspan 분리(호출 9 / 완료 8) — MONOTONE-RELAYOUT: 완료 인라인 차트 1행
...
test.describe('시나리오3 — AC-0 회귀 rebase', () => {
  ...
  // MONOTONE-RELAYOUT AC-4 supersede: HandRaiseFlow → HandToggle(✋ 단색 3-상태). pink 전이 1곳 보존(신설 write 0).
