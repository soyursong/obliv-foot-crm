...
test.describe('AC-2 — 이름 옆 이모지 인라인 버튼', () => {
  test('테두리형 이모지 버튼 스타일 토큰(NAME_EMOJI_BTN) 정의 + border', () => {
    const s = DASH();
    expect(s).toContain('NAME_EMOJI_BTN');
    const def = s.slice(s.indexOf('const NAME_EMOJI_BTN'));
...
test.describe('AC-3 — 임상경과 미리보기 유지 + 진료차트 칼럼 제거', () => {
  test('colspan 진료차트 칼럼 제거 반영(8 / 7)', () => {
    const s = DASH();
...
test.describe('item6 — 처방/임상경과 폭 확대', () => {
  test('CallFeed 콜그룹: 처방 20% · 임상경과 22%', () => {
    const s = DASH();
    expect(s).toContain('<col className="w-[20%]" />');
    expect(s).toContain('<col className="w-[22%]" />');
  });

  test('Completed 콜그룹: 처방 24% · 임상경과 23%', () => {
    const s = DASH();
    expect(s).toContain('<col className="w-[24%]" />');
    expect(s).toContain('<col className="w-[23%]" />');
  });
});
...
test.describe('item7 — 처방 드롭다운 폭 축소', () => {
  test('expand 행이 풀폭 단일 div가 아니라 우측 anchor + max-w-xs 로 축소', () => {
    const s = DASH();
    // 우측 정렬 wrapper + 폭 제한
    expect(s).toContain('max-w-xs');
    expect(s).toMatch(/mr-\[2[23]%\]/);
  });
});
