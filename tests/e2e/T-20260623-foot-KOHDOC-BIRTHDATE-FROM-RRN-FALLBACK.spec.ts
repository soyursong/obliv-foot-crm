...
// T-20260630-foot-KOHEXAM-ISSUE-RELOCATE-TXTABLE: 균검사 발급 동선이 치료테이블(ExamTargetsSection)로 이전.
//   생년월일 RRN 파생 폴백(effectiveBirth) → buildKohFieldData 발급 결선은 이제 이 파일에 있다.
//   KohReportTab 은 read-only(생년 표기용 effectiveBirth 만 잔존) → 발급 결선 가드는 ExamTargetsSection 대상.
const ISSUE_SRC = resolve(__dirname, '../../src/components/treatment/ExamTargetsSection.tsx');
...
test('AC④ PHI — 파생 입력은 birth_date_display 만(평문 RRN 미노출) + 결선 가드', () => {
  // KohReportTab(read-only 생년 표기) + ExamTargetsSection(발급 결선) 둘 다 PHI: display 만 수신.
  const koh = readFileSync(SRC, 'utf8');
  const issue = readFileSync(ISSUE_SRC, 'utf8');
  // RPC 호출은 fn_customer_birthdates, 수신은 birth_date_display 만(양쪽).
  expect(koh).toContain("supabase.rpc('fn_customer_birthdates'");
  expect(koh).toContain('birth_date_display');
  expect(issue).toContain("supabase.rpc('fn_customer_birthdates'");
  expect(issue).toContain('birth_date_display');
  // 평문 RRN 컬럼(rrn / resident_registration_number)을 select/수신하지 않음(PHI 미노출).
  expect(koh).not.toMatch(/birthMap[\s\S]{0,200}\brrn\b/);
  expect(issue).not.toMatch(/birthMap[\s\S]{0,200}\brrn\b/);
  // KohReportTab: 생년 표기용 effectiveBirth 잔존(read-only).
  expect(koh).toContain('const effectiveBirth');
  // 결선 가드(RELOCATE): effectiveBirth 가 발급 게이트/buildKohFieldData(call)에 연결 — 이제 치료테이블에 존재.
  expect(issue).toContain('const effectiveBirth');
  expect(issue).toContain('!!effectiveBirth(r)'); // 발급 게이트(data-publishable)
  expect(issue).toContain('buildKohFieldData(kohRow, doctorName, effectiveBirth(r))'); // 발급 fieldData
