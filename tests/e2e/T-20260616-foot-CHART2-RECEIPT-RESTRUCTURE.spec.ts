...
test.describe('T-20260616-foot-CHART2-RECEIPT-RESTRUCTURE — 영수증·수납내역 표시 재구성', () => {
  ...
  test('S2-a: 수납내역 일반결제는 영수증 업로드분 제외(진료비 수납내역만)', () => {
    ...
    // T-20260715-foot-CHART-SUSU-EXPPAY-INCLUDE: 체험(회수1·단건) 영수증결제는 예외로 포함되도록
    //   필터가 함수형으로 확장됨 — 일반 '영수증 업로드' 제외(진료비 수납내역만) 성질은 유지.
    expect(s).toContain('const feePayments = payments.filter((p) => {');
    expect(s).toContain("return !memo.startsWith('영수증 업로드');");
