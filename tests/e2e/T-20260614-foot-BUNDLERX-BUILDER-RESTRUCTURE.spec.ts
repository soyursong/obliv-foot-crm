...
test('S1-1: 빌더 ItemRow 에 용량·횟수·일수 입력칸 존재 (1/3/2 baked default)', () => {
  ...
  // T-20260616-foot-RX-COLUMN-INPUT-UNIFY-ALLSCREENS(deployed): 칼럼 라벨을 RX_COL SSOT(@/lib/rxFormat)로
  //   통일 → 하드코딩 '>횟수</Label>'/'>일수</Label>' 리터럴은 {RX_COL.count}/{RX_COL.days} 템플릿으로 이관.
  //   런타임 라벨 텍스트는 불변(RX_COL.count==='횟수', RX_COL.days==='일수'). 입력칸·RxCountInput 존속.
  expect(src).toContain("import { RX_COL } from '@/lib/rxFormat'");
  expect(src).toContain('{RX_COL.count}</Label>'); // 횟수 라벨 (RxCountInput, RX_COL SSOT)
  expect(src).toContain('{RX_COL.days}</Label>');  // 일수 라벨 (RX_COL SSOT)
...
test('S3-3: 설명(notes) 입력칸은 약 항목 행에서 제거·notes 데이터는 보존(DROP 0)', () => {
  const src = read(RXSET);
  // T-20260625-foot-BUNDLERX-DRUGROW-MEMO-REMOVE(deployed, AC-1): 묶음처방 약 항목 행 = 약이름+숫자3종만.
  //   설명(notes) 입력칸(rx-set-item-notes-input)·'>설명</Label>' 제거 — 메모는 DrugFoldersTab '설명' 인라인으로 이관.
  //   (AC-2) items[].notes 필드는 보존(DROP 0): onChange 미경유로 기존 저장값 유실 없이 그대로 upsert.
  expect(src).not.toContain('rx-set-item-notes-input'); // 입력칸 제거
  expect(src).not.toContain('>설명</Label>');            // 설명 라벨 제거
  expect(src).toMatch(/notes:\s*string/);               // notes 필드 타입 보존(DROP 0)
});
