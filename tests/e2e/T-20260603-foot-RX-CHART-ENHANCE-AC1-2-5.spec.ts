...
test.describe('T-20260603 RX-CHART-ENHANCE — 폴더링(AC-1) · 금기게이트(AC-2) · 마스터검색(AC-5)', () => {
  ...
  // ── #2 / AC-2 FIX(MSG-20260603-190947): 금기증 조회 실패 시 우회불가(자동 적재 금지) ──
  test('AC-2: 금기증 조회 실패 시 처방 차단 → 오류 게이트, 사용자 확인 없이는 미적재', async ({ page }) => {
    await openChartRxTab(page, seed!.customerId);

    // 금기증 조회 REST 요청을 500으로 가로채 조회 실패 상황 강제
    await page.route('**/rest/v1/prescription_contraindications**', (route) =>
      route.fulfill({ status: 500, contentType: 'application/json', body: '{"message":"injected failure"}' }),
    );

    await page.getByTestId('rx-set-option').filter({ hasText: SET_GATE }).first().click();

    // 우회불가: 일반 게이트가 아닌 "조회 실패" 오류 게이트가 떠야 함
    const errGate = page.getByTestId('rx-contra-gate-error');
    await expect(errGate).toBeVisible({ timeout: 8_000 });
    // 사용자 확인(재시도/override) 전 — 처방내역 미적재(commit 미호출)
    await expect(page.getByTestId('prescription-items-table')).toHaveCount(0);

    // 취소 → 여전히 미적재
    await page.getByTestId('rx-contra-error-cancel').click();
    await expect(errGate).toBeHidden({ timeout: 5_000 });
    await expect(page.getByTestId('prescription-items-table')).toHaveCount(0);
  });

  test('AC-2: 조회 실패 게이트 — 명시적 override(관리자 확인) 클릭 시에만 적재', async ({ page }) => {
    await openChartRxTab(page, seed!.customerId);
    await page.route('**/rest/v1/prescription_contraindications**', (route) =>
      route.fulfill({ status: 500, contentType: 'application/json', body: '{"message":"injected failure"}' }),
    );
    ...
    const errGate = page.getByTestId('rx-contra-gate-error');
    await expect(errGate).toBeVisible({ timeout: 8_000 });
    // override 클릭 전 미적재 보장
    await expect(page.getByTestId('prescription-items-table')).toHaveCount(0);

    // 명시적 override → 적재
    await page.getByTestId('rx-contra-error-override').click();
    await expect(errGate).toBeHidden({ timeout: 5_000 });
    await expect(page.getByTestId('prescription-items-table')).toContainText(NAME_CONTRA);
  });
