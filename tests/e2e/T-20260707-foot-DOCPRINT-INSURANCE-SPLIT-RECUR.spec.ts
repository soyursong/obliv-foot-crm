...
test.describe('T-20260707 null-grade fallback: 본인=급여전액 / 공단=0 (총괄 확정)', () => {
  ...
  test('AC-8(2) 회귀가드: grade 실재(general) 방문은 기존 split 유지(본인 4,100 / 공단 9,270)', () => {
    const fb = computeFootBilling(MIXED_VISIT, 'general');
    expect(fb.copaymentTotal).toBe(4100);                          // ceil(13370*0.3/100)*100
    expect(fb.liveBillingValues.insuranceCovered).toBe(9270);      // 공단 정상 분리(회귀 없음)
  });

  test('AC-8 Path A 회귀가드: grade 실재는 100원 절상 기존 산식 유지', () => {
    const billItems = [
      { amount: 13370, count: 1, days: 1, is_insurance_covered: true },
    ] as Array<{ amount: number; count?: number; days?: number; is_insurance_covered: boolean; copayment_amount?: number }>;
    fillBillItemCopayment(billItems, 'general');
    expect(billItems[0].copayment_amount).toBe(4100);              // 본인 4,100 (회귀 없음)
  });
