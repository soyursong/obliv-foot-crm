...
// FIX-REQUEST(MSG-20260702-164644) 정정 스코프: AC-1(직원 근무 캘린더 회색톤)만.
// 사이드바 근무 캘린더 실체 = CalendarNoticePanel.tsx. 체크완료 표시(☑)가 emerald-600(무채색
// 테마 램프에서 chromatic 잔존)로 남아 있던 것을 무채색 gray-700 로 치환 → warm/chromatic 장식톤 0.
const CALPANEL = resolve(__dirname, '../../src/components/CalendarNoticePanel.tsx');
const calPanel = readFileSync(CALPANEL, 'utf8');
...
test.describe('T-20260702-foot-SIDEBAR-STAFFCAL-GRAY-DUTYWIDTH — Handover 직원 근무 캘린더 브라운→무채색', () => {
  ...
  // ── AC1-확장(FIELD-SOAK FAIL 3차): 근무표 페이지 전체 warm 잔존 0 ──
  //   직원 근무 캘린더 섹션 뿐 아니라 같은 페이지 상단 '의사 근무표' 헤더 아이콘(구 text-teal-600
  //   =THEME-MONOCHROME 램프에서 Umber 브라운으로 렌더)까지 무채색화. className 색 토큰 전수 스캔.
  test('AC1+: Handover 전체 warm 팔레트 className 0건 (의사 근무표 헤더 아이콘 포함)', () => {
    const warmClass = full.match(/\b(?:teal|amber|orange|stone|brown)-\d{2,3}\b|\byellow-\d{2,3}\b/g) ?? [];
    // 주석 문자열 내 참조(text-teal-600 설명)는 className 이 아니므로 className= 컨텍스트만 검사.
    const warmInClassName = (full.match(/className[^>]*?\b(?:teal|amber|orange|stone|brown)-\d{2,3}\b/g) ?? []);
    expect(warmInClassName, `warm className 잔존: ${warmInClassName.join(', ')}`).toHaveLength(0);
    expect(full).toMatch(/의사 근무표/); // 헤더 마커 sanity
  });
});

test.describe('T-20260702-foot-SIDEBAR-STAFFCAL-GRAY-DUTYWIDTH — 사이드바 CalendarNoticePanel 체크완료 chromatic→무채색', () => {
  // ── AC1(사이드바 근무 캘린더): 체크완료 emerald(chromatic) 잔존 0, gray 치환 ──
  test('AC1: CalendarNoticePanel 체크완료 표시 emerald/chromatic 장식톤 0건', () => {
    expect(calPanel).not.toMatch(/text-emerald-\d/);
    expect(calPanel).not.toMatch(/\bteal-\d/);
    expect(calPanel).not.toMatch(/\bamber-\d/);
    expect(calPanel).not.toMatch(/\bstone-\d/);
    // 체크완료 표시가 무채색 gray 로 치환됨(is_checked 삼항).
    expect(calPanel).toMatch(/is_checked \? 'text-gray-700' : 'text-muted-foreground'/);
  });

  // ── AC2: 의미색(주말 red/blue·삭제 red-600) 보존 — 캘린더 표준 시맨틱 ──
  test('AC2: CalendarNoticePanel 의미색(주말·삭제) 보존', () => {
    expect(calPanel).toMatch(/text-red-500/);   // 일요일
    expect(calPanel).toMatch(/text-blue-500/);  // 토요일
  });
});
