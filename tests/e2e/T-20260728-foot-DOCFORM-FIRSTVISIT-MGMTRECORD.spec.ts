...
// ── AC-2: 9개 섹션 + 서명영역 렌더 ──────────────────────────────────────────
test('AC-2: HTML 템플릿에 9개 섹션 + 서명영역 마커 존재', () => {
  const html = getHtmlTemplate(NEW_KEY)!;
  // 제목
  expect(html).toContain('초 진 관 리 기 록 지');
  // ①고객정보 ②초진일 ③방문목적 ④증상경위 ⑤상태확인 ⑥촬영 ⑦초기관리 ⑧관리계획 ⑨특이사항
  for (const label of [
    '성', '생년월일', '연 락 처', '초 진 일',
    '방문 목적', '증상 발생 경위',
    '관리 부위', '발톱 상태', '피부 상태', '통증 여부', '보행 불편', '기타 확인 사항',
    '촬영 기록', '초기 관리 내용', '관리 계획', '특이사항',
    '발 급 일', '센 터 명', '담 당 자', '직',
  ]) {
    expect(html).toContain(label);
  }
  // 방문목적 5종 + 발가락 5개 + 좌우 체크 마커
  for (const cb of [
    '내성발톱', '무좀발톱', '두꺼운 발톱', '변형발톱',
    '엄지', '둘째', '셋째', '넷째', '다섯째', '좌(L)', '우(R)',
    '사진 촬영 완료', '촬영하지 않음',
    '발톱 정리', '문제성 발톱 관리', '압력 감소 조치', '보호패드 적용',
  ]) {
