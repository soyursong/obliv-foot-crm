...
// ── 시나리오 2 (AC2): 시간 선택 write — field clarify 확정(ts=1781796980.363939) 구현 완료
//   Q1=30분 단위(슬롯집계 재사용) / Q2=마감표시 불필요(숫자만) / Q3=저장 O(reservation_time update).
test.describe('AC2 시간 선택(write) — Q3=저장 확정 구현', () => {
  test('패널: onSelectTime 콜백 prop 으로 시간 선택을 부모에 버블업(패널 자체 write 0)', () => {
    expect(PANEL).toContain('onSelectTime');
    expect(PANEL).toContain('selectedTime');
    // 선택 모드 = onSelectTime 제공 시에만 클릭 가능(button) — read-only 기본 동작 보존
    expect(PANEL).toContain('typeof onSelectTime');
    // 패널은 여전히 DB write 0 — 저장은 부모 책임(쓰기 격리)
    expect(PANEL).not.toContain('.insert(');
    expect(PANEL).not.toContain('.update(');
    expect(PANEL).not.toContain('.delete(');
  });

  test('팝업: 시간대 패널에 selectedTime/onSelectTime wire-in (anchor 예약 reschedule 대상)', () => {
    expect(DETAIL_POPUP).toContain('onSelectTime={loadedMatch ? undefined : setSelectedSlotTime}');
    expect(DETAIL_POPUP).toContain('selectedTime={loadedMatch ? null : selectedSlotTime}');
    // 저장 버튼 + 선택 바
    expect(DETAIL_POPUP).toContain('data-testid="btn-reschedule-time"');
    expect(DETAIL_POPUP).toContain('rescheduleToSelectedTime');
  });

  test('팝업: reschedule = reservation_time update 경로 재사용 + reservation_logs(action=reschedule)', () => {
    expect(DETAIL_POPUP).toContain('reservation_date: newDate, reservation_time: newTime');
    expect(DETAIL_POPUP).toContain(".eq('id', reservation.id)");
    expect(DETAIL_POPUP).toContain("action: 'reschedule'");
  });

  test('GUARD(RESV-ROUTE-AUTOCLASS): reschedule update payload 에 visit_type/visit_route write 없음', () => {
    // rescheduleToSelectedTime 본문 추출 후 update 페이로드에 방문경로 컬럼 write 미포함 확인
    const fnStart = DETAIL_POPUP.indexOf('const rescheduleToSelectedTime');
    const fnEnd = DETAIL_POPUP.indexOf('// ── 액션: 체크인 전환', fnStart);
    expect(fnStart).toBeGreaterThan(-1);
    expect(fnEnd).toBeGreaterThan(fnStart);
    const body = DETAIL_POPUP.slice(fnStart, fnEnd);
    expect(body).toContain('.update({ reservation_date: newDate, reservation_time: newTime })');
    expect(body).not.toContain('visit_type:');
    expect(body).not.toContain('visit_route:');
  });

  test('Q2: 마감 표시(최대인원 기준 판정) 로직 미구현 — 패널은 카운트만 표기', () => {
    // 마감 판정 로직(최대인원 상수/임계 비교)이 패널에 없음(Q2=불필요). '마감' 단어는 주석 설명에만 등장.
    expect(PANEL).not.toContain('maxForSlot');
    expect(PANEL).not.toContain('/12');
    // 마감 차단 라벨(toast/표기)을 거는 비교 로직 없음
    expect(PANEL).not.toMatch(/total\s*>=?\s*\d+/);
    expect(PANEL).not.toContain('마감입니다');
  });

  test('AC2: 날짜 변경 시 stale 시간 선택 초기화(다른 날짜 슬롯 오저장 방지)', () => {
    // pickedDate 변경 effect 가 selectedSlotTime 을 null 로 리셋
    expect(DETAIL_POPUP).toContain('setSelectedSlotTime(null)');
    expect(DETAIL_POPUP).toContain('}, [pickedDate]);');
  });
});
