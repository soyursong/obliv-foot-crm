...
async function goToFeeSetTab(page: import('@playwright/test').Page) {
  // T-20260613-foot-PHRASEMGMT-SUBTAB-SPLIT: 수가세트는 '서비스관리 > 상용구관리' 서브탭으로 이전됨.
  //   딥링크 ?tab=fee_set_templates 로 진입(서비스관리는 직원 포함 접근).
...
test.describe('T-20260525-foot-FEE-SET-TEMPLATE — AC-4: 진료도구 탭 구성 확인', () => {
  ...
  test('AC-4a: 수가세트 탭 존재 + 클릭 가능 (서비스관리>상용구관리 서브탭)', async ({ page }) => {
    // T-20260613-foot-PHRASEMGMT-SUBTAB-SPLIT(67ab9ad): 수가세트는 진료관리에서 '서비스관리 > 상용구관리' 서브탭으로 이전됨.
    //   helper(goToFeeSetTab)와 동일 딥링크로 정합 — 잔존 /admin/clinic-management 제거.
    await page.goto('/admin/services?tab=fee_set_templates');
