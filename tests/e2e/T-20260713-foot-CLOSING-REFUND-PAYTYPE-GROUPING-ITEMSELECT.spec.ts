 ...
 */
/**
 * ⚙ unit 프로젝트 편입 (auth.setup·webServer 불요, 결정론):
 *   본 spec 은 순수 로직(selectedSum/isFullyRefunded) + 정적 소스 가드(Closing.tsx introspection)만으로
 *   AC-1~6 · AC-B1~B3 를 결정론적으로 검증한다. 로그인/서버 의존 real-browser 렌더는 제거하고
 *   supervisor 갤탭 field-soak 로 이관 — QA 워크트리(TEST_PASSWORD 부재)에서 desktop-chrome 만
 *   매칭돼 auth.setup 이 기동/실패하며 "No tests found"/env_missing 를 유발하던 것을 차단.
 *   (playwright.config.ts unit.testMatch 등록 + desktop-chrome.testIgnore 제외. 선례: DOCFEE-BODYCENTER-REDESIGN,
 *    DOCPRINT-GONGDAN-HIDE-COPAY-ONLY, FOREIGN-LANG-SAVE, DAILY-CLOSINGS-READ-OVEROPEN.)
 *   → FIX-REQUEST MSG-20260717-141523-ydgt (qa_fail phase2 / spec_missing) 대응.
 */
...
// ──────────────────────────────────────────────────────────────────────────────
// 실브라우저(로그인 필요) 환불창 렌더 검증은 supervisor 갤탭 field-soak 로 이관.
//   AC-1~6 · AC-B1~B3 배선은 위 정적 소스 가드가 결정론적으로 커버(refund-item-list/
//   refund-selected-sum/refund-open-btn/refund-submit 등 testid 는 Closing.tsx 에 상존).
//   실기기 태블릿 렌더·클릭 동선 confirm 은 현장(김주연 총괄) soak-validation 항목.
// ──────────────────────────────────────────────────────────────────────────────
