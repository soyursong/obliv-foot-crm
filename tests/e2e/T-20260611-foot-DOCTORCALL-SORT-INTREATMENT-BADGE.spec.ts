...
test.describe('T-20260611 DOCTORCALL-SORT-INTREATMENT-BADGE WS-1 — 진료콜 진입 시각 정렬', () => {
  ...
  test('AC-3 소스 정적 가드: activeList 정렬이 진입시각(compareCallOrder→callEntryTime) 사용 (checked_in_at 직접 비교 아님)', () => {
    const src = fs.readFileSync(COMPONENT_SRC, 'utf-8');
    // callEntryTime 헬퍼 export 존재
    expect(src).toMatch(/export function callEntryTime\(/);
    // T-20260615 WS-C 이후 정본: activeList는 단일 통합 비교자 compareCallOrder로 정렬(tier3=진입순).
    //   진입시각 비교(callEntryTime localeCompare)는 compareCallOrder 내부로 이전됨 → 거기서 락한다.
    ...
    // compareCallOrder tier3가 여전히 callEntryTime 진입순 비교 (정본 회귀 락)
    const cmpBlock = src.slice(src.indexOf('export function compareCallOrder'), src.indexOf('function loadRowHidden'));
    expect(cmpBlock).toContain('callEntryTime(a).localeCompare(callEntryTime(b))');
