...
test('AC-3: 기존 고객 멱등(update) 경로 불변 + consent_marketing 미참조', () => {
  ...
  // update 블록에 consent_marketing 잔존 없음 (본 티켓 rollback 불변식)
  expect(updBlock).not.toContain('consent_marketing');
  // 멱등 핵심 불변식 유지 (이름/광고추적 필드 등 다른 선택필드 반영은 그대로).
  //   T-20260713-foot-INGEST-NAME-OVERWRITE-GUARD(DA-20260713, verdict=GO) SUPERSEDE:
  //   무조건 `name,` 반영 → create-only/preserve-on-NULL 가드(`...(shouldFillName ? { name: pushName } : {})`)로 교체.
  //   update 블록의 name 참조 자체는 존속(멱등 회귀 0) → 하드값 `name,`만 stale, 가드형(name: pushName)으로 갱신.
  expect(updBlock).toContain('name: pushName');
