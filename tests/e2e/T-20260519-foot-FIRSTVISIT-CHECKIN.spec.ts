...
test('AC-1: box1-resv-card — "접수" 버튼 존재 확인 (DraggableBox2ResvCard 동일 패턴)', async ({ page }) => {
...
test('AC-1: box1-resv-card 접수 버튼 — 비활성화 상태 아님 (클릭 가능)', async ({ page }) => {
  await page.goto(`${BASE_URL}/admin/dashboard`);
  await expect(page.getByText('통합 시간표')).toBeVisible({ timeout: 15000 });
...
test('AC-3: DraggableBox1Card — DraggableBox2ResvCard와 동일 이벤트 분리 패턴 적용', async ({ page }) => {
  await page.goto(`${BASE_URL}/admin/dashboard`);
  await expect(page.getByText('통합 시간표')).toBeVisible({ timeout: 15000 });
...
test('AC-4: box2-resv-card — FIRSTVISIT-CHECKIN 적용 후 기존 동작 무영향', async ({ page }) => {
  await page.goto(`${BASE_URL}/admin/dashboard`);
  await expect(page.getByText('통합 시간표')).toBeVisible({ timeout: 15000 });
