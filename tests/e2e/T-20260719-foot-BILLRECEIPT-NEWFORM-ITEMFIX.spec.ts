...
test.describe('BILLRECEIPT 신양식 ITEMFIX — 항목분해(②) / 사전납부(③) / 레이아웃(④)', () => {
  ...
  test('②-static: 처치 및 수술료 행 비급여칸={{proc_noncov}}, 검사료 행 비급여칸={{exam_noncov}}', () => {
    ...
    // ⚠ T-20260722-foot-BILLRECEIPT-NEWFORM-CATSPLIT-PAIDBOX(결함A)로 급여칸 공란 전제 SUPERSEDED:
    //   급여 검사/처치가 진찰료로 흡수되지 않도록 처치/검사 행 급여칸에 {{proc_copay}}/{{exam_copay}} 등
    //   별도표기(별지6호). 비급여 마지막 셀({{proc_noncov}}/{{exam_noncov}})은 ITEMFIX 그대로 유지.
    expect(tpl).toMatch(
    ...
    // 검사료 행: 급여칸 {{exam_copay}}/{{exam_ins}} + 마지막 비급여셀 = {{exam_noncov}}
    expect(tpl).toMatch(
  ...
  // ── AC-③ 납부금액 사전 입력 ──
  test('③-static: ⑪ 합계={{paid_total}}, 납부하지 않은 금액(⑩-⑪)={{unpaid_amount}}', () => {
    const tpl = extractNewTemplate();
    // ⚠ T-20260722-foot-BILLRECEIPT-NEWFORM-CATSPLIT-PAIDBOX(결함B)로 SUPERSEDED:
    //   ⑪ 합계 = FE 수기 {{prepaid_amount}}(납부할금액 가정=허위영수증 위험) → payments 원장 실수납
    //   Σ(method별) = {{paid_total}} 로 교체. 카드/현금/현금영수증 method별 셀 별도 배선.
  ...
  test('회귀: 진찰료 행 급여 split — {{consult_copay}}/{{consult_ins}}(aggregate remainder) 바인딩', () => {
    const tpl = extractNewTemplate();
    // ⚠ T-20260722-foot-BILLRECEIPT-NEWFORM-CATSPLIT-PAIDBOX(결함A)로 SUPERSEDED:
    //   진찰료 행 = aggregate 직결({{copayment}}/{{insurance_covered}})이면 급여 검사가 흡수됨 →
    //   진찰료 행 = 최종 aggregate '잔여'(검사료/처치 급여분 차감) = 표시 전용 신 토큰
    //   {{consult_copay}}/{{consult_ins}}. 불변식 Σ(행별)=={{copayment}}/{{insurance_covered}} 는
    //   applyBillReceiptNewCoveredTokens 순수 산식 테스트(CATSPLIT-PAIDBOX spec A-invariant)로 강제.
    expect(tpl).toMatch(
      /<td>진찰료<\/td><td class="rn-num">\{\{consult_copay\}\}<\/td><td class="rn-num">\{\{consult_ins\}\}<\/td>/,
    );
    // aggregate 키는 합계 ①②·⑦ 에만 잔존(진찰료 행 직결 회귀 금지).
