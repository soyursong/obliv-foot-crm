...
// ─── AC-4: READ-only 회귀가드 — G2 read 마이그는 쓰기 3정책을 접촉하지 않는다(존재 보존) ───
// ⚠ 쓰기 3정책의 술어(staff vs canonical)는 별도 write 트랙
//    T-20260611-foot-CLINIC-EVENTS-WRITE-RLS-CANONICAL(20260611190000) 의 spec 가 소유.
//    같은 배치 적용 시 write 가 canonical 로 바뀌므로 여기서는 술어 단정 금지 — 존재만 가드한다.
test('AC-4: clinic_events INSERT/UPDATE/DELETE 3정책이 존재하며 G2 read 마이그가 쓰기 권한을 제거하지 않는다', async ({ request }) => {
  test.skip(!process.env.SUPABASE_ACCESS_TOKEN, 'SUPABASE_ACCESS_TOKEN not set');
  const rows = await dbQuery(request, `
