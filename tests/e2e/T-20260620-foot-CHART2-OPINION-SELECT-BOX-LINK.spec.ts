...
test.describe('T-20260620-foot-CHART2-OPINION-SELECT-BOX-LINK — 소견서 선택박스 연동', () => {
  ...
  test('AC-1/AC-8: 상담내역 인라인 소견서 선택박스 렌더', () => {
    ...
    // 팝업으로 띄우지 않음 — 선택박스 자체는 인라인(DialogTrigger 로 열리는 팝업 아님).
    //   T-20260717-foot-CHART2-CANCELREQ-ADD-FROM-REQLIST: '취소 요청' 확인 다이얼로그(Dialog)가 추가됐으나
    //   이는 취소 확인용이며 선택박스를 팝업으로 띄우는 트리거가 아님 → DialogTrigger 부재로 인라인 불변식 보장.
    expect(box).not.toContain('DialogTrigger');
  ...
  // AC-4 (BLOCKING): authoring 경계 — 실장 박스에 발행 행위 없음 (구조 검증)
  //   ★COPY-PURGE(deployed a6258097, 김주연 총괄 요청) 이후 '발행은 원장' 안내 카피는 정당히 제거됨.
  //     경계는 특정 안내 문자열이 아니라 '실장 surface 에 발행 경로가 구조적으로 부재'함으로 보장된다 →
  //     카피 문자열 의존을 제거하고, 발행 경로(발행 RPC·발행 본문 작성창·발행/확정 뮤테이션·발행 버튼)의
  //     미노출을 구조로 검증(문자열 의존 → 구조 견고화). 카피 복원 아님(기본 제거 유지).
  test('AC-4: 실장 박스는 요청만 — 발행(publish) 경로 구조적 부재', () => {
    const box = reqBox();
    // (1) 발행 RPC 직접 호출 금지
    expect(box).not.toContain('publish_opinion_doc');
    // (2) 발행 본문 작성창(원장 전용)은 실장 박스에 미마운트 — 참조/렌더 없음
    expect(box).not.toContain('OpinionEditorDialog');
    // (3) 실장 박스의 쓰기 경로 = 요청 draft 생성 + 자기 요청 '취소 요청'(회수)뿐. 발행/확정 뮤테이션 미노출.
    expect(box).toContain('useCreateOpinionRequest');
    //   발행/확정 뮤테이션 금지. (useResolveOpinionRequest 는 T-20260717-CANCELREQ-ADD-FROM-REQLIST 로
    //   '취소 요청'(reason:'cancelled', 자기 draft 회수)에 한해 허용 — 발행이 아님. 발행 경로만 금지.)
    expect(box).not.toMatch(/usePublishOpinion|usePublish\b/);
    // 실장 박스의 resolve 사용은 취소(cancelled) 전용 — 발행(published) resolve 금지(authoring 경계 유지).
    expect(box).not.toContain("reason: 'published'");
    // (4) 제출 액션은 '요청'(draft) 전용 — 발행 확정 버튼/testid 부재.
    expect(box).toContain('data-testid="opinion-req-submit"');
    expect(box).not.toContain('data-testid="opinion-publish"');
    const l = lib();
    // (5) 데이터 레이어도 실장 경로는 draft만 — publish RPC 호출 없음(주석 언급은 경계 문서화로 허용).
