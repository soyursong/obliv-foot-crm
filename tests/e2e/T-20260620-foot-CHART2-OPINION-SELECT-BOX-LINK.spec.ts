...
test.describe('T-20260620-foot-CHART2-OPINION-SELECT-BOX-LINK — 소견서 선택박스 연동', () => {
  ...
  // AC-7(P1-1): 진단서(표준)=단일배타 / 금기증=복수선택 — 원장 작성창과 동일 엔진 재사용
  test('AC-7-P1-1: 진단서 단일배타 XOR 금기증 복수 selection rule 와이어링', () => {
    const box = reqBox();
    // 원장 작성창(OpinionDocTab)과 동일 selection 엔진 재사용(일관성)
    expect(box).toContain('buildContraindKeySet');
    expect(box).toContain('classifySelection');
    expect(box).toContain("from '@/lib/opinionDocCompose'");
    // 단일배타 핸들러 — 진단서 클릭 시 다른 선택 전부 해제(next.clear()) 후 1개만
    expect(box).toContain('handleOptionClick');
    expect(box).toContain('contraindKeySet.has');
    expect(box).toContain('next.clear()');
    // 그룹 상태 + 버튼 배타 disable(진단서 선택 시 그 외 비활성 / 금기증 선택 시 진단서 비활성)
    expect(box).toContain('hasDiagnosis');
    expect(box).toContain('hasContraind');
    expect(box).toContain('disabled={disabled}');
    // 더 이상 무조건 다중 토글(toggle)이 아님 — 옛 toggle 핸들러 제거 확인(회귀 가드)
    expect(box).not.toContain('onClick={() => toggle(opt.key)}');
  });
