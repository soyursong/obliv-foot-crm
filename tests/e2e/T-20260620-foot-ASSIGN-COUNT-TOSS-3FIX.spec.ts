...
test('AC1-1: 당월 누적 배정/재진 카운트가 check_ins(monthCheckIns) 정본을 집계한다', () => {
  ...
  //   T-20260720-foot-ASSIGN-LABEL-DATE-SELECT: 일누적/당월누적 분리로 bumpAssign(isReturning) 형태로 리팩터
  //   — 재진 판정→returning 카운터, 그 외→assigned 카운터 라는 불변식은 보존.
  expect(src).toMatch(/const key = isReturning \? 'returning' : 'assigned'/);
